(()=>{
  ...
  const overlay=document.createElement('div');overlay.className='search-overlay';overlay.hidden=true;overlay.innerHTML='<section class="search-dialog" role="dialog" aria-modal="true" aria-labelledby="global-search-title" tabindex="-1"><header class="search-dialog-head"><div><p class="eyebrow">public archives</p><h2 id="global-search-title">전체 기록 검색</h2></div><button type="button" class="search-close" data-search-close aria-label="검색 닫기">닫기 <kbd>Esc</kbd></button></header><label class="global-search-field"><span>Wiki, Review, Post에서 찾기</span><input type="search" data-global-search placeholder="제목, 저자, 요약, 태그" autocomplete="off"></label><p class="global-search-count" data-global-search-count aria-live="polite"></p><div class="global-search-results" data-global-search-results></div></section>';document.body.appendChild(overlay);
  ...
  // 모달 동안 배경은 inert 로 막아 Tab·스크린리더 탐색이 뒤로 새지 않게 한다. 원래 inert 였던 요소는 건드리지 않는다.
  const dialog=overlay.querySelector('.search-dialog');let inerted=[];
  const setBackground=off=>{if(off){inerted=[...document.body.children].filter(el=>el!==overlay&&!el.inert);inerted.forEach(el=>{el.inert=true})}else{inerted.forEach(el=>{el.inert=false});inerted=[]}};
  const open=trigger=>{if(!overlay.hidden)return;restoreFocus=trigger||document.activeElement;overlay.hidden=false;document.body.classList.add('search-open');setBackground(true);input.value='';resultCount.textContent='불러오는 중…';results.replaceChildren();load().then(render).catch(()=>{resultCount.textContent='검색 인덱스를 불러오지 못했습니다.'});input.focus();if(document.activeElement!==input)dialog.focus()};
  const close=()=>{if(overlay.hidden)return;overlay.hidden=true;document.body.classList.remove('search-open');setBackground(false);const target=restoreFocus;restoreFocus=null;if(target&&target!==document.body&&target.isConnected)target.focus?.({preventScroll:true})};
  document.querySelectorAll('[data-search-open]').forEach(b=>b.addEventListener('click',()=>open(b)));overlay.querySelector('[data-search-close]').addEventListener('click',close);overlay.addEventListener('click',e=>{if(e.target===overlay)close()});input.addEventListener('input',render);
  // inert 미지원 브라우저용 안전망: 포커스가 대화상자 밖으로 나가면 되돌린다.
  document.addEventListener('focusin',e=>{if(!overlay.hidden&&!overlay.contains(e.target))input.focus()});
  document.addEventListener('keydown',e=>{if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='k'){e.preventDefault();overlay.hidden?open():close()}else if(e.key==='Escape'&&!overlay.hidden){e.preventDefault();close()}else if(!overlay.hidden&&window.FocusTrap)window.FocusTrap.handleTab(dialog,e,dialog)});
})();
