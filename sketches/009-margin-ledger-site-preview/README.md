...
| 파일 | 역할 | 주요 이동 |
| --- | --- | --- |
| `index.html` | 홈, 날짜순 wiki/review/Medium 혼합 스트림 | 필터, 열린 리뷰 표지, 각 인덱스와 대표 상세 |
...
- Review는 표지를 기본으로 펼친다. `<details open>`을 사용해 JavaScript가 없어도 내용과 표지가 보이며, 개별 접기와 일괄 접기 모두 지원한다.
...
- 홈: `all / wiki / reviews / elsewhere` 필터.
