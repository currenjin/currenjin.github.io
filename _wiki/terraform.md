...
summary : 핵심 개념, 실무 모범 사례, AWS · Docker 튜토리얼
date    : 2026-04-07 00:00:00 +0900
updated : 2026-04-10 00:00:00 +0900
...
HashiCorp Terraform의 핵심 개념, 실무 모범 사례, 튜토리얼을 정리한다.
공식 문서([Docs](https://developer.hashicorp.com/terraform/docs), [Tutorials](https://developer.hashicorp.com/terraform/tutorials))와 [Terraform Best Practices](https://www.terraform-best-practices.com/)를 기반으로 작성했다.

---

## 1) Terraform이란

인프라를 코드로 정의하고, 버전 관리하며, 안전하게 프로비저닝하는 도구다.
...
- **Provider 생태계** — AWS, Azure, GCP, Kubernetes, GitHub, Datadog 등 수천 개 지원
...
- `nullable = false` 로 null 허용 여부를 명시한다
- list/map 타입 변수는 **복수형** 이름을 사용한다
...
로컬 State는 협업 시 레이스 컨디션을 유발한다. **처음부터 원격 백엔드를 사용한다.**
...
- 원격 백엔드(HCP Terraform, S3+DynamoDB 등)의 State locking이 동시 실행 충돌을 방지한다
...
| 항목 | 고려 방향 |
|------|-----------|
| 프로젝트 복잡도 | 리소스 수와 Provider 다양성 |
| 변경 빈도 | 월간 업데이트 vs 지속적 배포 |
| 접근 제어 | 개발자 직접 적용 vs PR 기반 |
| 환경 구조 | 환경·리전·프로젝트 단위 구분 |

> 리소스 컬렉션이 작을수록 `terraform plan`, `terraform apply` 가 빠르다.
> 관련 없는 리소스를 분리하면 보안 침해 시 피해 범위도 줄어든다.
...
```hcl
# 이 방식만 사용한다 — // 이나 /* */ 는 쓰지 않는다

# ──────────────────────────────────────
# 섹션 구분선으로 큰 모듈을 시각적으로 나눈다
# ──────────────────────────────────────
```
...
**versions.tf** — Terraform과 Provider 버전을 선언한다:
...
## 17) 튜토리얼: Docker 컨테이너 관리

AWS 계정 없이 로컬에서 바로 실습할 수 있다.
