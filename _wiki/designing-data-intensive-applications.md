...
updated : 2026-06-25 14:32:31 +0900
...
### 영상 메모: 품질 속성은 숫자로 관측되어야 한다

* 영상에서는 신뢰성, 확장성, 유지보수성을 추상적 구호가 아니라 관측 가능한 품질 속성으로 다룬다.
* 신뢰성은 “장애가 안 난다”가 아니라 결함이 사용자 장애로 번지지 않게 하는 능력이다.
  * 하드웨어 이중화만으로 충분하지 않다.
  * 소프트웨어 버그와 운영 실수가 더 긴 장애를 만들 수 있다.
* 확장성은 시스템이 커질 수 있다는 말이 아니라 특정 부하 매개변수 변화에 어떻게 반응하는지 설명하는 일이다.
  * 평균 응답 시간보다 p95/p99가 사용자 불만을 더 잘 드러낸다.
  * 팬아웃, hot key, queue depth 같은 도메인별 부하 지표가 필요하다.
* 유지보수성은 코드를 예쁘게 만드는 문제가 아니라 운영자가 이해하고 고칠 수 있는 시스템을 만드는 일이다.
* 영상 토론에서는 RAID, Twitter fan-out 같은 예시가 나오며, 같은 설계도 부하의 모양에 따라 장점과 단점이 바뀐다는 점이 강조된다.
...
### 영상 메모: 모델 선택은 “데이터 모양”보다 “질문 모양”에서 시작한다

* 영상에서는 관계형/문서형/그래프 모델을 기능 목록으로 비교하기보다, 애플리케이션이 자주 던지는 질문에서 출발해야 한다는 점이 강조된다.
  * 조인이 많고 데이터 간 관계가 계속 바뀌면 관계형 모델이 단순하다.
  * 한 객체 단위로 자주 읽고 쓰며 하위 구조가 함께 움직이면 문서 모델이 읽기 경로를 줄인다.
  * 연결 관계 자체가 핵심이면 그래프 모델이 탐색 의도를 더 직접 표현한다.
* NoSQL은 “SQL의 대체재”라기보다 특정 접근 패턴을 더 싸게 처리하기 위한 선택지로 보는 편이 자연스럽다.
* 문서 모델의 유연성은 스키마 설계를 없애지 않는다.
  * 스키마 검증 시점이 쓰기에서 읽기로 이동할 뿐이다.
  * 읽는 코드가 늘어날수록 암묵적 스키마 비용이 커진다.
* 실무 판단 기준은 “나중에 어떤 쿼리를 안정적으로 바꿀 수 있는가”다.
  * 데이터 모델은 저장 구조이면서 동시에 팀의 사고방식과 변경 비용을 결정한다.
...
### 영상 메모: 인덱스는 읽기 최적화가 아니라 쓰기 비용의 재배치다

* 영상 논의에서 반복되는 관점은 인덱스를 “공짜 성능 향상”으로 보면 안 된다는 점이다.
  * 읽기는 빨라지지만 모든 쓰기는 인덱스 갱신 비용을 함께 부담한다.
  * 쿼리 패턴이 바뀌면 기존 인덱스는 부채가 될 수 있다.
* LSM-Tree와 B-Tree의 차이는 단순히 새 구조와 오래된 구조의 차이가 아니다.
  * LSM은 순차 쓰기와 높은 write throughput에 강하다.
  * B-Tree는 안정적인 point/range read와 예측 가능한 지연에 강하다.
* 컴팩션은 백그라운드 작업처럼 보이지만 실제로는 foreground 성능을 흔들 수 있다.
  * compaction debt가 쌓이면 읽기 증폭과 쓰기 지연이 같이 증가한다.
* 열 지향 저장소는 “컬럼이 많아서”가 아니라 “대량 스캔 중 필요한 컬럼만 읽기 위해” 선택한다.
  * OLTP와 OLAP는 같은 데이터를 다뤄도 I/O 패턴이 다르다.
...
### 영상 메모: 호환성은 포맷보다 배포 순서의 문제다

* 영상에서는 JSON, Protocol Buffers, Avro 같은 포맷 차이보다 신/구 코드가 동시에 존재하는 상황이 더 중요하게 다뤄진다.
* 스키마 변경은 한 번에 끝나는 작업이 아니다.
  * 먼저 새 필드를 읽을 수 있게 만든다.
  * 그 다음 새 필드를 쓰기 시작한다.
  * 마지막에 오래된 필드와 코드를 제거한다.
* 메시지 브로커를 사이에 둔 시스템에서는 생산자와 소비자가 같은 속도로 배포되지 않는다.
  * 소비자가 모르는 필드를 무시할 수 있어야 한다.
  * 새 소비자는 예전 메시지를 읽을 수 있어야 한다.
* 스키마 레지스트리는 “문서화 도구”가 아니라 배포 전 호환성 오류를 막는 안전장치다.
* 직렬화 포맷 선택은 디버깅 편의성, 크기, 속도, 타입 안정성, 조직의 운영 방식까지 함께 고려해야 한다.
...
### 영상 메모: 복제 토폴로지는 장애 시나리오까지 포함해 선택한다

* 영상에서는 leader, follower, multi-leader, leaderless를 정상 상태의 읽기/쓰기 흐름만으로 비교하지 않는다.
  * 리더가 죽었을 때 누가 승격되는가.
  * 승격 중 손실 가능한 쓰기는 어느 정도인가.
  * 클라이언트는 어느 노드에서 읽어야 하는가.
* 비동기 복제는 평상시에는 빠르지만 failover 순간에 데이터 손실 가능성을 만든다.
* 복제 지연은 단순한 “조금 늦음”이 아니라 사용자 경험의 일관성을 깨뜨린다.
  * 방금 쓴 글이 안 보이는 read-your-writes 위반.
  * 새로고침할수록 과거 상태로 돌아가는 monotonic reads 위반.
* multi-leader는 지리 분산과 오프라인 쓰기에 유리하지만 충돌 해결을 애플리케이션 문제로 끌어올린다.
* quorum 기반 모델은 숫자 계산보다 장애 상황에서 어떤 읽기/쓰기를 허용할지의 정책이다.
...
### 영상 메모: 파티션 키는 분산뿐 아니라 질의 경로를 결정한다

* 영상 논의의 핵심은 파티셔닝을 저장 용량 분할로만 보면 부족하다는 점이다.
  * 어떤 요청이 단일 파티션에서 끝나는가.
  * 어떤 요청이 scatter-gather가 되는가.
  * 어떤 키가 핫스팟을 만드는가.
* 해시 파티셔닝은 균등 분산에는 좋지만 범위 질의를 어렵게 만든다.
* 범위 파티셔닝은 정렬과 range scan에 좋지만 시간순 쓰기처럼 한쪽으로 몰리는 패턴에 약하다.
* 보조 인덱스는 파티셔닝 이후 더 복잡해진다.
  * 로컬 인덱스는 쓰기가 단순하지만 읽기 fan-out이 생긴다.
  * 글로벌 인덱스는 읽기가 빠르지만 갱신과 리밸런싱 비용이 커진다.
* 리밸런싱은 노드 추가 이벤트가 아니라 지속적인 운영 기능으로 설계해야 한다.
...
### 영상 메모: 트랜잭션은 “안전한 착각”을 제공한다

* 영상에서는 트랜잭션을 성능 기능이 아니라 복잡한 동시성 상황을 단순한 모델로 숨겨주는 장치로 설명한다.
* 격리 수준 이름은 데이터베이스마다 다르게 구현될 수 있으므로 이름만 믿으면 안 된다.
  * 실제로 어떤 anomaly를 막고 허용하는지 확인해야 한다.
* write skew는 특히 애플리케이션 불변식을 깨뜨린다.
  * 각 트랜잭션은 올바른 판단을 했지만 합쳐진 결과는 틀릴 수 있다.
* 계좌 이체 같은 예시는 ACID의 필요성을 잘 보여준다.
  * 중간 실패에도 총액 불변식이 유지되어야 한다.
  * 동시 실행에도 잃어버린 업데이트가 없어야 한다.
* 분산 트랜잭션은 원자성을 넓혀주지만 운영 난이도와 장애 모드를 함께 키운다.
  * 서비스 간 흐름에서는 SAGA, Outbox, 멱등성이 더 실용적인 경우가 많다.
...
### 영상 메모: 분산 시스템의 기본값은 “모른다”이다

* 영상에서는 분산 시스템을 느린 단일 시스템으로 보지 말고, 관측 불가능성이 있는 시스템으로 봐야 한다는 점이 드러난다.
* 응답이 없다는 사실은 많은 가능성을 남긴다.
  * 요청이 도착하지 않았을 수 있다.
  * 요청은 처리됐지만 응답이 사라졌을 수 있다.
  * 상대가 느릴 뿐 아직 살아 있을 수 있다.
* 타임아웃은 진실을 알려주는 도구가 아니라 의심을 결정하는 정책이다.
* 로드밸런서, 네트워크 큐, 대역폭, GC pause 같은 요소가 모두 tail latency를 만든다.
* 리더십과 락은 시간만으로 보호하기 어렵다.
  * 멈췄던 프로세스가 돌아와 예전 권한으로 쓰지 못하게 fencing token이 필요하다.
...
### 영상 메모: 강한 일관성은 읽기 편한 대신 운영 비용을 요구한다

* 영상에서는 linearizability를 “가장 최근 값을 읽는다”보다 “단일 복사본처럼 보인다”는 관점으로 다룬다.
* 선형성은 분산 락, 리더 선출, 유일 ID/제약처럼 잘못되면 복구가 어려운 곳에 필요하다.
* 반대로 모든 읽기에 선형성을 요구하면 지연과 가용성 비용이 커진다.
* 순서 보장은 합의의 핵심이다.
  * 모든 노드가 같은 순서로 작업을 적용하면 복제 상태를 맞출 수 있다.
  * 로그 복제 기반 시스템은 이 아이디어 위에 있다.
* 2PC는 원자적 커밋을 제공하지만 coordinator 장애 시 블로킹 문제가 있다.
* 합의 시스템은 마법의 저장소가 아니라 작은 메타데이터와 순서 결정에 신중히 사용해야 하는 비용 큰 도구다.
...
### 영상 메모: 배치의 힘은 재실행 가능성에서 나온다

* 영상 논의에서는 배치 처리를 오래된 기술이 아니라 재현 가능한 데이터 변환 방식으로 본다.
* Unix 철학은 데이터 파이프라인에도 그대로 적용된다.
  * 입력은 그대로 둔다.
  * 출력을 새로 만든다.
  * 작은 단계를 조합한다.
* MapReduce의 핵심은 map/reduce 함수보다 shuffle이다.
  * 같은 key를 같은 장소로 모으는 비용이 전체 성능을 좌우한다.
* 조인은 데이터 크기와 분포에 따라 전략이 달라진다.
  * 작은 테이블은 broadcast가 유리하다.
  * 큰 테이블끼리는 partition/sort 기반 조인이 필요하다.
* Spark 같은 현대 엔진은 실행 모델을 개선했지만 shuffle, skew, 메모리 압박 문제를 없애지는 않는다.
* 배치 결과는 원자적으로 공개되어야 한다.
  * 실패한 중간 결과를 사용자가 읽지 못하게 해야 한다.
...
### 영상 메모: 스트림은 “작은 배치”가 아니라 시간과 상태의 문제다

* 영상에서는 스트림 처리를 단순히 빠른 배치로 보지 않는다.
  * 이벤트가 언제 발생했는가.
  * 언제 시스템에 도착했는가.
  * 어느 상태와 결합되어야 하는가.
* at-least-once 전달은 실무 기본값에 가깝지만 중복 처리를 애플리케이션에 요구한다.
* exactly-once는 시스템 경계 안에서만 의미가 분명하다.
  * 외부 DB, API, 메시지 브로커까지 이어지는 end-to-end 정확성은 별도 설계가 필요하다.
* watermark와 allowed lateness는 정확도와 지연 사이의 정책이다.
* stream-table duality 관점이 중요하다.
  * 스트림은 변경의 연속이다.
  * 테이블은 그 변경을 누적한 현재 상태다.
* 여러 consumer가 같은 메시지를 다른 속도로 처리하므로 lag와 backpressure를 운영 지표로 봐야 한다.
...
### 영상 메모: 미래의 데이터 시스템은 하나의 DB보다 데이터 흐름 설계에 가깝다

* 영상의 마지막 장 논의는 특정 신기술보다 여러 시스템을 조합할 때 생기는 책임에 초점이 있다.
* 파생 데이터는 읽기 성능을 위해 필요하지만 원천 데이터와의 차이를 계속 만든다.
  * 검색 인덱스, 캐시, 분석 테이블은 모두 복구 가능한 파생물이어야 한다.
* 로그 중심 아키텍처는 변경 이벤트를 여러 읽기 모델로 흘려보내는 방식이다.
  * 장점은 통합과 재처리다.
  * 단점은 운영 복잡도와 지연 관측 필요성이다.
* Lambda/Kappa 논의는 “배치냐 스트림이냐”보다 재처리와 정확성을 어떻게 확보할지의 문제다.
* end-to-end 정확성은 개별 컴포넌트 보장보다 넓다.
  * 클라이언트 재시도, 서버 커밋, 이벤트 발행, 소비자 반영까지 하나의 경로로 봐야 한다.
* 데이터 시스템 설계는 윤리와 개인정보 문제도 포함한다.
  * 수집, 보존, 삭제, 감사 가능성을 기술 설계의 일부로 다뤄야 한다.
