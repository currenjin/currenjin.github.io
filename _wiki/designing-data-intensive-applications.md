...
updated : 2026-06-30 13:52:00 +0900
...
- 하드웨어 결함
  ...
  - 더 큰 데이터 및 컴퓨팅 요구로 인해 종종 소프트웨어 내결함성 기술을 사용하여 다중 시스템 중복으로 전환되었습니다.
- 소프트웨어 오류
...
### 맵리듀스 쿼리
- 명령형과 선언형 사이
- 두 개의 별도 작업을 작성하고 매핑하고 축소합니다.
  - 단일 행에 외부 정보나 작업이 필요하지 않은 순수 함수여야 합니다.
### 그래프와 유사한 데이터 모델
- 다대다 관계를 더 잘 처리합니다.
...
- 이제 두 가지 백그라운드 프로세스가 있습니다. 세그먼트 압축 외에도 memtable의 일괄 처리를 새 파일 세그먼트에 주기적으로 작성합니다.
- 최적화에는 다음이 포함됩니다.
...
### 거래 처리 또는 분석
- 초기 비즈니스 데이터 처리는 상업 거래에 의해 주도되었으며, 이는 온라인 거래 처리(OLTP)라는 용어로 이어졌습니다.
...
- 컬럼 저장소의 정렬 순서
  - 바닐라 로그 파일이 특별한 순서 없이 추가 전용이지만 SSTable이 정렬되는 것처럼 열 저장소 데이터를 정렬할 수 있습니다.
  - 정렬 순서는 정렬 키 열을 압축하는 데 도움이 됩니다.
...
- 컬럼 저장소에 대한 쓰기가 더 복잡합니다.
  - LSM 트리와 마찬가지로 일괄 데이터가 메모리에 축적된 다음 주기적으로 병합되어 디스크에 기록되는 2단계 구조를 가질 수 있습니다.  정렬 순서와 압축 작업은 백그라운드에서 일괄 처리됩니다.

### 집계: 데이터 큐브 및 구체화된 뷰
- 특정 집계가 자주 요청될 것임을 알고 있는 경우(예: 하루에 매장별 부서별 매출 합계를 계산하면 해당 데이터의 물리적 복사본을 구체화된 뷰에 저장할 수 있습니다.
- 데이터 큐브는 하나의 뷰뿐만 아니라 여러 차원에 의한 집계를 저장합니다. 백화점별 매출 합계, 매장별 합계, 일별 합계, 백화점&점포별 합계, 백화점&일별 합계, 매장&일별 합계, 백화점&점포&일별 합계입니다.  이 예와 같이 3차원을 시각화하면 모든 백화점별, 매장별, 요일별 합계를 하나의 큐브로 정리하게 됩니다.  두 요소로 구성된 소계는 행, 열 또는 스택별로 합산됩니다.  단일 요소 부분합은 숫자의 제곱에 대한 합계입니다.

### 요약
- 이 장에서는 관계형 스토리지와 로그 구조 스토리지를 중점적으로 다뤘습니다.
  - 관계형은 각 레코드의 복사본을 하나만 유지하면서 업데이트되지만 임의 액세스가 필요합니다(페이지에 집계됨).
  - 로그 기반 집중은 더 적은 수의 순차적 청크로 기록하지만 여러 데이터 구조가 필요하고 더 이상 사용되지 않는 레코드 사본이 더 고급 사용 사례를 복잡하게 만들 수 있습니다.
...
- 현재 버전은 전체 텍스트 색인 및 공간 색인을 지원합니다.
- 데이터 저장은 매우 간단합니다.
  - 테이블은 B-트리입니다.
  - 인덱스는 B-트리입니다.
...
- 데이터를 인코딩하는 방법에는 여러 가지가 있습니다(JSON, XML, Protos, Thrift, Avro). 이 장에서 다루는 주요 질문은 다음과 같습니다.
  ...
  - 이러한 각 형식은 데이터 저장 및 통신에 어떻게 사용됩니까? (웹 서비스, REST, RPC, 메시지 전달 - 행위자 및 대기열)
...
### 3. 이러한 라이브러리는 이전 버전/앞 버전 호환성을 무시할 수 있습니다. 버전 관리는 나중에 생각하는 것입니다. 빠르고 쉬운 인코딩을 의미합니다.
...
- 프로토콜 버퍼
  ...
  - 이전 버전과의 호환성: 필드 태그를 변경할 수 없습니다. 변경하면 기존의 모든 인코딩된 데이터가 무효화됩니다. 태그는 바이트 시퀀스에서 많이 사용됩니다(그림 4-4 참조). 각각의 새 필드에는 새 태그 번호가 필요합니다. 새 코드는 이전 코드로 인코딩된 데이터를 읽을 수 없으므로 새 필드가 필요할 수 없습니다(필요한 데이터가 인코딩되지 않았을 수 있음). 이전 버전과의 호환성을 유지하려면 초기 배포 이후의 모든 필드는 선택 사항이거나 기본값을 가져야 합니다.
  - 상위 호환성: 이전 코드는 새 태그 번호를 무시할 수 있습니다.
...
- Apache Avro: Protos 및 Thrift와는 다른 바이너리 인코딩 형식입니다.
  ...
  - 인코딩된 데이터는 쓰기 시 사용되는 스키마인 "작성자의 스키마"를 사용합니다. 읽을 때 "리더의 스키마", 즉 애플리케이션 코드가 의존하는 스키마를 사용하며 빌드 프로세스 중에 생성되었을 수 있습니다.
  ...
  - 필드 이름과 "공용체 유형"을 변경할 때 이전 버전과의 호환성 제한이 있습니다.
  - 작가의 도식이 무엇인지 독자는 어떻게 알 수 있는가? 차이점을 해결하려면 독자에게 작성자 스키마가 필요합니다.
    - 작성자 스키마가 파일 상단에 존재할 수 있습니다. Avro는 동일한 스키마의 수백만 개의 레코드가 있는 Hadoop에서 일반적으로 사용됩니다.
- 또는 - 작성자 스키마의 버전 번호가 기록에 포함될 수 있습니다. 리더는 버전 번호를 가져온 다음 올바른 작성자 스키마를 조회하고 가져올 수 있습니다.
    - 또는 - 스키마 버전은 연결 설정 시작 시 전달될 수 있습니다. Avro RPC 프로토콜을 참조하세요.
  - 스키마 데이터베이스는 어떤 옵션을 사용하든 유용합니다. 문서화 및 스키마 호환성(?)을 확인할 수 있는 기회입니다.
...
- 요약: 바이너리 인코딩은 더 간결하고, 스키마는 문서를 제공하며(디코딩에 필요하기 때문에 항상 최신 상태임), 스키마 데이터베이스를 유지하면 이전 버전과 이후 버전의 호환성을 확인할 수 있으며, 코드 생성은 컴파일 시 유형 확인에 유용합니다.

### 데이터 흐름 모델: 프로세스 간에 데이터가 어떻게 흐르나요?
- 데이터베이스를 통한 데이터 흐름
  - 이전 버전과의 호환성이 필요합니다. 데이터베이스에서 쓰고 읽는 프로세스는 하나만 있을 수 있습니다. 해당 프로세스와 데이터베이스의 향후 버전도 이전 데이터를 읽어야 합니다.
  - 상위 호환성도 필요합니다. 많은 프로세스가 데이터베이스를 읽고 씁니다. 일부 프로세스는 최신 업데이트를 받았을 수 있으며 최신 코드로 데이터베이스에 새 데이터를 쓰고 있습니다. 그러면 이전 코드에서 해당 데이터를 읽을 수 있습니다.
...
- 클라이언트와 서버가 서로 다른 언어를 사용하는 경우 언어 간 데이터 유형 호환성 문제가 발생할 수 있습니다.
  ...
  - RPC의 이전 버전 및 이후 버전과의 호환성 속성은 RPC가 사용하는 인코딩에서 상속됩니다.
...
- 메시지 전달을 통한 데이터 흐름
  - 메시지는 "큐" 또는 "주제"로 전송됩니다. 메시지 브로커는 해당 주제에 대해 1명 이상의 "소비자" 또는 "구독자"에게 메시지가 전달되도록 합니다.
  - 비동기 메시지 전달 시스템은 RPC와 데이터베이스 사이에 있습니다. 혜택:
  ...
  - 한 주제의 소비자는 메시지를 처리하고 다른 주제에 대기열에 추가할 수도 있습니다! 또는 원래 보낸 사람이 응답을 받을 수 있도록 응답 대기열에 추가할 수도 있습니다.
  - 모든 인코딩 형식을 사용할 수 있습니다. 해당 인코딩이 이전 버전 및 이후 버전과 호환되는 한 게시자와 소비자는 독립적으로 배포 및 출시될 수 있습니다.
  - 분산 액터 프레임워크: 로직이 스레드가 아닌 액터에 캡슐화됩니다. 애플리케이션은 여러 노드에 걸쳐 확장됩니다. 위치 투명성이 지원됩니다. 발신자와 수신자는 동일한 노드 또는 다른 노드에 있을 수 있으며 행위자 모델이 메시지가 손실될 수 있다고 가정하므로 RPC보다 더 잘 작동합니다. <<< 여기에 더 많은 예제가 필요합니다. 손실된 메시지를 처리하는 행위자 모델이 위치 투명성에 더 나은 이유를 완전히 이해하지 못합니다.
...
- 공유 메모리 및 공유 디스크 아키텍처가 있지만 이 논의는 비공유 아키텍처에 관한 것입니다. 왜냐하면 다른 것들은 특히 지리적 확장에 심각한 제한이 있기 때문입니다.
...
### 단일 리더 복제
### 리더와 추종자
- 하나의 복제본/노드가 리더로 지정됩니다. 다른 사람들은 모두 추종자입니다
- 모든 쓰기는 리더를 거쳐야 합니다. 읽기는 모든 복제본에서 올 수 있습니다.
- 쓰기 시 리더는 변경 로그 정보를 모든 팔로어에게 보냅니다.
  - 동기식 - 명확한 성공/실패; 클라이언트는 모든 복제본을 기다려야 합니다.
  - 비동기식 - 대기 시간이 가장 낮습니다. 쓰기 성공 후 내구성이 보장되지 않습니다.
  - 단일 동기 팔로워로 내구성 향상
- 팔로어 추가에는 일반적으로 스냅샷(예: 백업)과 스냅샷 이후 변경 사항 적용이 포함됩니다.
- 추종자 실패는 따라잡기 회복을 사용합니다.

### 리더 실패 - 장애 조치
- 리더가 실패하면 추종자 중 한 명을 리더로 승격시킵니다.  간단해 보이지만…
...
- 쉬운 대답이 없기 때문에 일부 운영 팀은 수동으로 장애 조치를 수행합니다.
...
### 복제 지연 문제
- 현실적으로 많은 팔로어를 비동기식으로만 확장할 수 있습니다.
- 이상적으로 최종 일관성은 짧지만 복제 지연은 몇 초 또는 몇 분이 될 수 있습니다.
- 자신이 쓴 글을 읽는다

### 오래된 복제본에서 쓰기 후 읽기는 사용자에게 데이터가 손실된 것처럼 보입니다.
...
### 다중 리더 복제
  - 다수의 리더가 있는 경우 각 리더는 다른 리더의 추종자 역할도 수행합니다.
...
### 비활성 모니터링
    - 리더 기반 복제를 사용하면 각 리더와 관련하여 각 팔로워의 "버전"을 비교하여 팔로어가 쓰기 적용에 얼마나 뒤떨어져 있는지 확인할 수 있기 때문에 일반적으로 부실 상태를 모니터링하는 것이 매우 쉽습니다.
...
### 다중 데이터센터 운영
    - 여러 데이터 센터를 포함하는 복제는 각 데이터 센터의 n개 노드 중 일부를 사용하여 구현할 수 있지만 그 외에는 바닐라 복제와 유사합니다.
...
- 거래는 하나의 단위로 그룹화된 일련의 작업으로, 시스템은 위에서 언급한 특정 보증 세트를 제공합니다.
...
- 어떤 종류의 대기열로 인해 지연이 발생하는 경우가 많습니다.
...
- 이더넷과 IP는 패킷 교환 프로토콜이므로 대기열이 필요하며 이로 인해 무한한 지연이 발생합니다.
...
- 여러분 중에 나쁜 행위자가 있을 수 있으며 이에 맞서 싸우려면 다수와 동의를 얻어야 합니다.
...
- 참고: 선형화 가능성과 직렬화 가능성은 비슷하게 들리지만 독립적인 개념입니다.  시스템은 이들 중 어느 것도 제공하지 않거나, 하나 또는 둘 다를 제공할 수 없습니다.
### 선형성에 의존
- 사용 사례는 다음과 같습니다.
  - 잠금 및 리더 선출
  - 고유한 제약 조건
...
- 복제된 시스템은 다음과 같이 선형화될 수 있습니다.
  - 단일 리더 복제 - 잠재적으로; 스냅샷 격리를 사용하는 경우는 아님, 비동기식 쓰기를 사용하는 경우는 아님 리더 장애 조치가 만족스러운지 확인해야 함
  - 합의 알고리즘 -- 예
  - 다중 리더 복제 - 선형적이지 않고 동시 쓰기 가능
  - 리더 없는 복제 - 아마도 선형화가 불가능할 것입니다. 갈등 해결에 달려 있으며 엄격한 정족수를 요구합니다.
...
### 분산 거래 및 합의
- 리더 선택 및 원자적 커밋(여러 노드에 걸쳐)을 포함하여 모든 노드가 특정 사항에 동의하기를 원합니다.
...
### 실제 분산 트랜잭션
- 분산 거래에는 장단점이 있습니다.  원자적 커밋은 바람직한 기능이지만 많은 시스템에 성능 문제와 운영 문제가 있습니다.
- 저자는 두 가지 사용 사례를 구분합니다.
  - 데이터베이스 내부 - 모두 동일한 소프트웨어에 있음
  - 이기종 분산 트랜잭션 - 다른 시스템과 호환되며 지원하기가 더 어렵습니다.
- 정확히 한 번만 메시지 처리 - 메시지 대기열에서 메시지 확인을 원자적으로 커밋하고 데이터베이스 쓰기를 하나의 분산 트랜잭션으로 수행합니다.
...
- 스큐 처리
  ...
  - 다양한 도구를 활용한 전투 가능
### 맵 측 조인
- 입력 데이터에 대해 가정을 할 수 있다면 맵 측 조인을 사용하여 더 빠르게 조인할 수 있습니다.
...
- 시스템이 Google의 지도 축소 작업에 대한 우선순위를 잃기 때문에 내결함성을 염두에 두고 설계되었습니다. 반드시 기계가 신뢰할 수 없기 때문은 아닙니다.
### 맵리듀스를 넘어서
- 지도 축소 작업을 작성하기 어렵습니다. 털이 많은 부분을 더 쉽게 추상화할 수 있도록 다양한 도구가 작성되었습니다.
  - 돼지, 하이브, 크런치, 캐스케이딩
### 중간상태의 구현
...
- 데이터 흐름 엔진은 다양한 방법으로 이러한 점 중 일부를 완화하려고 시도합니다.
  - 스파크, 테즈, 플링크
- 구체화되지 않으면 일부 내결함성이 더 까다로워지지만 꼭 필요한 것은 아닐 수도 있습니다.
### 그래프 및 반복 처리
...
- 소비자가 생산자를 따라갈 수 없는 경우 - 일반적으로 메시지 손실이 발생할 수 있지만 이러한 일이 발생하기 전에 관리자에게 경고할 수 있습니다.  부수적인 이점은 종료된 소비자가 큰 메시지 대기열을 발생시키지 않는다는 것입니다.
...
- CDC는 변경 사항을 즉시 스트림으로 제공할 수 있으며, 검색 색인 등 다른 시스템에서 지속적으로 적용할 수 있습니다.
- 로그 기반 메시지 브로커는 메시지 순서를 보존하여 올바른 종류의 전달을 제공할 수 있습니다.
- 기본적으로 하나의 데이터베이스가 단일 리더가 되고 나머지 데이터베이스는 팔로어 역할을 합니다.
...
- 매핑 및 필터링은 파티셔닝 및 병렬화와 마찬가지로 일괄 처리와 유사하게 작동합니다.
...
- 구체화된 뷰 유지 - 논의된 대로 데이터의 또 다른 복사본을 유지할 수 있습니다.
...
- 테이블-테이블 조인(구체화된 뷰 유지 관리) - 두 테이블 간 조인의 구체화된 뷰를 업데이트하는 스트림의 개념입니다.  두 스트림 중 하나를 변경하면 조인 출력에서 ​​다른 테이블의 여러 행에 잠재적으로 알려야 합니다.
...
- 귀하의 모든 요구 사항을 충족하는 기술이 하나도 없고 여러 가지 기술을 함께 사용해야 하는 경우가 있을 수 있습니다.
### 데이터 도출을 통한 전문 도구 결합
...
- 구체화된 뷰 및 캐싱은 사전에 아무것도 처리하지 않고 모든 것을 처리할 수 있으며 읽기 경로와 쓰기 경로를 교환할 수 있습니다.
...
- 계정이 마이너스가 되거나 보유하지 않은 주식을 판매하는 등의 일이 발생하지 않도록 할 수 있습니다.
  ...
  - 거래 예 - 단일 요청을 기록하고 단일 지시사항에서 대변 및 차변 지시사항을 파생시킬 수 있습니다.
### 적시성 및 무결성
