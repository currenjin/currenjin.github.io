...
layout  : wiki
title   : 증강 코딩 운영 기록
summary : 개인 검증 이후 팀 적용까지, 실제 운영 방식 정리
date    : 2026-03-12 11:30:00 +0900
updated : 2026-03-12 15:10:00 +0900
...
처음에는 바이브 코딩이나 AI 전면 위임 방식에 회의적이었다.
이유는 세 가지였다.
...
이후 Kent Beck의 증강 코딩 관점을 참고하면서 방향을 바꿨다.
핵심은 “AI가 다 한다”가 아니라, 사람이 목표와 제약, 완료 기준을 먼저 정하고 AI를 그 안에서 실행시키는 방식이라는 점이었다.
...
팀에서 실제로 바뀐 행동은 "AI를 작업자로 두고, 작업자로서의 의무(보고/수정/반복)를 수행하도록 의도하는 방식"으로 운영한 점이다.

핵심은 개인 역량 편차를 줄이고, 팀 기준으로 결과를 안정화하는 데 있다.
...
운영 원칙은 간단하다.
자동화는 실행량을 늘리는 데 쓰고, 사람은 고비용 판단에 집중한다.
...
즉, 증강 코딩은 도구 사용법 문제가 아니라 팀 운영 방식의 문제다.
