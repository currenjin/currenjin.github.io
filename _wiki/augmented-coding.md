...
updated : 2026-05-14 14:00:00 +0900
...
증강 코딩 도입 전후의 개인 git 활동을 같은 길이 기간(약 113일)으로 비교했다.
한 사람, 한 저장소(roouty-backend) 데이터라 일반화 근거는 아니다.
운영 설계 변경이 개인 작업 패턴에 어떻게 드러나는지 보여주는 사례로만 본다.

### 8-1) 비교 기간

- 이전: 2025-10-01 ~ 2026-01-20
- 이후: 2026-01-21 ~ 2026-05-14 (`augmented-coding` 스킬 도입일 기준)
...
|---|---:|---:|---:|
| 본인 커밋 수 | 171 | 170 | ≈ |
| 머지된 PR (squash 기준) | 145 | 151 | +4% |
| 활동일수 | 43일 | 52일 | +21% |
| 총 변경 라인 | 20,594 | 115,272 | 5.6x |
| 파일 변경 횟수 | 846 | 1,900 | 2.2x |
| 라인/일 평균 | 479 | 2,217 | 4.6x |
| 라인/커밋 평균 | 120 | 678 | 5.6x |

### 8-3) 패턴 변화

- PR 수는 거의 동일하지만 PR 당 변경 규모가 5.6배. 더 큰 단위 작업을 한 번에 출하한다.
- 20시 이후 야간 커밋 비중이 26%에서 6%로 줄었다. 업무시간 안에서 처리량을 끌어올렸다.
- 커밋 타입 분포가 달라졌다.
  - 이전: `fix` 41, `feat` 8. `fix`가 `feat`의 5배. 사후 대응 위주였다.
  - 이후: `fix` 17, `feat` 3, `chore` 6, `test` 3, `refactor` 2.
  - `fix` 절대 수는 절반 이하로 줄었고, `test`와 `refactor`가 별도 타입으로 등장했다. 4-2)에서 말한 "구현 루프 안에 정리 루프를 의도적으로 넣는다" 원칙이 커밋 로그에 그대로 남았다.

### 8-4) 해석 시 유의점

- 라인 수는 노력의 대용지표다. 코드 품질을 직접 말해주지 않는다. PR 수가 같은데 라인이 5배라는 건 작업 단위 규모가 커졌다는 신호 정도로만 본다.
- 도입 후 기간에는 `auto`, `handoff`, `work` 같은 다른 스킬도 같이 정착했다. 이 변화는 `augmented-coding` 단독 효과가 아니라 AI 보조 워크플로 전체의 효과다.
- 비교 기간 외부 변수(팀 구성, 도메인 난이도, 처리한 티켓 성격)는 보정하지 않았다.

### 8-5) 정리

실행량(라인/일, 변경 파일 수)은 자동화로 올라갔다.
야간 커밋 감소, 테스트와 리팩토링 커밋의 등장은 사람 쪽 운영에서 나온 변화다.

생성 능력이 아니라 판정 체계와 운영 설계가 결과를 만든다.
1인 사례지만 4-8), 4-9)에서 적어둔 가설과 같은 방향이다.
