...
updated : 2026-03-13 22:20:00 +0900
...
그래서 이 방식은 “잘 나오길 기대”하는 게 아니라, “기준을 만족할 때까지 검증”하는 방식에 가깝다.
...
Kent Beck 관점에서 보면, 이 지점은 기능(features)과 선택지(options) 균형을 잃는 순간과 같다.

### 4-3) TDD는 설계를 방해하는 게 아니라 설계를 드러내는 방식이다

작은 테스트와 일반화 단계를 반복하면 설계 결정이 더 명확해진다.
핵심은 테스트 개수가 아니라 테스트를 판정 기준으로 쓰는 데 있다.
즉 TDD는 코드 작성 순서가 아니라, 설계 판단을 외부로 드러내는 절차로 보는 편이 맞다.
