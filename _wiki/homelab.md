...
```
┌─────────────────────────────────────────────────────────────────┐
│  🖥  맥북 (Apple M2 / Darwin 25)                                  │
│                                                                  │
│  층1: macOS                                                      │
│  ──────────────────────────────────────────────────────────     │
│  • 다니엘이 직접 만지는 곳                                          │
│  • brew, kubectl, terraform, git 등 CLI 도구                     │
│  • Tailscale 데몬 작동                                           │
│  • ~/srv/data/  ← 영구 데이터 자리                                │
│                                                                  │
│   ┌──────────────────────────────────────────────────────────┐  │
...
│   │  │                                                     │  │  │
│   │  │   ┌──────────────────────────────────────────────┐ │  │  │
│   │  │   │  층3: k3d cluster (= 컨테이너들의 집합)         │ │  │  │
│   │  │   │  ───────────────────────────────────────────  │ │  │  │
│   │  │   │  ┌──────┐  ┌──────┐  ┌──────┐                │ │  │  │
│   │  │   │  │서버  │  │워커1 │  │워커2 │   ← 각각 도커 컨테이너│ │  │  │
│   │  │   │  │노드  │  │노드  │  │노드  │     안에서 k3s 실행 │ │  │  │
│   │  │   │  └──────┘  └──────┘  └──────┘                │ │  │  │
│   │  │   │     │         │         │                    │ │  │  │
│   │  │   │     └─── 이 안에 또 Pod들이 들어감 ───        │ │  │  │
│   │  │   │     (nginx, argocd, prometheus 등)            │ │  │  │
│   │  │   └──────────────────────────────────────────────┘ │  │  │
│   │  └────────────────────────────────────────────────────┘  │  │
│   └──────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
...
│                                                  │
│   ┌─────────────────┐                            │
│   │ Control Plane   │ (k3s 용어로 "server")      │
│   │ Node            │                            │
│   └─────────────────┘                            │
│                                                  │
│   ┌──────┐ ┌──────┐                              │
│   │Worker│ │Worker│ (k3s 용어로 "agent")          │
│   │ Node │ │ Node │                              │
│   └──────┘ └──────┘                              │
└─────────────────────────────────────────────────┘
```
...
│  ┌──────────────────────────────────────────┐    │
│  │ ④ controller-manager                       │    │
│  │ • 매니저 (계속 watch + reconcile)            │    │
│  │ • deployment/replicaset/node 등 여러         │    │
│  │   controller의 집합                          │    │
│  └──────────────────────────────────────────┘    │
└──────────────────────────────────────────────────┘
```
...
│  ┌────────────────┐  ┌────────────────────┐    │
│  │ ① kubelet       │  │ ② kube-proxy        │    │
│  │ • apiserver와    │  │ • Service ClusterIP │    │
│  │   long-watch     │  │   → 실제 Pod IP     │    │
│  │ • 컨테이너 런타임│  │   라우팅              │    │
│  │   에 명령        │  │ • iptables/IPVS 룰  │    │
│  └────────────────┘  └────────────────────┘    │
│                                                  │
│  ┌──────────────────────────────────────────┐  │
│  │ ③ Container Runtime (containerd)            │  │
│  │ • 실제 컨테이너 프로세스를 띄우고 돌리는 엔진  │  │
│  └──────────────────────────────────────────┘  │
│           ▲                                     │
│           │ 컨테이너 명령                         │
│           │                                     │
...
```
kubelet ──long-lived watch──▶ apiserver
        ◀──"네 노드에 Pod 할당됨"───
        ──"Pod 떴음, status 보고"─▶
```
...
```
┌─ 워커 1 ────────────┐  ┌─ 워커 2 ────────────┐
│ Pod-A (apps ns)      │  │ Pod-D (apps ns)      │
│ Pod-B (platform ns)  │  │ Pod-E (monitoring ns)│
│ Pod-C (monitoring ns)│  │ Pod-F (platform ns)  │
└──────────────────────┘  └──────────────────────┘
```
...
💿 OrbStack VM 노출 포트
   │
   ▼
🔵 k3d 로드밸런서 컨테이너 (k3d-homelab-serverlb)
   │
   ▼
📦 ingress-nginx Pod (워커 안, platform ns)
   │  (host header / path 보고 라우팅 결정)
   ▼
🎯 Service ClusterIP (가상 IP)
   │  (kube-proxy의 iptables 룰이 변환)
   ▼
🟢 실제 앱 Pod
```
...
시간 ────────────────────────────────────────────────────────────▶
  ...
  t=0                 t=1                  t=2                t=∞
   ...
   ▼                   ▼                    ▼                  ▼
 ┌─────────┐       ┌─────────┐         ┌─────────┐       ┌─────────┐
 │bootstrap│       │Terraform│         │ ArgoCD  │       │ ArgoCD  │
 │  .sh    │       │  apply  │         │ 첫 sync │       │ 평생 watch│
 └─────────┘       └─────────┘         └─────────┘       └─────────┘
 사람 1회           사람 가끔             자동 1회         자동 영원
```
...
☁ GitHub (homelab-gitops repo)
   ...
   ▼ (polling 또는 webhook)
   │
📦 ArgoCD Pod (워커, platform ns)
   │  매니페스트 비교: "git에 있는 desired vs 클러스터의 current"
   │
   ▼
   apiserver → etcd → controller-manager가 적용
   │
   ▼
🔧 클러스터 상태 변경 (Pod 새로 뜨거나 업그레이드됨)
```
...
```
📦 앱 Pod (/metrics 엔드포인트 노출)
   ▲
   │ Prometheus가 주기적으로 pull (스크래핑)
   │
📦 Prometheus Pod (monitoring ns)
   │
   ▼
📊 Grafana Pod
   │
...
```
📦 앱 Pod (stdout/stderr로 로그)
   │
   ▼ kubelet이 stdout을 노드 디스크에 떨굼
   │
📦 수집 에이전트 (DaemonSet: Promtail/Vector/Fluent-bit)
   │  각 워커 노드에 떠있음
   ▼
📦 Loki Pod (monitoring ns)
   │
   ▼
📊 Grafana Pod
   │
...
```
              ...
              │                     │
              ▼                     ▼
[다니엘] ──▶ apiserver ──▶ etcd ──▶ workers
                ▲
                │ 🔵-meta (Prometheus가 "타겟 목록" 가져갈 때만)
                │
[Prometheus] ───┘
                │
                │ 🔵-data (실제 메트릭은 Pod 직접 접근)
                ▼
              [Pods]
                ▲
                │ 🔴 사용자
                │
[브라우저] ──▶ ingress-nginx ──▶ Service ──▶ Pods
```
