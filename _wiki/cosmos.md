...
updated : 2026-07-21 12:53:00 +0900
...
COSMOS는 인프라의 이름인 동시에 생각과 프로젝트, 서비스가 존재하는 전체 세계를 뜻한다. Mac mini 위의 개인 인프라는 이 세계가 현실에 구현된 기반이다.

| 이름 | 정의 |
|---|---|
| 작은 빛 | COSMOS를 돌아다니며 발견하고 관찰하고 기록하는 존재 |
| SUBSPACE | 현실의 사용자와 우주를 떠도는 작은 빛 사이에 있는 보이지 않는 초월적 통신 차원 |
| Cosmic Filament | 생각들이 색인과 접점을 통해 연결되어 드러나는 비공개 사유의 구조. 여러 생각이 공유하는 색인을 `접점`이라 부른다. |

### 프로젝트와 천체

COSMOS에 배포되어 운영되는 하나의 앱 프로젝트는 **항성계**다. 항성계를 구성하는 앱과 서비스는 모두 **천체**로 부른다.

| 분류 | 역할 |
|---|---|
| 별(항성) | 프로젝트의 핵심 앱 |
| 행성 | 항성계 안에서 독립된 역할을 맡는 나머지 서비스 |
| 위성 | 특정 행성 서비스를 보조하는 서비스 |

Kubernetes의 Pod, Ingress, PVC처럼 구현을 위한 내부 자원까지 천체로 부르지는 않는다. 세계관의 분류는 사용자에게 의미 있는 제품과 서비스의 경계에 적용하며, 내부 인프라는 본래의 기술 용어를 유지한다.

프로젝트는 구상과 운영 상태에 따라 다음과 같이 이어진다.

```text
성운                  항성계                 항성 잔해
아이디어·실험·원형  →  배포·운영 중인 프로젝트  →  종료됐지만 기록이 남은 프로젝트
```

### COSMOS Observatory

**COSMOS Observatory**, 평소 이름으로 **관측소**는 COSMOS의 모든 항성계와 천체의 상태, 변화, 흔적을 관측하고 기록하는 시설이다. 관찰 대상인 천체나 하나의 앱 프로젝트가 아니므로 항성계로 분류하지 않는다.

| 구성 요소 | 관측소에서의 역할 |
|---|---|
| Grafana | 관측창 |
| Prometheus | 상태와 수치의 관측 기록 |
| Loki | 사건과 흔적의 기록 |
| Alloy | 각 노드에 배치된 관측 장비 |
