...
updated : 2026-07-21 15:30:22 +0900
...
COSMOS는 인프라 이름이면서 생각과 프로젝트, 서비스가 함께 놓이는 개인 우주다. Mac mini 인프라는 그 우주를 실제로 운영하는 기반이다.

| 이름 | 정의 |
|---|---|
| 작은 빛 | COSMOS를 돌아다니며 새로운 것을 발견하고 기록하는 관찰자 |
| SUBSPACE | 다니엘과 우주를 떠도는 작은 빛이 대화를 주고받는 보이지 않는 통신 차원 |
| Cosmic Filament | 개인적인 생각을 색인으로 잇고 그 관계를 보여주는 비공개 생각 지도. 여러 생각이 같은 색인을 공유하면 그곳을 `접점`이라 부른다. |

### 프로젝트와 천체

COSMOS에 올라와 운영을 시작한 앱 프로젝트 하나를 **항성계**라고 한다. 그 안의 앱과 서비스는 모두 **천체**다.

| 분류 | 역할 |
|---|---|
| 별(항성) | 프로젝트의 핵심 앱 |
| 행성 | 핵심 앱과 함께 운영되는 나머지 서비스 |
| 위성 | 특정 행성 서비스를 보조하는 서비스 |

이 이름은 프로젝트 안에서 독립된 역할이 있는 앱과 서비스에 붙인다. Kubernetes의 Pod, Ingress, PVC 같은 내부 자원은 기술 용어 그대로 부른다.

배포 전의 아이디어는 성운이고, 운영을 마친 프로젝트는 항성 잔해로 남는다.

```text
성운                         항성계                    항성 잔해
아이디어·실험·프로토타입  →  배포해 운영하는 프로젝트  →  운영은 끝났지만 기록이 남은 프로젝트
```

### COSMOS Observatory

모니터링 스택의 이름은 **COSMOS Observatory**다. 보통은 **관측소**라고 부른다. COSMOS 안의 항성계와 천체의 상태와 변화를 관측하고, 그 흔적을 기록한다.

관측소는 관찰 대상이 아니라 운영 시설이다. 천체나 항성계로 분류하지 않는다.

| 구성 요소 | 관측소에서의 역할 |
|---|---|
| Grafana | 관측창 |
| Prometheus | 상태와 수치를 쌓는 기록계 |
...
![가상 데이터로 구성한 Cosmic Filament 예시 화면](https://raw.githubusercontent.com/currenjin/cosmic-filament/main/docs/images/cosmic-filament-meetings.png)

*가상 생각 6개와 색인 6개를 펼친 예시다. 큰 빛은 색인을, 작은 빛은 생각을 나타내며 같은 색인을 공유하는 생각만 선으로 이어진다.*
...
| 관측소(관측성) | Prometheus + Grafana + Loki + Alloy | `COSMOS Observatory`: 메트릭, 시각화, 로그 수집·조회 |
