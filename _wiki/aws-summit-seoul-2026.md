...
피지컬 AI 세션은 아직 정리 전이라 빼고, 나머지 세션들을 가로질러 본 인상을 네 가지로 묶었다.

### AgentCore가 올해 AWS의 새 추상화 계층

삼성과 람다, 두 세션이 같은 메시지를 따로 던졌다. 모델 호출 자체는 이제 별로 어려운 일이 아니고, 그 위에 Runtime / Identity / Gateway / Memory / Evaluations를 얹은 운영 계층이 본격적으로 보였다. Bedrock에서 AgentCore로 무게중심이 옮겨가는 해라는 인상이 강했다.

### AI Ops의 가치는 자동 판단이 아니라 탐색 시간 단축

요기요가 이걸 대놓고 말했고, 삼성도 같은 방향에 서 있다. 삼성 PoC에서 사람 정확도가 90%, 에이전트가 50%였으니 자율 실행은 아직 무리다. 다만 분석 시간이 20분에서 5분으로, 그것도 24시간 가능해진 부분에서 가치가 나왔다. 지금 단계의 에이전트가 하는 일은 판단이 아니라 사람의 시야를 좁혀 주는 일이다.

### 컨텍스트가 전부다

Kiro가 직접 말했다. "컨텍스트가 메시지 배열의 전부다." 삼성 AgentCore Memory의 단기/장기 분리도, 라포랩스의 Steering도 같은 얘기를 다른 그릇에 담고 있다. 핵심은 더 많이 넣는 게 아니라 다음 단계에 필요한 만큼만 정확히 채우는 거다. 삼성 발표에서 인용된 Context Rot도 같은 맥락이었다.

### 운영자는 콘솔에서 탈출하고 있다

요기요 세션 제목부터가 "SRE 운영의 콘솔 탈출기"였다. 삼성의 끝없는 대시보드 탐색 부담, 라포랩스의 배포 자동화도 같은 갈래다. 운영의 사용자 경험이 콘솔과 대시보드 클릭에서 자연어 질의와 자동 리포트로 옮겨가는 중이라는 게 이번 이틀의 가장 큰 변화였다.
