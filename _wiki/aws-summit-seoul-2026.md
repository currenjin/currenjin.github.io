...
피지컬 AI 세션은 아직 정리 전이라 빼고, 나머지 세션들을 가로질러 본 관찰.
...
- **컨텍스트가 전부다.** Kiro가 직접 말했다 ("컨텍스트가 메시지 배열의 전부다"). 삼성 AgentCore Memory의 단기/장기 분리, 라포랩스 Steering이 같은 얘기를 다른 그릇에 담고 있다. 핵심은 더 많이 넣는 게 아니라 다음 단계에 필요한 만큼만 정확히 채우는 거다.
- **단기/장기 메모리가 분리되어 다뤄진다.** 삼성 AgentCore Memory의 단기(대화·세션·작업) 대 장기(선호·과거 상호작용·사실)가 가장 명확한 사례. Context Rot 인용도 같은 맥락이었다.
