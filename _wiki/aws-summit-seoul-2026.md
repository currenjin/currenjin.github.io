...
- **AgentCore가 올해 AWS의 새 추상화 계층이다.** 삼성과 람다 세션이 같은 메시지를 따로 던졌다. 모델 호출 위에 Runtime / Identity / Gateway / Memory / Evaluations를 얹은 운영 계층이 본격적으로 보였고, 무게중심이 Bedrock에서 AgentCore로 옮겨가는 해라는 인상이 강했다.
- **AI Ops의 가치는 자동 판단이 아니라 탐색 시간 단축이다.** 요기요가 대놓고 말했고 삼성도 같은 방향이다. 삼성 PoC에서 사람 정확도 90%, 에이전트 50%였으니 자율 실행은 아직 무리. 다만 분석 시간이 20분에서 5분으로, 24시간 가능해진 부분에서 가치가 나왔다.
- **자동화는 자율 실행이 아니라 "제안 + 승인" 사이클로 들어오고 있다.** 요기요가 명시했고, 삼성의 us-east-1 Failover도 에이전트 분석 뒤 사람이 강제 종료한 후 진행했다. 자율은 아직 신뢰의 영역이 아니다.
- **컨텍스트가 전부다.** "컨텍스트가 메시지 배열의 전부다"라고 Kiro 세션이 못 박았다. 삼성 AgentCore Memory의 단기/장기 분리, 라포랩스 Steering이 같은 얘기를 다른 그릇에 담고 있다. 핵심은 더 많이 넣는 게 아니라 다음 단계에 필요한 만큼만 정확히 채우는 거다.
- **단기/장기 메모리를 둘로 나눠 다룬다.** 삼성 AgentCore Memory의 단기(대화·세션·작업) 대 장기(선호·과거 상호작용·사실)가 가장 명확한 사례. Context Rot 인용도 같은 결이었다.
- **운영자는 콘솔에서 탈출하고 있다.** 요기요 세션 제목부터가 "SRE 운영의 콘솔 탈출기". 삼성의 끝없는 대시보드 탐색 부담, 라포랩스의 배포 자동화도 같은 갈래다. UX가 콘솔/대시보드 클릭에서 자연어 질의와 자동 리포트로 옮겨가고 있다.
- **알림은 짧은 경보 한 줄이 아니라 맥락이 묶인 리포트로 간다.** 넥슨의 신문 형식(THE ANOMALY TIMES), 라포랩스 Raku의 Daily Reporting이 같은 결. 사람의 대응 속도를 결정하는 건 정보 밀도다.
- **단일 벤더 의존을 피하는 Failover/Hybrid 설계가 공통 답안이었다.** 당근의 KMS-CloudHSM, 삼성의 us-east-1 → AP Region Failover가 같은 골격. Agentic 시스템도 결국 인프라라 SPOF 제거가 그대로 따라온다.
- **MCP가 에이전트의 표준 손발로 자리잡았다.** Kiro의 MCP 모범 사례, AgentCore의 MCP Tool Functions, Raku의 도구 연동이 같은 표준 위에 있다.
- **에이전트 평가가 별도 프로덕트로 분리됐다.** AgentCore Evaluations는 모델 평가가 아니라 에이전트 자체를 데이터로 평가한다. 품질·비용·지연 시간의 균형 조정이 운영 계층의 일이 됐다.
- **한국 기업 발표의 톤이 "도입 검토"에서 "프로덕션 운영"으로 옮겨갔다.** 보안(당근), 게임(넥슨), 배달(요기요), B2C(삼성), AI 플랫폼(라포랩스), 개발 도구(Kiro) 전부 실 운영 시스템 얘기였다.
- **운영의 새 변수는 Cold Start와 토큰 사용량이다.** 람다 위 AgentCore의 더블 Cold Start, 호출당 토큰 누적이 SLO와 비용에 그대로 들어온다.
...
24시간 가동되는 게임 운영에서 새벽 3시에 누가 깨어 이상을 감지하느냐, 여기에 넥슨이 내놓은 답.
사람 대신 18만 개의 모델이 시계열 신호를 동시에 살피고, 의미를 모아 사람한테는 "기사" 형태로 던지는 에이전틱 Ops 시스템이다.
...
동시 접속자 같은 핵심 지표에서는 절대 임계치보다 **시계열 자체의 급격한 변화**가 더 결정적이다. 서버/서비스 사이의 이질성도 같이 본다.
...
Amazon Bedrock AgentCore 위에서 21억 사용자 스케일 운영을 에이전트가 어떻게 받치는지 보여준 사례.
...
![AgentCore Observability 다이어그램](/resource/aws-summit-seoul-2026/samsung-01-observability.jpg)

또 다른 가시성, 재사용성, 거버넌스를 단순화하기 위한 도구로 **AWS Agent Registry**가 소개됐다.

![AWS Agent Registry 슬라이드](/resource/aws-summit-seoul-2026/samsung-02-agent-registry.jpg)
...
![Key Take-home Messages 슬라이드](/resource/aws-summit-seoul-2026/samsung-03-key-takeaways.jpg)
...
![AIOps Agent PoC Diagram](/resource/aws-summit-seoul-2026/samsung-04-aiops-poc.jpg)
...
![사람 분석에서 AI 기반 자동 분석으로 진화](/resource/aws-summit-seoul-2026/samsung-05-ai-evolution.jpg)
...
![Agentic AI Platform 아키텍처](/resource/aws-summit-seoul-2026/samsung-06-agentic-platform.jpg)
...
![Strands Agent on Lambda 아키텍처](/resource/aws-summit-seoul-2026/lambda-01-strands-on-lambda.jpg)
...
![Strands Agent + Lambda + AgentCore 결론](/resource/aws-summit-seoul-2026/lambda-02-conclusion.jpg)
...
![Raploy on AWS 아키텍처](/resource/aws-summit-seoul-2026/rapport-01-raploy-on-aws.jpg)
...
![Raku on AWS 아키텍처](/resource/aws-summit-seoul-2026/rapport-02-raku-on-aws.jpg)
