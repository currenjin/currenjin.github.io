...
updated : 2026-08-31 15:50:00 +0900
...
그래서 본문 서술을 두 등급으로 적었다. 인용블록으로 낸 문장과 그 아래 출처가 붙은 것은 원문이나 목차로 확인한 것이고, 산문 안에서 「확인하지 못했다」·「~로 보인다」로 여는 것은 확인하지 못한 채 남겨 둔 추정이다.
...
이벤트 기반 아키텍처가 세 번 나오는 이유가 이 표에 드러난다. 메시징 방식이 퀀텀 수를 정하는 것이 아니라 정적 결합점이 정한다.

```mermaid
flowchart LR
  subgraph T1["중재자 이벤트 기반 · 퀀텀 1"]
    MED["요청 오케스트레이터"] --> S1["서비스들"] --> D1[("공유 데이터베이스")]
  end
  subgraph T2["브로커 이벤트 기반 · 퀀텀 1"]
    S2["서비스들"] --> D2[("공유 데이터베이스")]
  end
  subgraph T3["저장소를 가른 이벤트 기반 · 퀀텀 2"]
    S3["서비스 집합"] --> D3[("데이터베이스")]
    S4["서비스 집합"] --> D4[("데이터베이스")]
    S3 -.->|"비동기 통신만"| S4
  end
  ...
  style T1 color:#111827
  style T2 color:#111827
  style T3 color:#111827
```
