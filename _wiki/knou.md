...
summary : 한국방송통신대학교(KNOU) 컴퓨터과학과 5과목 강의별 정리·평가 범위·암기표
...
학교·학과 안내부터 5과목 강의별 정리, 평가 범위, 암기표까지 한 곳에 모았다.
...
전체 시험범위 번호 리스트를 먼저 외운다.
이어서 핵심 비교·빈칸 위주의 표로 회독하고, 짧은 시간엔 압축본 위주로 반복한다.
헷갈리는 강의는 핵심 정리를 다시 읽고, 연습문제는 객관식 형태로 직접 풀어본다.

문서는 크게 세 부분으로 나뉜다.

- 과목별 강의 정리: 5과목 × 강의별 핵심 개념과 연습문제
- 핵심 암기표: 비교·빈칸 위주 압축 표
- 전체 시험범위 암기 리스트와 지하철 압축본
...
> 정리 기준: U-KNOU 강의 페이지의 학습개요·학습목표·연습문제·정리하기·참고자료를 시험 준비용으로 재구성.
> 우선순위: 통계자료 검색·엑셀 함수/그래프·R/Python 분석 흐름을 절차 중심으로 암기

#### 빠른 복습 순서

1. 각 강의의 `시험 포인트`만 먼저 훑기
2. `연습문제`를 실제 객관식 형태로 다시 풀기
3. 헷갈리는 강의만 `핵심 정리`를 다시 읽기
4. `참고자료/링크`는 공식 문서 확인이 필요한 항목만 열기

#### 평가/시험 범위

> U-KNOU `과제/시험` 탭 기준. 변경될 수 있으므로 최종 응시 전 학교 공지사항을 다시 확인.
...
|---|---|---|
| HTML5웹프로그래밍 | 교재 1~3장, 강의 1~9강 | 교재 1~7장, 강의 1~15강 |
| Java프로그래밍 | 교재 1장~7장 및 해당 강의 | 교재 전체 및 해당 멀티미디어 강의 |
| 파이썬프로그래밍기초 | 멀티미디어 강의 1강~6강 | 멀티미디어 강의 전체 |
| 데이터정보처리입문 | 온라인과제물, 학교 공지 참고 | 교재 및 강의 전 범위 |
| 유비쿼터스컴퓨팅개론 | 중간과제물, 학교 공지 참고 | 교재 전체 및 해당 멀티미디어 강의 |
...
| 구분 | 키워드 | 외울 문장 | 헷갈림 방지 |
|---|---|---|---|
| 웹 기본 | HTML/CSS/JS | HTML은 구조, CSS는 표현, JavaScript는 동작이다. | 역할 구분 |
| 문서 기본 | DOCTYPE | `<!DOCTYPE html>`은 HTML5 문서 선언이다. | 태그 아님 |
| 문자 인코딩 | charset | `<meta charset="UTF-8">`은 문자 깨짐을 막는다. | head 안 |
| 반응형 | viewport | viewport는 모바일 화면 크기 대응 설정이다. | CSS 아님 |
| 제목 | h1~h6 | h1~h6은 제목 구조를 나타낸다. | 글자 크기용 X |
| 줄/구분 | br/hr | `br`은 줄바꿈, `hr`은 주제 전환이다. | br은 빈 요소 |
| 컨테이너 | div/span | div는 블록, span은 인라인 컨테이너다. | 의미 없음 |
| 목록 | ul/ol/dl | ul은 순서 없음, ol은 순서 있음, dl은 설명 목록이다. | li는 ul/ol 자식 |
| 링크 | a | `a href`는 하이퍼링크를 만든다. | href가 목적지 |
| 이미지 | img | `img src alt`는 이미지와 대체 텍스트다. | alt 중요 |
| 표 | table | table은 표, tr은 행, th는 제목 셀, td는 데이터 셀이다. | 행 먼저 |
| 미디어 | audio/video | audio와 video는 외부 플러그인 없이 미디어를 재생한다. | controls 기억 |
| 시맨틱 | header/nav/main | 시맨틱 태그는 영역의 의미를 드러낸다. | div와 구분 |
| 폼 | form/input | form은 입력 묶음, input은 입력 요소다. | type별 기능 |
| 접근성 | label | label은 입력 요소 설명과 접근성을 높인다. | for-id 연결 |
| 필수입력 | required | required는 입력 필수 검사를 지정한다. | JS 없이 가능 |
| 선택자 | 우선순위 | inline > id > class/속성/가상클래스 > 요소. | 높은 것 우선 |
| 박스모델 | content/padding/border/margin | 박스 모델은 content → padding → border → margin 순서다. | 안에서 밖으로 |
| display | block/inline | block은 한 줄 차지, inline은 내용만큼 차지한다. | inline 크기 제한 |
| position | static/relative/absolute/fixed/sticky | position은 요소 배치 방식을 정한다. | absolute 기준 부모 |
| 변형 | transform | transform은 요소 모양/위치를 변형한다. | 변화 과정 아님 |
| 전환 | transition | transition은 값 변화 과정을 부드럽게 한다. | hover와 자주 사용 |
| 애니메이션 | animation | animation은 keyframes 기반 반복 동작이다. | transition과 구분 |
| DOM | document | DOM은 HTML 문서를 객체로 다루는 모델이다. | 요소 조작 |
| BOM | window | BOM은 브라우저 창/주소/기록 등을 다루는 모델이다. | 브라우저 조작 |
| 이벤트 | addEventListener | addEventListener는 이벤트 처리 함수를 등록한다. | onclick 대체 가능 |
| 저장 | local/sessionStorage | localStorage는 영구, sessionStorage는 세션 동안 저장이다. | 객체는 JSON 변환 |
| Canvas | getContext | canvas 2D는 `getContext('2d')`로 얻는다. | 그림판 |
| API | Geolocation | Geolocation은 위치 정보를 얻는 API다. | 권한 필요 |

##### 2. 달달 암기용 압축문장

- 웹 = HTML 구조 + CSS 표현 + JS 동작.
- HTML 기본 = doctype, html, head, body.
- head = meta/title/link/script, body = 화면 내용.
- div = 블록 묶음, span = 인라인 묶음.
- table = tr 안에 th/td.
- form = action + method + input.
- CSS 우선순위 = inline, id, class, element.
- 박스모델 = content, padding, border, margin.
- DOM = 문서 객체, BOM = 브라우저 객체.
- Canvas 선 = beginPath, moveTo, lineTo, stroke.
- Storage 객체 저장 = JSON.stringify, 복원 = JSON.parse.

##### 3. 비교/구분 암기

| A | B | 차이 |
|---|---|---|
| HTML | CSS | HTML은 구조, CSS는 표현 |
| CSS | JavaScript | CSS는 스타일, JS는 동작 |
| div | span | div는 블록, span은 인라인 |
| ul | ol | ul은 순서 없음, ol은 순서 있음 |
| th | td | th는 제목 셀, td는 데이터 셀 |
| radio | checkbox | radio는 하나 선택, checkbox는 복수 선택 |
| id 선택자 | class 선택자 | id는 `#`, class는 `.` |
| padding | margin | padding은 안쪽 여백, margin은 바깥 여백 |
| relative | absolute | relative는 원래 위치 기준, absolute는 기준 부모 기준 |
| transition | animation | transition은 변화 과정, animation은 keyframes 동작 |
| DOM | BOM | DOM은 문서, BOM은 브라우저 |
| localStorage | sessionStorage | local은 영구, session은 탭/세션 동안 |
...
> 기존 요약은 너무 뼈대만 있음. Java는 아래 15강 흐름까지 암기해야 함.

##### 1. 핵심 암기표

| 구분 | 키워드 | 외울 문장 | 헷갈림 방지 |
|---|---|---|---|
| 1강 | Java | Java는 클래스 기반 객체지향 언어이며 JVM 위에서 바이트코드가 실행된다. | 플랫폼 독립 |
| 1강 | Java 플랫폼 | Java 플랫폼은 JVM + Java API다. | JDK는 개발도구 포함 |
| 1강 | JDK | Java 프로그래밍을 하려면 JDK가 필요하다. | JRE만으로 개발 X |
| 1강 | 클래스 | Java 프로그램의 기본 구성 단위는 클래스다. | 파일명/public class |
| 1강 | 객체지향 | 클래스는 데이터와 메서드로 객체를 추상화한 틀이다. | 데이터+행위 |
| 2강 | 변수 종류 | 변수는 인스턴스 변수, 클래스 변수, 지역 변수, 파라미터로 나뉜다. | 위치/소속 구분 |
| 2강 | 기본형 8개 | 기본형은 byte, short, int, long, float, double, char, boolean이다. | String은 참조형 |
| 2강 | 참조형 | 클래스와 배열은 참조형이고 참조값을 저장한다. | 객체 자체 저장 X |
| 2강 | 형변환 | 묵시적 형변환은 손실 없을 때 자동, 명시적 형변환은 캐스팅이다. | 큰→작은 강제 |
| 2강 | main 인자 | 명령행 매개변수는 main의 String[] args로 전달된다. | 실행 시 클래스명 뒤 |
| 3강 | 접근 제어 | 일반 최상위 클래스 접근 제어자는 public 또는 생략만 가능하다. | private 클래스 X |
| 3강 | default 접근 | 접근 제어자 생략은 같은 패키지에서 접근 가능하다. | package-private |
| 3강 | 생성자 | 생성자는 클래스 이름과 같고 반환형이 없다. | void 쓰면 메서드 |
| 3강 | 배열 | 배열은 생성 후 사용하며 `배열.length`로 크기를 얻는다. | length 메서드 아님 |
| 3강 | 문자열 + | 문자열에 `+`를 적용하면 문자열 연결이다. | 숫자+문자 주의 |
| 4강 | static 필드 | 모든 객체가 공유하는 데이터는 static 필드로 둔다. | 클래스 소속 |
| 4강 | 초기화 | 필드 선언문, 초기화 블록, 생성자로 초깃값을 지정한다. | 실행 순서 주의 |
| 4강 | 오버라이딩 | 부모 메서드 몸체를 자식 클래스에서 다시 정의하는 것이다. | 상속 필요 |
| 4강 | 오버로딩 | 같은 이름의 메서드를 매개변수 목록으로 구별한다. | 반환형만 다르면 X |
| 4강 | this/super | this는 현재 객체, super는 부모 객체 쪽을 가리킨다. | 생성자 호출도 가능 |
| 4강 | this()/super() | this()는 같은 클래스 생성자, super()는 부모 생성자를 호출한다. | 생성자 첫 줄 |
| 5강 | 인터페이스 | 인터페이스는 추상 메서드 중심이며 default/static 메서드는 몸체가 있다. | Java 8 이후 |
| 5강 | 추상클래스 | 의미적으로 유사한 클래스 묶음은 추상 클래스를 쓴다. | 일부 구현 가능 |
| 5강 | 인터페이스 용도 | 기능적으로 유사한 클래스 묶음은 인터페이스를 쓴다. | 다중 구현 가능 |
| 5강 | enum | 열거형 상수값은 하나의 객체처럼 취급된다. | 정해진 상수 집합 |
| 5강 | 다형성 | 다형성은 오버로딩/오버라이딩/상속/형변환/동적 바인딩으로 구현된다. | 부모 타입 참조 |
| 5강 | 익명 클래스 | 익명 클래스는 이름 없이 일회성 상속/구현 객체를 만든다. | 한 번 쓰는 클래스 |
| 6강 | 제네릭 타입 | 제네릭 타입은 자료형을 매개변수로 가지는 클래스/인터페이스다. | 컴파일 시 타입검사 |
| 6강 | 타입 파라미터 | 타입 파라미터는 필드형, 반환형, 인자형으로 쓰일 수 있다. | `<T>` |
| 6강 | 제네릭 메서드 | 제네릭 메서드는 자료형을 매개변수로 가지는 메서드다. | 메서드 자체 T |
| 6강 | 람다식 | 람다식은 함수형 인터페이스 객체 생성을 간결하게 한다. | 추상 메서드 1개 |
| 6강 | 타깃 타입 | 람다식의 결과 타입에 해당하는 인터페이스가 타깃 타입이다. | 문맥으로 결정 |
| 7강 | 패키지 | 패키지는 관련 클래스와 인터페이스의 계층적 묶음이다. | 디렉터리 구조 |
| 7강 | import | 다른 패키지 클래스를 편하게 쓰려면 import를 사용한다. | java.lang은 예외 |
| 7강 | CLASSPATH | 필요한 클래스/패키지는 CLASSPATH 경로에서 찾을 수 있어야 한다. | 실행 환경 |
| 7강 | 예외 | 예외는 실행 중 정상 흐름을 벗어난 비정상 상황이다. | Error와 구분 |
| 7강 | checked 예외 | checked Exception은 반드시 처리하거나 throws로 선언해야 한다. | 컴파일러 검사 |
| 7강 | try-catch | 직접 처리는 try-catch, 전파는 throws다. | throw는 발생 |
| 8강 | Object | toString()과 equals()는 Object에 정의되어 있고 재정의될 수 있다. | 모든 클래스 조상 |
| 8강 | java.lang | java.lang 패키지는 import 없이 사용할 수 있다. | 자동 import |
| 8강 | String | String은 immutable이라 생성 후 내용 변경이 불가능하다. | 반복 변경 비효율 |
| 8강 | StringBuffer/Builder | 자주 변경되는 문자열은 StringBuffer나 StringBuilder가 효율적이다. | Buffer 동기화 O |
| 8강 | Wrapper | 기본형 값을 객체로 다루기 위해 포장 클래스가 있다. | int→Integer |
| 8강 | System | System은 in, out, err 표준 입출력 객체를 제공한다. | 표준 입력/출력/오류 |
| 9강 | 스트림 | 스트림은 생산자/소비자 종류와 무관하게 입출력을 수행하게 한다. | 입출력 통로 |
| 9강 | 보조 스트림 | Buffered 계열은 보조 스트림으로 버퍼링 성능을 제공한다. | 기본 스트림 감쌈 |
| 9강 | InputStreamReader | InputStreamReader는 바이트 입력을 문자 입력으로 변환한다. | bridge stream |
| 9강 | File | File은 파일/디렉터리를 표현하지만 입출력 메서드는 제공하지 않는다. | 경로 조작 |
| 9강 | RandomAccessFile | RandomAccessFile은 파일 포인터 위치에서 임의 읽기/쓰기가 가능하다. | 순차만 아님 |
| 10강 | NIO | java.nio는 java.io를 개선한 새 입출력 패키지다. | New IO |
| 10강 | Path | Path는 File보다 풍부한 파일 경로 기능을 제공한다. | java.nio.file |
| 10강 | Files | Files는 파일/디렉터리 조작과 읽기·쓰기 static 메서드를 제공한다. | 유틸 클래스 |
| 10강 | Buffer | 채널 입출력에서 버퍼는 기본형 데이터 보관소다. | 버퍼 단위 IO |
| 10강 | FileChannel | FileChannel은 입출력 모두 제공하고 기본 버퍼링을 사용한다. | 멀티스레드 안전 설계 |
| 10강 | WatchService | WatchService는 디렉터리 이벤트를 감시한다. | 변경 감시 |
| 11강 | JCF | JCF는 컬렉션을 효율적으로 다루기 위한 API 프레임워크다. | Java Collections Framework |
| 11강 | Collection | 컬렉션은 여러 원소를 저장·관리하는 객체다. | Map은 별도 계열 |
| 11강 | Set | Set은 순서 의미가 없고 중복을 허용하지 않는다. | 중복 제거 |
| 11강 | List | List는 순서 의미가 있고 중복을 허용한다. | 인덱스 |
| 11강 | Queue | Queue는 순서 의미가 있고 FIFO 방식으로 자료를 관리한다. | offer/poll |
| 11강 | Map | Map은 key-value 쌍을 관리한다. | 키 중복 X |
| 11강 | LinkedList | LinkedList는 List와 Queue를 모두 구현하고 스택 메서드도 제공한다. | 다용도 |
| 12강 | 내부 반복 | 내부 반복은 반복 코드를 직접 쓰지 않고 처리 코드만 람다로 전달한다. | forEach |
| 12강 | forEach | forEach는 원소를 외부로 꺼내지 않고 내부에서 탐색한다. | 람다와 사용 |
| 12강 | 필터링 | 필터링은 중복 제거 또는 조건 만족 원소만 걸러내는 중간 연산이다. | distinct/filter |
| 12강 | 매핑 | 매핑은 원소를 다른 원소로 변환하는 중간 연산이다. | map |
| 12강 | 매칭 | 매칭은 조건 만족 여부를 확인하는 최종 연산이다. | any/all/noneMatch |
| 12강 | 수집 | 수집은 결과 원소를 List/Set/Map 등으로 취합하는 최종 연산이다. | collect |
| 13강 | 스레드 코드 | 스레드 실행 코드는 `public void run()`에 정의한다. | start가 run 호출 |
| 13강 | Thread/Runnable | 스레드는 Thread 상속 또는 Runnable 구현으로 만든다. | Runnable 선호 |
| 13강 | start | start()는 새 스레드를 시작하고 run()을 실행하게 한다. | run 직접 호출 X |
| 13강 | 제어 메서드 | start, join, interrupt, yield, sleep, wait, notify를 구분한다. | wait/notify는 Object |
| 13강 | 상태 | 스레드는 Startable, Runnable, Running, Not Running 등의 상태를 거친다. | 상태 전이 |
| 13강 | 동기화 | synchronized는 공유 객체 접근을 한 번에 한 스레드로 제한한다. | 데이터 무결성 |
| 14강 | JDBC | JDBC는 Java에서 관계형 DB와 연동하는 표준 API다. | java.sql |
| 14강 | MariaDB | MariaDB는 MySQL 호환 오픈소스 RDBMS다. | 드라이버 필요 |
| 14강 | 드라이버 | JDBC 사용에는 DB용 JDBC 드라이버 jar를 Classpath에 추가해야 한다. | .jar |
| 14강 | Connection | DBMS 연결은 java.sql.Connection 객체로 표현한다. | 연결 객체 |
| 14강 | Statement | SQL 실행은 execute(), executeQuery(), executeUpdate()를 사용한다. | 결과별 구분 |
| 14강 | executeQuery | SELECT 실행은 executeQuery()이고 결과는 ResultSet이다. | 조회 전용 |
| 14강 | executeUpdate | INSERT/UPDATE/DELETE는 executeUpdate()를 사용한다. | 변경 행 수 |
| 15강 | 라이브러리 | 라이브러리는 자주 쓰는 클래스/인터페이스를 컴파일된 형태로 제공한 것이다. | 재사용 코드 |
| 15강 | jar | Java 라이브러리는 .class 파일을 묶은 .jar 형태다. | 압축 파일 |
| 15강 | 모듈 | 모듈은 라이브러리의 업그레이드로 패키지 단위 접근성을 설정한다. | Java 9 |
| 15강 | Classpath/Modulepath | 라이브러리는 Classpath, 모듈은 Modulepath에 추가한다. | 경로 구분 |
| 15강 | module-info | module-info.java에서 exports와 requires로 의존관계를 설정한다. | 모듈 선언 |
| 15강 | java.base | java.base는 Java 표준 모듈 중 가장 기본 모듈이다. | 자동 의존 |

##### 2. 강의별 달달 암기용 압축문장

- 1강 = Java는 클래스 단위, JVM이 바이트코드 실행, 플랫폼은 JVM+API, 개발은 JDK.
- 2강 = 변수 4종, 기본형 8개, 참조형은 주소, 형변환은 묵시/명시, args는 명령행 인자.
- 3강 = 최상위 클래스 public/생략, default는 같은 패키지, 생성자는 이름 같고 반환형 없음, 배열 length.
- 4강 = static 공유, 초기화는 필드/블록/생성자, 오버로딩 매개변수, 오버라이딩 재정의, this/super.
- 5강 = 추상 클래스는 의미 묶음, 인터페이스는 기능 묶음, enum은 상수 객체, 다형성은 동적 바인딩.
- 6강 = 제네릭은 타입 매개변수, 컴파일 시 타입 검사, 람다는 함수형 인터페이스, 타깃 타입은 인터페이스.
- 7강 = 패키지는 묶음, import는 편의, CLASSPATH는 검색 경로, checked 예외는 처리/선언 필수.
- 8강 = Object의 toString/equals, java.lang 자동, String 불변, Buffer/Builder 변경 문자열, Wrapper.
- 9강 = 스트림은 입출력 통로, Buffered는 보조 스트림, InputStreamReader는 byte→char, File은 입출력 없음.
- 10강 = NIO는 개선 IO, Path/Files, Channel+Buffer, WatchService는 디렉터리 감시.
- 11강 = JCF, Set 중복X, List 순서O중복O, Queue FIFO, Map key-value, LinkedList는 List+Queue.
- 12강 = Stream 내부 반복, filter/distinct 중간, map 중간, match 최종, collect 최종.
- 13강 = Thread/Runnable, run에 실행 코드, start로 시작, synchronized로 공유 데이터 보호.
- 14강 = JDBC 표준 DB API, 드라이버 jar, Connection, Statement, executeQuery는 SELECT/ResultSet.
- 15강 = 라이브러리 jar, 모듈은 접근성 설정, Classpath와 Modulepath 구분, exports/requires.

##### 3. 비교/구분 암기

| A | B | 차이 |
|---|---|---|
| JDK | JRE | JDK는 개발도구 포함, JRE는 실행환경 중심 |
| JVM | Java API | JVM은 바이트코드 실행, API는 표준 라이브러리 |
| 기본형 | 참조형 | 기본형은 값 저장, 참조형은 참조값 저장 |
| 인스턴스 변수 | 클래스 변수 | 인스턴스 변수는 객체별, 클래스 변수는 static 공유 |
| 지역 변수 | 파라미터 | 지역 변수는 블록 내부, 파라미터는 메서드 인자 |
| 묵시적 형변환 | 명시적 형변환 | 묵시적은 자동, 명시적은 캐스팅 필요 |
| public 클래스 | default 클래스 | public은 어디서나, default는 같은 패키지 |
| 생성자 | 메서드 | 생성자는 반환형 없음, 메서드는 반환형 있음 |
| length | length() | 배열은 length 필드, 문자열은 length() 메서드 |
| 오버로딩 | 오버라이딩 | 오버로딩은 매개변수 다름, 오버라이딩은 상속 재정의 |
| this | super | this는 현재 객체, super는 부모 쪽 |
| this() | super() | this()는 같은 클래스 생성자, super()는 부모 생성자 |
| 추상 클래스 | 인터페이스 | 추상 클래스는 의미 묶음/일부 구현, 인터페이스는 기능 약속/다중 구현 |
| 추상 메서드 | default 메서드 | 추상 메서드는 몸체 없음, default는 몸체 있음 |
| enum | 상수 변수 | enum 상수는 객체처럼 취급, 상수 변수는 값 중심 |
| 제네릭 클래스 | 제네릭 메서드 | 클래스가 타입 매개변수 vs 메서드가 타입 매개변수 |
| 람다식 | 익명 클래스 | 람다는 함수형 인터페이스 간결 구현, 익명 클래스는 일회성 클래스 |
| throw | throws | throw는 예외 발생, throws는 예외 전파 선언 |
| checked | unchecked | checked는 처리/선언 필수, unchecked는 런타임 중심 |
| String | StringBuilder | String은 불변, StringBuilder는 변경 가능 |
| StringBuffer | StringBuilder | Buffer는 동기화 O, Builder는 동기화 X |
| InputStream | Reader | InputStream은 바이트, Reader는 문자 |
| OutputStream | Writer | OutputStream은 바이트 출력, Writer는 문자 출력 |
| 기본 스트림 | 보조 스트림 | 기본 스트림은 실제 대상 연결, 보조 스트림은 기능 추가 |
| File | Files | File은 경로/파일 표현, Files는 실제 조작 static 메서드 |
| File | Path | File은 java.io, Path는 java.nio.file의 경로 표현 |
| List | Set | List는 순서O 중복O, Set은 중복X |
| Queue | Stack | Queue는 FIFO, Stack은 LIFO |
| Collection | Map | Collection은 원소 묶음, Map은 key-value |
| filter | map | filter는 걸러냄, map은 변환 |
| 중간 연산 | 최종 연산 | 중간은 스트림 반환, 최종은 결과 생성/소비 |
| run() | start() | run 직접 호출은 일반 메서드, start는 새 스레드 시작 |
| sleep() | wait() | sleep은 Thread static, wait는 Object 메서드 |
| executeQuery | executeUpdate | query는 SELECT/ResultSet, update는 변경/행 수 |
| Classpath | Modulepath | 라이브러리 jar는 Classpath, 모듈은 Modulepath |
| exports | requires | exports는 내 패키지 공개, requires는 다른 모듈 요구 |
...
| 구분 | 키워드 | 외울 문장 | 헷갈림 방지 |
|---|---|---|---|
| 특징 | Python | Python은 인터프리터, 동적 타입, 들여쓰기 블록 언어다. | 세미콜론 중심 X |
| 변수 | 이름 | 변수는 값을 가리키는 이름이다. | 타입 선언 없음 |
| 자료형 | 기본 | 주요 자료형은 int, float, str, bool, list, tuple, dict, set이다. | 컬렉션 구분 |
| 입력 | input | input은 문자열을 입력받는다. | 숫자 변환 필요 |
| 출력 | print | print는 값을 화면에 출력한다. | sep/end 가능 |
| 대입/비교 | =/== | `=`는 대입, `==`는 비교다. | 시험 자주 나옴 |
| 문자열 | index | 문자열 인덱스는 0부터 시작한다. | 음수 인덱스 가능 |
| 슬라이싱 | start:end:step | 슬라이싱은 end를 포함하지 않는다. | 끝 미포함 |
| 조건 | if/elif/else | 조건문은 if, elif, else로 분기한다. | elif 오타 주의 |
| 반복 | for/while | for는 정해진 반복, while은 조건 반복에 주로 쓴다. | 무한루프 주의 |
| range | 범위 | range(n)은 0부터 n-1까지다. | 끝 미포함 |
| break | 종료 | break는 반복문을 종료한다. | 전체 반복 종료 |
| continue | 건너뜀 | continue는 이번 반복만 건너뛴다. | 다음 반복 진행 |
| 함수 | def | 함수는 `def name(params):`로 정의한다. | 들여쓰기 필수 |
| 반환 | return | return은 값을 반환하고 함수를 끝낸다. | print와 구분 |
| 지역변수 | local | 지역변수는 함수 안에서만 유효하다. | 밖에서 접근 X |
| 전역변수 | global | 전역변수 변경은 global 선언이 필요하다. | 읽기와 변경 구분 |
| 리스트 | list | list는 순서가 있고 변경 가능하다. | mutable |
| 튜플 | tuple | tuple은 순서가 있고 변경 불가능하다. | immutable |
| 딕셔너리 | dict | dict는 key-value 구조다. | key로 접근 |
| 세트 | set | set은 중복을 허용하지 않는다. | 순서 의미 약함 |
| 클래스 | class | 클래스는 객체의 설계도다. | 객체지향 |
| 생성자 | __init__ | `__init__`은 객체 생성 시 초기화 메서드다. | 언더바 2개 |
| self | 자기 자신 | self는 객체 자기 자신을 가리킨다. | 첫 매개변수 |
| 모듈 | import | import는 모듈을 가져온다. | 별칭 as 가능 |
| 파일 | with open | 파일은 `with open(...) as f`로 열면 자동으로 닫힌다. | close 생략 가능 |

##### 2. 달달 암기용 압축문장

- Python = 인터프리터 + 동적 타입 + 들여쓰기 + 객체지향 지원.
- input은 문자열, 숫자는 int/float 변환.
- 인덱스 0부터, 슬라이싱 끝 미포함.
- if/elif/else, for/while, break/continue.
- range(n) = 0부터 n-1.
- 함수 = def, 반환 = return.
- list 변경 O, tuple 변경 X, dict key-value, set 중복 X.
- class 설계도, object 인스턴스, __init__ 초기화, self 자기 자신.
- 파일 = open, 안전하게는 with open.
...
|---|---|---|
| `=` | `==` | `=`는 대입, `==`는 비교 |
| print | return | print는 출력, return은 반환 |
| break | continue | break는 반복 종료, continue는 이번 반복 건너뜀 |
| for | while | for는 반복 대상, while은 조건 |
| list | tuple | list는 변경 가능, tuple은 변경 불가 |
| dict | set | dict는 key-value, set은 중복 없는 집합 |
| 지역변수 | 전역변수 | 지역은 함수 내부, 전역은 함수 밖 |
| 함수 | 메서드 | 함수는 독립 호출, 메서드는 객체 소속 호출 |
| 모듈 | 패키지 | 모듈은 파일, 패키지는 모듈 묶음 |
| open | with open | open은 직접 close 필요, with open은 자동 close |
...
| 구분 | 키워드 | 외울 문장 | 헷갈림 방지 |
|---|---|---|---|
| 분석 절차 | 문제정의 | 데이터 분석은 문제 정의 → 수집 → 정리/분석 → 평가/해석 순서다. | 순서 암기 |
| 데이터 | 관찰값 | 데이터는 관찰·측정된 값이다. | 원자료 |
| 정보 | 처리결과 | 정보는 데이터를 목적에 맞게 처리한 결과다. | 의미 부여 |
| 범주형 | 분류 | 범주형 자료는 성별, 지역처럼 분류값이다. | 숫자 연산 X |
| 양적 | 수치 | 양적 자료는 키, 점수처럼 수치값이다. | 계산 가능 |
| 모집단 | 전체 | 모집단은 관심 대상 전체다. | population |
| 표본 | 일부 | 표본은 모집단에서 뽑은 일부다. | sample |
| 평균 | 중심 | 평균은 모든 값을 더해 개수로 나눈 값이다. | 이상치 약함 |
| 중앙값 | 가운데 | 중앙값은 정렬했을 때 가운데 값이다. | 이상치 강함 |
| 최빈값 | 빈도 | 최빈값은 가장 자주 나타나는 값이다. | 여러 개 가능 |
| 분산 | 흩어짐 | 분산은 평균에서 얼마나 흩어졌는지를 나타낸다. | 제곱 단위 |
| 표준편차 | 흩어짐 | 표준편차는 분산의 제곱근이다. | 원 단위 |
| SPSS | GUI | SPSS는 GUI 중심 통계 분석 도구다. | 쉬운 조작 |
| SAS | 대용량 | SAS는 대용량 자료 처리에 강하다. | 상용 |
| R | 통계 | R은 오픈소스 통계 분석 언어다. | r-project |
| Python | 범용 | Python은 범용 프로그래밍과 데이터 분석에 모두 쓴다. | 라이브러리 활용 |
| Anaconda | 배포판 | Anaconda는 Python/R 데이터 분석 배포판이다. | 패키지 포함 |
| 엑셀 함수 | = | 엑셀 함수 입력은 `=`로 시작한다. | 수식 표시 |
| 상대참조 | A1 | 상대참조는 복사 시 위치가 변한다. | 기본 참조 |
| 절대참조 | $A$1 | 절대참조는 복사해도 고정된다. | `$` 고정 |
| 혼합참조 | $A1/A$1 | 혼합참조는 열 또는 행만 고정한다. | `$` 위치 |
| 막대그래프 | 범주 비교 | 막대그래프는 범주별 크기 비교에 쓴다. | category |
| 꺾은선 | 시간 변화 | 꺾은선그래프는 시간에 따른 변화를 보여준다. | trend |
| 원그래프 | 구성비 | 원그래프는 전체 대비 구성 비율을 보여준다. | 비율 |
| 산점도 | 관계 | 산점도는 두 변수 관계를 보여준다. | correlation 감각 |
| pandas | read_csv | `pd.read_csv()`는 CSV 파일을 읽는다. | DataFrame |
| describe | 요약통계 | `describe()`는 요약 통계를 출력한다. | count/mean/std |

##### 2. 달달 암기용 압축문장

- 데이터 분석 = 문제정의, 수집, 정리/분석, 평가/해석.
- 데이터 = 관찰값, 정보 = 처리된 의미.
- 범주형 = 분류, 양적 = 수치.
- 모집단 = 전체, 표본 = 일부.
- 평균 = 이상치 약함, 중앙값 = 이상치 강함.
- SPSS = GUI, SAS = 대용량, R = 통계 오픈소스, Python = 범용+분석.
- 엑셀 함수는 `=`, 절대참조는 `$A$1`.
- 막대 = 범주 비교, 꺾은선 = 시간 변화, 원 = 구성비, 산점도 = 두 변수 관계.
- pandas CSV = read_csv, 엑셀 = read_excel, 요약 = describe.

##### 3. 비교/구분 암기

| A | B | 차이 |
|---|---|---|
| 데이터 | 정보 | 데이터는 원값, 정보는 처리 결과 |
| 범주형 | 양적 | 범주형은 분류, 양적은 수치 |
| 모집단 | 표본 | 모집단은 전체, 표본은 일부 |
| 평균 | 중앙값 | 평균은 이상치 영향 큼, 중앙값은 영향 작음 |
| 분산 | 표준편차 | 분산은 제곱 평균, 표준편차는 분산의 제곱근 |
| SPSS | SAS | SPSS는 GUI 쉬움, SAS는 대용량 강점 |
| R | Python | R은 통계 중심, Python은 범용+분석 |
| 상대참조 | 절대참조 | 상대는 복사 시 변함, 절대는 고정 |
| 막대그래프 | 원그래프 | 막대는 범주 비교, 원은 구성 비율 |
| 꺾은선그래프 | 산점도 | 꺾은선은 시간 변화, 산점도는 두 변수 관계 |
| read_csv | read_excel | read_csv는 CSV, read_excel은 엑셀 파일 |
...
| 구분 | 키워드 | 외울 문장 | 헷갈림 방지 |
|---|---|---|---|
| 기본 | 유비쿼터스 | 유비쿼터스는 언제 어디서나 자연스럽게 컴퓨팅을 이용하는 환경이다. | ubiquitous |
| 공간 | 물리+전자 | 유비쿼터스 공간은 물리공간과 전자공간의 융합이다. | 핵심 문장 |
| 상황인식 | context | 상황인식은 사용자, 위치, 시간, 환경 정보를 바탕으로 서비스한다. | 맥락 기반 |
| HCI | 인간-컴퓨터 | HCI는 인간과 컴퓨터의 상호작용이다. | computer |
| HRI | 인간-로봇 | HRI는 인간과 로봇의 상호작용이다. | robot |
| SoC | 단일 칩 | SoC는 여러 기능을 하나의 칩에 통합한 것이다. | System on Chip |
| MEMS | 초소형 | MEMS는 초소형 기계·전자 시스템이다. | 센서/구동 |
| RFID | 무선식별 | RFID는 무선 주파수로 대상을 식별한다. | 태그/리더 |
| USN | 센서망 | USN은 유비쿼터스 센서 네트워크다. | 센서+네트워크 |
| 나노기술 | 초미세 | 나노기술은 초미세 단위 물질 조작 기술이다. | 10^-9 |
| 차세대 전지 | 지속성 | 차세대 전지는 모바일/센서 기기의 지속성을 높인다. | 에너지 |
| 네트워크 | 연결 | 유비쿼터스 네트워크는 언제 어디서나 연결되는 네트워크다. | anytime |
| 클라우드 | 자원 제공 | 클라우드는 컴퓨팅 자원을 네트워크로 제공한다. | 소유보다 이용 |
| 클라우드 장점 | 확장성 | 클라우드 장점은 확장성, 비용 효율, 접근성이다. | 장점 |
| 클라우드 단점 | 의존성 | 클라우드 단점은 보안, 장애 의존성, 개인정보 문제다. | 단점 |
| 블록체인 | 분산원장 | 블록체인은 분산 원장으로 위변조가 어렵다. | 탈중앙 |
| 핀테크 | 금융+IT | 핀테크는 금융과 IT의 결합이다. | finance+tech |
| VR | 가상현실 | VR은 완전한 가상현실이다. | 현실 차단 |
| AR | 증강현실 | AR은 현실 위에 정보를 덧붙인다. | 현실 기반 |
| MR | 혼합현실 | MR은 현실과 가상을 상호작용하게 섞는다. | 혼합 |
| 메타버스 | 가상세계 | 메타버스는 지속적 가상 세계와 사회적 상호작용이다. | 사회성 |
| 스마트카 | 자율/통신 | 스마트 자동차는 센서, 통신, AI, 자율주행을 결합한다. | 자동차+ICT |
| 빅데이터 | 3V | 빅데이터 3V는 Volume, Velocity, Variety다. | 크기/속도/다양성 |
| AI | 지능처리 | AI는 학습, 추론, 인식 등 지능형 처리를 수행한다. | 머신러닝 포함 |
| 정보보호 | CIA | 정보보호 3요소는 기밀성, 무결성, 가용성이다. | Confidentiality/Integrity/Availability |
| 인증 | 본인 확인 | 인증은 사용자가 누구인지 확인하는 것이다. | Authentication |
| 접근통제 | 권한 제한 | 접근통제는 허가된 자원만 사용하게 제한한다. | Authorization |

##### 2. 달달 암기용 압축문장

- 유비쿼터스 = 언제 어디서나 자연스럽게 컴퓨팅.
- 핵심 공간 = 물리공간 + 전자공간 융합.
- 상황인식 = 사용자/위치/시간/환경 기반 서비스.
- HCI = 인간-컴퓨터, HRI = 인간-로봇.
- SoC = 하나의 칩, MEMS = 초소형 기계·전자 시스템.
- RFID = 무선 식별, USN = 센서 네트워크.
- 클라우드 = 네트워크로 컴퓨팅 자원 제공.
- 블록체인 = 분산 원장 + 위변조 어려움.
- VR = 가상, AR = 현실 위 정보, MR = 현실+가상 혼합.
- 빅데이터 3V = Volume, Velocity, Variety.
- 정보보호 CIA = 기밀성, 무결성, 가용성.

##### 3. 비교/구분 암기

| A | B | 차이 |
|---|---|---|
| 물리공간 | 전자공간 | 물리공간은 현실, 전자공간은 디지털 정보 공간 |
| HCI | HRI | HCI는 컴퓨터 상호작용, HRI는 로봇 상호작용 |
| SoC | MEMS | SoC는 기능 통합 칩, MEMS는 초소형 기계·전자 시스템 |
| RFID | USN | RFID는 식별, USN은 센서 네트워크 |
| 클라우드 | 로컬 | 클라우드는 네트워크 자원 이용, 로컬은 직접 보유 |
| 블록체인 | 중앙DB | 블록체인은 분산 원장, 중앙DB는 중앙 관리 |
| VR | AR | VR은 가상세계, AR은 현실 위 정보 증강 |
| AR | MR | AR은 정보 덧붙임, MR은 현실·가상 상호작용 |
| 인증 | 접근통제 | 인증은 신원 확인, 접근통제는 권한 제한 |
| 기밀성 | 무결성 | 기밀성은 비밀 유지, 무결성은 변조 방지 |
| 무결성 | 가용성 | 무결성은 정확성, 가용성은 필요할 때 사용 가능 |
...
1. 웹 페이지는 (     ) 구조 + (     ) 표현 + (     ) 동작으로 구성된다.
2. `ul`은 (     ) 목록, `ol`은 (     ) 목록이다.
3. `label`은 입력 요소의 (     )과 (     )을 높인다.
4. `position: fixed`는 (     )에 고정된다.
5. Canvas 2D context는 (     )로 얻는다.
...
1. Python 블록은 (     )로 구분한다.
2. 문자열 인덱스는 (     )부터 시작한다.
3. `range(a,b)`는 (     )부터 (     )까지다.
4. 함수 정의 키워드는 (     )이다.
5. 파일 자동 close 구문은 (     )이다.
...
2. 최빈값은 가장 (     ) 나타나는 값이다.
3. 표준편차는 (     )의 제곱근이다.
4. Anaconda는 Python/R 데이터 분석 (     )이다.
5. 산점도는 두 변수의 (     )를 본다.

##### 유비쿼터스

1. 유비쿼터스 핵심은 물리공간과 (     )의 융합이다.
2. MEMS는 초소형 (     )·(     ) 시스템이다.
3. 핀테크는 (     ) + (     )다.
4. 스마트 자동차는 센서, 통신, AI, (     )을 결합한다.
5. 접근통제는 허가된 (     )만 사용하게 제한한다.

---

#### 30초 루프

- HTML5 = 구조/표현/동작, 시맨틱, form/input, CSS 우선순위, 박스모델, DOM/BOM, Canvas, Storage.
- Java = JVM, 클래스/객체, 생성자, 오버로딩/오버라이딩, static/final, 인터페이스, 제네릭, 컬렉션, 예외, JDBC.
- Python = 들여쓰기, 자료형, 슬라이싱, 조건/반복, 함수, list/tuple/dict/set, class/self, import, with open.
- 데이터정보처리 = 분석절차, 데이터/정보, 범주형/양적, 모집단/표본, 평균/중앙값, 엑셀참조, 그래프, R/Python.
- 유비쿼터스 = 물리+전자, 상황인식, SoC/MEMS/RFID/USN, 클라우드, 블록체인, VR/AR/MR, 빅데이터 3V, 보안 CIA.

## 전체 시험범위 암기 리스트

### 전체 시험범위 암기표

> 용도: 전 과목 시험범위 전체를 달달 외우기 위한 리스트.
> 원칙: 강의별 표가 아니라 과목별로 쭉 외운다. 설명성 개요는 빼고 시험 포인트·핵심 정리·정답 감각 위주로 남긴다.
...
1. [시험 포인트] 웹 페이지의 구성 요소 → HTML + CSS + JavaScript
2. [시험 포인트] HTML 문서 → 요소들의 집합
3. [시험 포인트] 전역 속성 → id (요소의 정체성, 중복 불가), class (CSS 스타일 적용 시 사용), title (툴팁, 말풍선), data-* (숨겨진 데이터), role/aria-* (웹 접근성 관련)
4. [핵심 정리] 웹 → 인터넷 위에서 동작하는 하이퍼텍스트 서비스. “인터넷 ⊃ 웹”
5. [핵심 정리] 주요 용어 → 웹 사이트, 웹 문서, 웹 페이지, HTTP, 웹 서버, 웹 클라이언트 등
6. [핵심 정리] 작동 원리 → 서버(“응답”)-클라이언트(“요청”) 구조
7. [핵심 정리] 웹 페이지의 구성 요소 → HTML + CSS + JavaScript
8. [핵심 정리] 프로그래밍 범위 → 프론트엔드 프로그래밍 + 백엔드 프로그래밍 + 데이터 교환
9. [핵심 정리] 프로그래밍 과정 → 기획&설계 – 디자인 – 퍼블리싱 – 프론트엔드 개발 – 백엔드 개발
10. [핵심 정리] HTML 문서 → 요소들의 집합
11. [핵심 정리] 전역 속성 → id (요소의 정체성, 중복 불가), class (CSS 스타일 적용 시 사용), title (툴팁, 말풍선), data-* (숨겨진 데이터), role/aria-* (웹 접근성 관련)
12. [핵심 정리] 현업 기준의 HTML 문서의 최소 기본 구조는 아래 템플릿처럼 정리한다.
13. [시험 포인트] li 요소 → 리스트 항목을 표현 → 종료 태그 사용 권장
14. [시험 포인트] dl + dt/dd → '용어/정의-설명', '키-값' 구조가 중요한 데이터 표현
15. [시험 포인트] img → 이미지 삽입을 위한 인라인 요소
16. [핵심 정리] ul + li → 순서가 의미 없는 항목의 집합
17. [핵심 정리] ol + li → 순서 자체가 의미인 리스트
18. [핵심 정리] dl + dt/dd → '용어/정의-설명', '키-값' 구조가 중요한 데이터 표현
19. [핵심 정리] img → 이미지 삽입을 위한 인라인 요소
20. [핵심 정리] a → 링크 표현을 위한 인라인 요소 → 내부 이동, 외부 이동, 다운로드, 메일/전화 연동이 가능
21. [핵심 정리] 앵커 이동 → 긴 페이지에서 특정 위치로 이동하는 경우
22. [핵심 정리] 다양한 링크 지정 → 이메일, 전화/문자, 파일 재생 및 압축 파일 다운로드, 블록 링크
23. [핵심 정리] 이미지맵 → 좌표가 고정되어 반응형 웹에서 좌표 문제가 발생 → 사용 지양
24. [핵심 정리] iframe → 현재 페이지 안에 다른 웹 페이지를 삽입할 때 → 실무에서는 유튜브/지도 삽입, 결제창 등 외부 기능을 빌려올 때 사용 → title 속성 사용 중요/권장
25. [시험 포인트] audio 요소 → 오디오 재생
26. [시험 포인트] source 요소 → audio/video 요소의 단일 src 속성을 대신해서 대체 미디어 소스를 정의 → 브라우저 호환성 확보
27. [시험 포인트] video 요소 → 비디오 재생
28. [핵심 정리] audio 요소 → 오디오 재생
29. [핵심 정리] source 요소 → audio/video 요소의 단일 src 속성을 대신해서 대체 미디어 소스를 정의 → 브라우저 호환성 확보
30. [핵심 정리] video 요소 → 비디오 재생
31. [핵심 정리] 미디어 콘텐츠의 재생 구간 지정
32. [핵심 정리] track 요소 → 미디어 요소 안에서 사용되어 자막, 캡션 등과 같은 텍스트 정보를 제공
33. [핵심 정리] 시멘틱 요소 → "이 부분이 어떤 역할을 하는지" 명확한 의미를 포함/전달하는 요소
34. [핵심 정리] 문서 구조화를 위한 시멘틱 요소 → 의미 있는 구조의 문서 작성이 가능
35. [핵심 정리] figure 요소 → 독립적인 콘텐츠와 캡션의 밀접한 관계를 기계에서 명확히 알려주는 역할
36. [핵심 정리] details 요소 → 아코디언 메뉴(클릭 여부에 따라 세부 정보를 보여주거나 숨기는 형태의 UI)를 만들 수 있는 기능 제공
37. [시험 포인트] form과 input 요소
38. [시험 포인트] form 요소 → 여러 입력값을 '전송 가능한 데이터'라는 의미 있는 단위로 묶어 서버로 전달하기 위한 컨테이너
39. [시험 포인트] input 요소 → 다양한 입력 타입을 통해서 여러 유형의 값들을 입력하기 위한 요소
40. [핵심 정리] form과 input 요소
41. [핵심 정리] 웹 폼 → 프론트엔드와 백엔드의 접점
42. [핵심 정리] form 요소 → 여러 입력값을 '전송 가능한 데이터'라는 의미 있는 단위로 묶어 서버로 전달하기 위한 컨테이너
43. [핵심 정리] input 요소 → 다양한 입력 타입을 통해서 여러 유형의 값들을 입력하기 위한 요소
44. [핵심 정리] label 요소 → "웹 접근성의 핵심: 반드시 사용할 것"
45. [핵심 정리] fieldset 요소 → form 안에서 관련된 입력 요소들을 하나의 그룹으로 묶고 제목(캡션)을 붙일 때 사용 → 시각적으로 회색 테두리가 표시됨
46. [핵심 정리] 데이터 입력과 선택(1)
47. [핵심 정리] 특정 형식의 데이터 입력
48. [시험 포인트] 기본 구문 → '선택자 { 속성: 값; 속성: 값; … }'
49. [시험 포인트] 적용 방법 → 실무 표준인 외부 파일(.css), 페이지 단위의 내부 스타일, 특정 요소의 인라인 방식
50. [시험 포인트] 기본 선택자 → 전체(*), 요소(타입), 클래스(.), 아이디(#), 속성([ ]) 선택자
51. [시험 포인트] 클래스 중심의 설계 권장 → 의미 기반 네이밍 및 BEM 방법론 활용
52. [시험 포인트] 가상 클래스 → 요소의 상태 선택(:hover, :focus, :checked, :nth-child 등)
53. [시험 포인트] 가상 요소 → 요소의 특정 부분 선택 (::before, ::after, ::placeholder)
54. [핵심 정리] 개념 → HTML(구조)과 분리하여 디자인(표현,외양)을 담당하는 표준 스타일 언어
55. [핵심 정리] 기본 구문 → '선택자 { 속성: 값; 속성: 값; … }'
56. [핵심 정리] 적용 방법 → 실무 표준인 외부 파일(.css), 페이지 단위의 내부 스타일, 특정 요소의 인라인 방식
57. [핵심 정리] 명시도 → 스타일 충돌 시 점수제 적용
58. [핵심 정리] 기본 선택자 → 전체(*), 요소(타입), 클래스(.), 아이디(#), 속성([ ]) 선택자
59. [핵심 정리] 클래스 중심의 설계 권장 → 의미 기반 네이밍 및 BEM 방법론 활용
60. [핵심 정리] 아이디 → 유일성 강조. 스타일보다는 앵커 링크, 폼 라벨 연결, JS 제어용으로 권장
61. [핵심 정리] 가상 클래스 → 요소의 상태 선택(:hover, :focus, :checked, :nth-child 등)
62. [핵심 정리] 가상 요소 → 요소의 특정 부분 선택 (::before, ::after, ::placeholder)
63. [핵심 정리] 최신 선택자 → 코드 그룹화용(:is(), 명시도 0점인 :where()), 특정 자식을 가진 부모를 선택하는 :has()
64. [핵심 정리] 결합자 → 자손(공백), 자식(>), 인접 형제(+), 일반 형제(~), 그룹(,) 관계로 위치 기반 선택
65. [핵심 정리] 키워드 → 현재 글자색을 따르는 currentColor, 완전 투명한 검은색 transparent
66. [핵심 정리] CSS 변수 → :root에 '--변수명'으로 선언하고 var(--변수명)와 같이 호출하여 유지보수성 극대화
67. [시험 포인트] 속성 → font-family, font-size, font-style, font-variant, font-weight, font, @font-face
68. [시험 포인트] 속성 → line-height, letter-spacing, word-spacing, text-indent, text-align, text-align-last, vertical-align, text-decoration, text-transform, text-shadow, white-space, word-break, text-overflwo, overflow-wrap(word-wrap)
69. [시험 포인트] 정렬 및 배치 → text-align, vertical-align(인라인/표 요소 전용), 'text-wrap: balance'(제목 줄바꿈 최적화)
70. [시험 포인트] 속성 → list-style-type, list-style-position, list-style-image, list-style(단축 속성)
71. [시험 포인트] 커스텀 마커 → 가상 요소 ::marker로 간단한 스타일링을 하거나, ::before를 사용하여 정교한 아이콘 불릿 구현
72. [시험 포인트] 속성 → border-collapse, table-layout, border-spacing, caption-side, empty-cells
73. [핵심 정리] 폰트 설계 원칙 → 가독성(본문은 sans-serif 선호), 브랜드 정체성, 성능(경량화 및 시스템 폰트 혼용)의 균형이 중요함.
74. [핵심 정리] 속성 → font-family, font-size, font-style, font-variant, font-weight, font, @font-face
75. [핵심 정리] 속성 → line-height, letter-spacing, word-spacing, text-indent, text-align, text-align-last, vertical-align, text-decoration, text-transform, text-shadow, white-space, word-break, text-overflwo, overflow-wrap(word-wrap)
76. [핵심 정리] 간격 및 가독성:
77. [핵심 정리] 정렬 및 배치 → text-align, vertical-align(인라인/표 요소 전용), 'text-wrap: balance'(제목 줄바꿈 최적화)
78. [핵심 정리] 장식 및 변형 → text-decoration(밑줄/형광펜 효과), text-shadow(그림자), text-transform(대소문자 변환)
79. [핵심 정리] 줄바꿈 제어 → 'white-space: nowrap'과 'text-overflow: ellipsis'를 조합한 한 줄 말줄임 처리
80. [핵심 정리] 단어 끊기 → 한글은 'word-break: keep-al'l, 긴 URL 등은 'overflow-wrap: break-word'로 박스 터짐 방지
81. [핵심 정리] 속성 → list-style-type, list-style-position, list-style-image, list-style(단축 속성)
82. [핵심 정리] 스타일 초기화 → 메뉴 구현 등을 위해 'list-style: none'과 패딩/마진 제거를 기본적으로 수행
83. [핵심 정리] 커스텀 마커 → 가상 요소 ::marker로 간단한 스타일링을 하거나, ::before를 사용하여 정교한 아이콘 불릿 구현
84. [핵심 정리] 속성 → border-collapse, table-layout, border-spacing, caption-side, empty-cells
85. [핵심 정리] 시각화 기술 → nth-child(even)를 활용한 줄무늬 배경으로 데이터 가독성 증대
86. [핵심 정리] 반응형 대응 → overflow-x: auto를 이용한 가로 스크롤 생성 또는 미디어 쿼리를 통한 카드 형태 레이아웃 전환
87. [시험 포인트] 기타 제어 속성:
88. [핵심 정리] 여백(margin vs padding)
89. [핵심 정리] 위치 지정(position)
90. [핵심 정리] 레이아웃(display)
91. [핵심 정리] 기타 제어 속성:
92. [핵심 정리] 기본 테두리(border)
93. [핵심 정리] 모서리 깎기(border-radius)
94. [핵심 정리] 박스 그림자(box-shadow)
95. [핵심 정리] 선형 그라디언트 (linear-gradient)
96. [핵심 정리] 원형/타원형 그라디언트 (radial-gradient)
97. [핵심 정리] 원뿔 그라디언트 (conic-gradient)
98. [핵심 정리] 반복 그라디언트 (repeating-*)
99. [핵심 정리] 변형 기준점 (transform-origin)
100. [핵심 정리] 3D 원근감 (perspective)
101. [핵심 정리] 뒷면 제어 (backface-visibility)
102. [시험 포인트] 요소가 상태 A에서 B로 변할 때 즉시 바뀌지 않고 일정 시간에 걸쳐 부드럽게 변화하도록 돕는 상호작용 피드백 도구
103. [시험 포인트] 세부 제어 속성(animation-*)
104. [핵심 정리] 세부 제어 속성(animation-*)
105. [핵심 정리] 변수, 연산자, 제어문, 함수
106. [핵심 정리] 배열 순회와 주요 메서드
107. [시험 포인트] 브라우저는 DOM(구조)과 CSSOM(스타일)을 결합해 최종 화면인 '렌더 트리'를 구성
108. [시험 포인트] 사용자 정의 속성 (data-* 속성과 dataset 프로퍼티)
109. [핵심 정리] DOM ("문서 객체 모델")
110. [핵심 정리] 사용자 정의 속성 (data-* 속성과 dataset 프로퍼티)
111. [핵심 정리] 성능 최적화 (DocumentFragment)
112. [핵심 정리] BOM ("브라우저 객체 모델")
113. [핵심 정리] 개념 및 window 객체
114. [핵심 정리] 타이머 제어 (setTimeout & setInterval)
115. [시험 포인트] HTML5에 추가된 도화지(컨테이너) 요소로, JS를 이용한 고속 픽셀 드로잉에 최적화되어 있음
116. [시험 포인트] 캔버스의 개념 및 좌표계
117. [시험 포인트] 해상도와 CSS 크기 분리
118. [핵심 정리] 캔버스의 개념 및 좌표계
119. [핵심 정리] 해상도와 CSS 크기 분리
120. [핵심 정리] 고해상도(DPR) 보정 원리
121. [핵심 정리] 사각형 그리기 → (즉시 실행 방식) 코드를 호출하자마자 도화지에 즉시 그려짐
122. [핵심 정리] 상태 머신 (save/restore)
123. [핵심 정리] 패스 기반 드로잉 (다각형/선 그리기)
124. [핵심 정리] 원/원호 및 부채꼴 그리기
125. [시험 포인트] 선형 createLinearGradient( ), 원형 createRadialGradient( ), 원뿔형 createConicGradient( ) 객체를 생성하고 addColorStop( )으로 색상 중단점을 지정해 다채로운 빛과 그림자 효과를 구현
126. [핵심 정리] 이동 (translate)
127. [핵심 정리] 확대/축소/반전 (scale)
128. [시험 포인트] 상태 제어 (속성 및 메서드)
129. [핵심 정리] 상태 제어 (속성 및 메서드)
130. [핵심 정리] 상태 감지 및 UI 동기화 (이벤트)
131. [핵심 정리] 목적지 이벤트 및 제어 규칙 (가장 중요)
132. [핵심 정리] 데이터 추출 및 시각적 피드백
133. [시험 포인트] 기존 쿠키의 단점(매번 서버로 전송되어 트래픽 유발, 4KB의 적은 용량)을 극복하여, 서버 전송 없이 도메인당 약 5MB의 넉넉한 공간을 제공함
134. [시험 포인트] 스토리지의 2가지 종류와 생명주기
135. [시험 포인트] 핵심 개념 및 쿠키와의 차별성
136. [핵심 정리] 핵심 개념 및 쿠키와의 차별성
137. [핵심 정리] 스토리지의 2가지 종류와 생명주기
138. [핵심 정리] 데이터 직렬화 (JSON 처리 필수)
139. [핵심 정리] 핵심 개념 및 보안 정책
140. [핵심 정리] 지도 서비스 연동 및 그리기 (Leaflet.js 기반 4단계)

##### 정답 감각 리스트

1. 서버-클라이언트 구조에서 클라이언트의 역할로 가장 적절한 것은? → 4
2. 다음 중 meta 요소의 설명으로 틀린 것은? → 3
3. 다음 중 문장의 주제가 전환될 때 의미적 구분선의 역할로 사용되는 요소는? → 2
4. 제목 요소(h1~h6)의 사용 규칙으로 옳은 것은? → 3
5. 다음 중 물리적 요소와 이를 대체하는 논리적 요소의 짝이 잘못 연결된 것은? → 4
6. 위 지문과 같은 특징을 갖는 요소는? → 1
7. 다음 중 리스트의 불릿(기호)을 없애기 위해 실무에서 가장 많이 사용하는 CSS 속성은? → 2
8. 정의-설명 리스트에서 "설명/값"에 해당하는 요소는? → 2
9. 위 지문 중 img 요소의 srcset 속성이나 picture 요소를 언급하지 않았을 때, 반응형 환경에서 이미지를 유연하게 만드는 CSS 설정에서 (가)와 (나)에 알맞은 내용은? → 2
10. a 요소에서 target="_blank" 속성을 사용할 때 실무 보안 규칙으로 가장 적절한 것은? → 3
11. 동일한 페이지 내의 특정 위치(id="section1")로 이동하는 앵커 링크의 올바른 표현은? → 2
12. 표의 '제목 셀'을 생성하며, 기본적으로 굵게 표시되고 가운데 정렬되는 요소는? → 1
13. thead, tbody, tfoot 요소의 사용 목적에 대한 올바른 설명은? → 2
14. audio 요소에서 자동 재생을 시도할 때의 기본 전제로서 가장 적절한 코드 형태는?(단, src 속성 표현은 생략한다.) → 2
15. 미디어 표현에서 source 요소를 사용하는 가장 핵심적인 목적은? → 3
16. 시멘틱 요소를 사용하는 핵심 목적(실무 중심)으로 가장 적절한 것은? → 4
17. figure 요소의 핵심 역할로 가장 적절한 것은? → 1
18. time 요소에서 기계가 읽는 값을 나타내기 위한 속성은? → 2
19. progress 요소에서 value 속성을 사용하지 않았을 때 나타나는 현상은? → 3
20. 다음 중 form 요소의 속성 표현 method="get"을 적용하기에 가장 적합한 상황에 해당하는 것은? → 4
21. input 요소의 텍스트 입력 필드에서 사용자가 입력을 시작하면 사라지는 힌트 텍스트를 제공하는 속성은? → 3
22. inputmode 속성의 목적은? → 2
23. fieldset 요소의 역할로 올바른 것은? → 1
24. input 요소에서 radio 입력을 같은 그룹으로 만드는 핵심 조건은? → 2
25. datalist 요소와의 연동을 위해서 input 요소에서 반드시 지정해야 할 속성은? → 3
26. 인라인 스타일(inline style) 방식의 적용을 실무에서 가급적 지양해야 하는 이유로서 적절하지 못한 것은? → 3
27. 위 지문의 코드에서 화면에 출력될 "안녕하세요"의 글자 색상은? → 2
28. 다음 중 필수 입력 속성(required)이 있는 input 요소만 골라내어 스타일을 적용하기 위한 올바른 CSS 선택자 표현은? → 2
29. 다음 코드 중 ul 바로 밑에 있는 li들만 선택하기 위한 결합자의 표현으로 올바른 것은? → 1
30. 현재 글자 색상(color)을 그대로 가져와서 테두리 등에 사용하게 해주는 키워드는? → 4
31. 다음 중 rem 단위 설명으로 옳은 것은? → 1
32. 다음 중 한글의 문단 가독성이라는 관점에서 기본적인 설정 조합으로 가장 적절한 것은? → 2
33. 폰트 단축 속성(font)에 포함될 수 있는 속성은? → 3
34. 말줄임 표시를 위한 CSS 속성은? → 4
35. 내비게이션 메뉴(GNB) 등을 만들 때 리스트 초기화 코드로 가장 적절한 것은? → 2
36. 테이블에서 열 너비 계산을 단순화해 레이아웃을 안정화할 때 쓰는 속성은? → 1
37. 브라우저의 기본 box-sizing 설정("box-sizing: content-box;")에서 주어진 스타일이 적용되는 요소의 실제 렌더링 가로 너비는 얼마인가? → 4
38. 요소의 왼쪽 바깥쪽 여백을 30px 벌려서 다른 요소를 오른쪽으로 밀어내고 싶을 때 사용하는 속성은? → 1
39. 특정 블록 요소(.box)를 부모 컨테이너(.container)의 '가로 중앙'에 오도록 정렬할 때 margin 속성을 활용하려고 한다. 이에 알맞은 코드 표현은? → 2
40. grid에서 열을 3개로 나누는 가장 명확한 선언은? → 1
41. 알약 형태(pill shape)의 둥근 버튼을 만들기 위해, 버튼의 너비가 유동적으로 변해도 양쪽 끝이 항상 완벽한 반원 형태를 유지하도록 실무에서 주로 사용하는 border-radius 속성 값은? → 3
42. box-shadow 속성에서 그림자를 박스 '바깥쪽'이 아니라 박스 '안쪽'으로 생기게 해서, 버튼이 꾹 눌린 듯한 효과를 낼 때 쓰는 단어(키워드)는? → 2
43. 배경 이미지가 잘려 나가지 않고 온전히 다 보이도록, 요소의 크기에 맞춰 원본 비율을 유지한 채 최대 크기로 이미지를 맞추는 background-size 속성값은? → 2
44. 배경 이미지나 배경색이 테두리(border) 영역에는 칠해지지 않고, '패딩(padding) 영역까지만' 칠해지도록 제한 범위를 설정하는 CSS 속성은? → 3
45. linear-gradient(90deg, red, blue)에서 각도 지정 90deg가 의미하는 진행 방향은? → 2
46. 피자 조각을 나눈 것처럼 중심점을 기준으로 시계 방향으로 360도 돌아가며 색상이 변하는 CSS 그라디언트 함수로서, '컬러 휠'이나 '파이 차트'를 만들 때 쓰이는 것은? → 4
47. 위 지문에서 요소를 화면의 정중앙에 완벽하게 배치할 때, 'top: 50%; left: 50%;' 로 밀어낸 후 요소 자기 자신의 크기 절반만큼 다시 위/왼쪽으로 당겨오기 위해 사용하는 (A)에 알맞은 것은? → 1
48. CSS로 카드를 뒤집는 효과(Flip Card)를 만들었을 때, 카드가 180도 회전하여 뒷면을 보이고 있을 때 그 뒷면의 내용(거울처럼 뒤집힌 글자)이 보이지 않게 숨겨주는 속성은? → 4
49. 위 지문에서 요소의 상태가 변할 때, 특정 CSS 속성에만 부드러운 애니메이션 효과를 주고자 할 때 대상을 지정하기 위해 밑줄 친 부분에 들어갈 속성은? → 3
50. 전환 속도를 처음엔 느리게, 중간엔 빠르게, 끝날 땐 부드럽게 등 속도의 흐름(가감속)을 조절하는 속성은? → 1
51. 위 지문에서 애니메이션이 정방향(0→100)으로 실행된 후, 다시 툭 끊겨 처음으로 돌아가지 않고 역방향(100→0)으로 부드럽게 되돌아오는 '요요 효과'를 주는 속성값은? → 2
52. 위 지문에서 진행 중인 애니메이션을 마우스 오버 시 일시 정지하고 싶을 때 사용하는 속성은? → 2
53. 다단 레이아웃에서 단의 개수를 고정하지 않고, 각 단의 너비가 최소 300px은 되어야 한다라고 브라우저에 최적의 너비를 제시하는 속성은? → 1
54. 텍스트의 분량에 따라 단의 높이가 결정될 때, 브라우저가 알아서 모든 단의 높이를 최대한 비슷하게(균등하게) 맞추도록 내용을 재분배하는 속성은? → 4
55. 위 지문의 자바스크립트에서 값이 중간에 변할 수 있는 일반적인 변수를 선언할 때 사용하는 가장 모던하고 올바른 키워드는? → 2
56. 위 지문에서 논리 연산자의 단축 평가를 활용한 코드이다. 변수 result에 할당되는 값은? → 4
57. 위 지문에서 자바스크립트의 비교 연산자 '=='와 '==='를 적용하였을 때 (A)와 (B) 문장의 결과값으로 올바른 것은? → 1
58. 위 지문의 배열에 데이터를 추가하고 삭제하는 내장 메서드들의 동작을 바르게 설명한 것은? → 2
59. 위 지문의 배열에서 기존 원본을 훼손(수정)하지 않고, 각 요소를 콜백 함수로 가공하여 완전히 새로운 똑같은 길이의 배열을 만들어 반환하는 함수형 메서드는? → 3
60. 사용자가 폼 전송 버튼()을 눌렀을 때, 페이지가 새로고침되며 서버로 데이터가 넘어가는 브라우저의 '기본 동작'을 강제로 취소시키는 메서드는? → 3
61. 위 지문 중 배열 순회 시 최신 브라우저에서 forEach 메서드를 별도의 변환 없이 바로 사용할 수 있는 DOM 컬렉션은? → 4
62. 개별 요소 100개에 각각 이벤트를 걸지 않고, 부모인 요소 하나에만 이벤트를 걸어 동적으로 추가되는 자식들의 이벤트까지 모두 처리하는 기법을 일컫는 표현은? → 2
63. 위 지문에서 classList를 사용하여 요소에 특정 클래스가 있으면 제거하고, 없으면 추가하기 위해 사용하는 메서드는? → 3
64. 위 지문에서 DOM 요소의 인라인 스타일을 직접 수정할 때, 하이픈이 포함된 CSS 속성명(background-color)은 자바스크립트에서 어떻게 작성해야 하는가? → 4
65. 위 지문에서 쇼핑 장바구니처럼 안의 모든 상품(자식 요소)을 한 번에 싹 지워버리려 한다. 성능이 가장 뛰어나고 깔끔한 최신 메서드 호출 방식은? → 3
66. 위 지문에서 현재 웹 페이지의 주소(URL) 정보를 읽거나, 다른 페이지로 강제 이동시킬 때 사용하는 BOM의 핵심 객체와 속성은? → 3
67. ctx.save( ) 메서드를 호출했을 때 스택에 저장되는 대상이 아닌 것은? → 3
68. 패스(Path) 기반 방식으로 그림을 그릴 때 새로운 밑그림을 시작하겠다는 의미의 선언으로 이전 경로들과의 연결을 끊는 리셋 버튼 역할을 하는 필수 메서드는? → 3
69. 다각형의 내부 색상(노란색)과 테두리(파란색, 두께 5)를 모두 그릴 때, 테두리 두께가 색상에 덮이지 않고 온전하게 5px 굵기를 유지하기 위한 올바른 메서드 호출 순서는? → 2
70. 원이나 부채꼴을 그리는 ctx.arc( ) 함수에서 기본적으로 각도를 재는 방향과 기준점(0라디안)은 어디인가? → 3
71. 위 지문에서 굵은 선을 그렸을 때 선의 끝부분 모서리 형태를 지정하는 속성으로, 둥근 버튼 형태 UI를 그릴 때 주로 사용하는 스타일 값은 무엇인가? → 3
72. 위 지문 중 포토샵처럼 점선이 스르륵 흐르는 듯한 애니메이션을 구현할 때, 점선의 패턴 설계도를 왼쪽이나 오른쪽으로 미세하게 밀어주는 역할을 하는 속성은? → 2
73. 위 지문은 선형 그라디언트를 생성하는 코드의 일부이다. 그라디언트가 그려지는 방향은 어떻게 되는가? → 1
74. 위 지문의 캔버스에서 외부 이미지를 불러와 타일처럼 반복되는 패턴 객체를 생성하는 메서드는? → 1
75. 위 지문에서 그림자의 경계선을 칼같이 자르지 않고, 뿌옇게 퍼지도록 만드는 속성은 무엇인가? → 4
76. 위 지문에서 지정된 X 좌표를 기준으로 텍스트를 "가운데 정렬(수평 중앙)" 시키기 위해 사용하는 속성은? → 2
77. 위 지문의 캔버스에 외부에서 가져온 사진이나 이미지를 그려넣는 가장 기본적이고 널리 쓰이는 메서드는 무엇인가? → 4
78. ctx.scale(2,2)를 적용한 후 도형을 그렸다. 도형의 크기가 2배 커지는 것 외에 또 어떤 속성이 2배로 영향을 받아 시각적으로 달라지는가? → 2
79. 위 지문에서 모던 브라우저 정책상 오디오나 비디오를 웹페이지 접속 시 자동으로 재생(autoplay)되게 하려면, 사용자 경험을 위해 반드시 함께 명시해야 하는 속성은 무엇인가? → 3
80. 위 지문의 자바스크립트에서 비디오나 오디오 객체를 선택하여 현재 '일시정지' 상태인지 아닌지를 확인하기 위해 사용하는 프로퍼티는 무엇인가? → 2
81. 위 지문의 HTML5 미디어 API에는 stop( ) 메서드가 존재하지 않는다. 자바스크립트로 영상을 완전히 정지시키고 재생 위치를 맨 처음(0초)으로 되돌리는 로직으로 올바른 것은? → 3
82. 위 지문에서 드래그 앤 드롭 통신의 핵심으로, 드래그가 시작될 때 데이터를 담고 드롭될 때 데이터를 꺼낼 수 있게 해주는 '수레(바구니)' 역할을 하는 이벤트 객체 내부의 프로퍼티 이름은 무엇인가? → 2
83. 위 지문에서 드래그 앤 드롭 구현 시, 목적지 영역에 요소를 내려놓기 위해 타겟의 dragover와 drop 핸들러에서 기본 동작의 차단 목적으로 반드시 사용해야 하는 메서드는? → 2
84. 컴퓨터 바탕화면에 있는 이미지 파일을 끌어다가 브라우저 화면 안으로 떨어뜨렸다. 이때 목적지의 drop 이벤트 내에서 넘어온 파일 데이터를 꺼내보려면 어떤 객체를 확인해야 하는가? → 4
85. 위 지문에서 자바스크립트 객체를 웹 스토리지에 에러 없이 온전하게 보관하기 위한 실무 표준 코드로 (가)에 적합한 것은? → 3
86. 웹 스토리지에 저장된 캔버스 환경설정 정보를 가져오려고 한다. 이를 위한 메서드의 올바른 사용은? → 2
87. window.addEventListener('storage', ...) 이벤트가 브라우저에서 발생하는 정확한 시점은 언제인가? → 4
88. 배달 라이더 앱에서처럼 사용자가 이동할 때마다 브라우저가 알아서 연속적으로 위치 변화를 감지해서 콜백을 실행해 주는 실시간 위치 추적 메서드는? → 3
89. 위 지문에서 위치 정보를 얻어올 때 3번째 인자로 옵션(options) 객체를 전달한다. 이 중 배터리 소모가 심해지더라도 기지국 대신 GPS 센서를 강제로 깨워 최대한 오차 없는 정밀한 위치를 원할 때 사용하는 속성은? → 1
90. 위 지문에서 getCurrentPosition( )이 성공해서 position 객체를 콜백으로 받았고, 여기서 가장 핵심적인 위도와 경도 데이터를 뽑아내려고 한다. 이때 밑줄 친 부분에 들어갈 객체는? → 1

#### Java프로그래밍

##### 암기 리스트

1. [시험 포인트] Java 프로그램의 기본 구성단위는 클래스이다.
2. [시험 포인트] Java 플랫폼은 Java VM과 Java API로 구성된다.
3. [시험 포인트] 클래스는 동종 객체를 추상화시켜 표현한 틀이며, 데이터와 메서드로 구성된다.
4. [시험 포인트] Java에서는 변수의 자료형에 대한 검사가 엄격하다.
5. [시험 포인트] Java 프로그래밍을 위해서 JDK를 설치해야 한다.
6. [시험 포인트] Java 소스는 컴파일된 후, 바이트코드가 Java 플랫폼에서 실행된다.
7. [시험 포인트] main( ) 메서드의 형식은 public static void main(String[ ] args) 이다.
8. [시험 포인트] 객체는 특정 클래스의 인스턴스이다.
9. [핵심 정리] Java 프로그램의 기본 구성단위는 클래스이다.
10. [핵심 정리] Java에서는 변수의 자료형에 대한 검사가 엄격하다.
11. [핵심 정리] Java 프로그래밍을 위해서 JDK를 설치해야 한다.
12. [핵심 정리] Java 소스는 컴파일된 후, 바이트코드가 Java 플랫폼에서 실행된다.
13. [핵심 정리] Java 플랫폼은 Java VM과 Java API로 구성된다.
14. [핵심 정리] main( ) 메서드의 형식은 public static void main(String[ ] args) 이다.
15. [핵심 정리] 클래스는 동종 객체를 추상화시켜 표현한 틀이며, 데이터와 메서드로 구성된다.
16. [핵심 정리] 객체는 특정 클래스의 인스턴스이다.
17. [시험 포인트] 변수의 종류로는 인스턴스 변수, 클래스 변수, 지역 변수, 파라미터가 있다.
18. [시험 포인트] Java의 기본형을 나타내는 키워드는 byte, short, int, long, float, double, char, boolean의 8가지이다.
19. [시험 포인트] 인스턴스 변수는 객체가 소유하는 변수이며, 클래스 변수는 해당 클래스의 객체들이 공유하는 변수이다.
20. [시험 포인트] 클래스와 배열은 대표적 참조형으로, 참조형 변수에는 참조값(주소값)이 저장된다.
21. [시험 포인트] 묵시적 형변환은 자료의 손실이 발생하지 않고, 컴파일러가 자동으로 수행하는 형변환이다.
22. [시험 포인트] 명령행 매개변수란 main( ) 함수에 전달되는 인자로서, 프로그램을 실행시킬 때 클래스 이름 뒤에 나열하는 인자를 말한다.
23. [시험 포인트] break문, continue문, return문은 제어문 중 점프문(분기문)에 해당한다.
24. [핵심 정리] Java의 기본형을 나타내는 키워드는 byte, short, int, long, float, double, char, boolean의 8가지이다.
25. [핵심 정리] 변수의 종류로는 인스턴스 변수, 클래스 변수, 지역 변수, 파라미터가 있다.
26. [핵심 정리] 인스턴스 변수는 객체가 소유하는 변수이며, 클래스 변수는 해당 클래스의 객체들이 공유하는 변수이다.
27. [핵심 정리] 클래스와 배열은 대표적 참조형으로, 참조형 변수에는 참조값(주소값)이 저장된다.
28. [핵심 정리] 묵시적 형변환은 자료의 손실이 발생하지 않고, 컴파일러가 자동으로 수행하는 형변환이다.
29. [핵심 정리] 명령행 매개변수란 main( ) 함수에 전달되는 인자로서, 프로그램을 실행시킬 때 클래스 이름 뒤에 나열하는 인자를 말한다.
30. [핵심 정리] break문, continue문, return문은 제어문 중 점프문(분기문)에 해당한다.
31. [시험 포인트] 클래스를 정의할 때, 내부 클래스가 아니라면 클래스의 접근 제어자로서 생략과 public만 지정할 수 있다.
32. [시험 포인트] 정의 부분에서 접근 제어자가 생략된 클래스나 데이터 필드는 ‘해당 클래스’ 외에 ‘같은 패키지의 다른 클래스’에서만 사용할 수 있다.
33. [시험 포인트] 생성자를 정의하려면 이름을 클래스 이름과 같게 하고 반환형을 지정하지 않는다.
34. [시험 포인트] 배열은 원소의 초기화 과정이나 원소의 생성 과정을 거쳐야만 사용할 수 있다.
35. [시험 포인트] ‘배열이름.length’을 사용하여 배열의 크기를 표현할 수 있다.
36. [시험 포인트] 문자열 변수나 리터럴에 대해 + 연산을 적용한 결과는 두 문자열이 연결된 문자열이다.
37. [시험 포인트] Scanner 클래스를 이용하면 키보드나 파일로부터 편리하게 문자열(또는 기본형 값)을 입력받을 수 있다.
38. [핵심 정리] 배열은 원소의 초기화 과정이나 원소의 생성 과정을 거쳐야만 사용할 수 있다.
39. [핵심 정리] ‘배열이름.length’을 사용하여 배열의 크기를 표현할 수 있다.
40. [핵심 정리] 문자열 변수나 리터럴에 대해 + 연산을 적용한 결과는 두 문자열이 연결된 문자열이다.
41. [핵심 정리] Scanner 클래스를 이용하면 키보드나 파일로부터 편리하게 문자열(또는 기본형 값)을 입력받을 수 있다.
42. [핵심 정리] 클래스를 정의할 때, 내부 클래스가 아니라면 클래스의 접근 제어자로서 생략과 public만 지정할 수 있다.
43. [핵심 정리] 정의 부분에서 접근 제어자가 생략된 클래스나 데이터 필드는 ‘해당 클래스’ 외에 ‘같은 패키지의 다른 클래스’에서만 사용할 수 있다.
44. [핵심 정리] 생성자를 정의하려면 이름을 클래스 이름과 같게 하고 반환형을 지정하지 않는다.
45. [시험 포인트] 클래스를 정의할 때, 클래스의 모든 객체가 공유하는 데이터는 static 데이터 필드로 정의한다.
46. [시험 포인트] 클래스 정의에 있는 데이터 필드의 선언문, 초기화 블록, 생성자를 통해서 객체 생성 시 필요한 데이터 필드의 초깃값을 지정할 수 있다.
47. [시험 포인트] 부모로부터 상속받은 메서드의 몸체를 자식 클래스에서 다시 정의하는 것을 메서드 오버라이딩이라 한다.
48. [시험 포인트] 메서드 오버로딩이란 한 클래스에서 이름이 같은 여러 메서드가 존재하는 상황을 말한다. 이때 오버로딩된 메서드끼리는 매개변수 목록이 달라 구별될 수 있다.
49. [시험 포인트] 인스턴스 메서드와 생성자에서 숨은 인자인 this를 사용할 수 있다.
50. [시험 포인트] this( )는 같은 클래스에 있는 다른 생성자를, super( )는 자식 클래스 생성자에서 부모 클래스의 생성자를 호출하기 위한 것이다.
51. [핵심 정리] 클래스를 정의할 때, 클래스의 모든 객체가 공유하는 데이터는 static 데이터 필드로 정의한다.
52. [핵심 정리] 클래스 정의에 있는 데이터 필드의 선언문, 초기화 블록, 생성자를 통해서 객체 생성 시 필요한 데이터 필드의 초깃값을 지정할 수 있다.
53. [핵심 정리] 메서드 오버로딩이란 한 클래스에서 이름이 같은 여러 메서드가 존재하는 상황을 말한다. 이때 오버로딩된 메서드끼리는 매개변수 목록이 달라 구별될 수 있다.
54. [핵심 정리] 부모로부터 상속받은 메서드의 몸체를 자식 클래스에서 다시 정의하는 것을 메서드 오버라이딩이라 한다.
55. [핵심 정리] 인스턴스 메서드와 생성자에서 숨은 인자인 this를 사용할 수 있다.
56. [핵심 정리] this( )는 같은 클래스에 있는 다른 생성자를, super( )는 자식 클래스 생성자에서 부모 클래스의 생성자를 호출하기 위한 것이다.
57. [시험 포인트] 인터페이스는 추상 메서드로만 구성된다. 단, default 인스턴스 메서드와 static 메서드는 몸체가 있어야 한다.
58. [시험 포인트] 의미적으로 유사한 클래스를 묶을 때는 추상 클래스로, 기능적으로 유사한 클래스를 묶을 때는 인터페이스를 사용한다.
59. [시험 포인트] 열거 자료형은 여러 상수값을 미리 정의하기 위한 자료형이며, 각 상수값은 하나의 객체와 같다.
60. [시험 포인트] 몸체가 없는 메서드를 추상 메서드라고 하며, 추상 클래스 또는 인터페이스에 포함될 수 있다.
61. [시험 포인트] 다형성은 메서드 오버라이딩과 오버로딩, 클래스 간 상속과 형변환, 인터페이스의 구현과 형변환, 메서드 동적 바인딩을 통해 구현될 수 있다.
62. [시험 포인트] 익명 클래스는 이름이 없는 클래스로, 일회성으로 상속 또는 구현을 통해 자식 객체를 생성하는 용도로만 사용되는 클래스를 의미한다.
63. [핵심 정리] 몸체가 없는 메서드를 추상 메서드라고 하며, 추상 클래스 또는 인터페이스에 포함될 수 있다.
64. [핵심 정리] 인터페이스는 추상 메서드로만 구성된다. 단, default 인스턴스 메서드와 static 메서드는 몸체가 있어야 한다.
65. [핵심 정리] 의미적으로 유사한 클래스를 묶을 때는 추상 클래스로, 기능적으로 유사한 클래스를 묶을 때는 인터페이스를 사용한다.
66. [핵심 정리] 다형성은 메서드 오버라이딩과 오버로딩, 클래스 간 상속과 형변환, 인터페이스의 구현과 형변환, 메서드 동적 바인딩을 통해 구현될 수 있다.
67. [핵심 정리] 열거 자료형은 여러 상수값을 미리 정의하기 위한 자료형이며, 각 상수값은 하나의 객체와 같다.
68. [핵심 정리] 익명 클래스는 이름이 없는 클래스로, 일회성으로 상속 또는 구현을 통해 자식 객체를 생성하는 용도로만 사용되는 클래스를 의미한다.
69. [시험 포인트] 자료형을 매개변수로 가지는 클래스와 인터페이스를 제네릭 타입이라고 한다.
70. [시험 포인트] 제네릭 클래스를 사용할 때 제공되는 타입 파라미터는 필드의 자료형, 메서드의 반환형, 메서드에서 인자의 자료형으로 사용될 수 있다.
71. [시험 포인트] 자료형을 매개변수로 가지는 메서드를 제네릭 메서드라고 한다.
72. [시험 포인트] 제네릭을 활용하면 컴파일 시점에 명확한 자료형 검사를 수행할 수 있다.
73. [시험 포인트] 함수형 인터페이스를 구현하는 클래스의 객체를 생성할 때 람다식을 사용하는 것이 효율적이다.
74. [시험 포인트] 람다식의 결과 타입에 해당하는 인터페이스를 람다식의 타깃 타입이라고 한다.
75. [핵심 정리] 자료형을 매개변수로 가지는 클래스와 인터페이스를 제네릭 타입이라고 한다.
76. [핵심 정리] 제네릭 클래스를 사용할 때 제공되는 타입 파라미터는 필드의 자료형, 메서드의 반환형, 메서드에서 인자의 자료형으로 사용될 수 있다.
77. [핵심 정리] 자료형을 매개변수로 가지는 메서드를 제네릭 메서드라고 한다.
78. [핵심 정리] 제네릭을 활용하면 컴파일 시점에 명확한 자료형 검사를 수행할 수 있다.
79. [핵심 정리] 함수형 인터페이스를 구현하는 클래스의 객체를 생성할 때 람다식을 사용하는 것이 효율적이다.
80. [핵심 정리] 람다식의 결과 타입에 해당하는 인터페이스를 람다식의 타깃 타입이라고 한다.
81. [시험 포인트] Java의 패키지는 관련이 있는 클래스와 인터페이스의 묶음이며 계층 구조로 구성된다.
82. [시험 포인트] 다른 패키지에 있는 클래스를 사용할 때는 import 구문을 사용하는 것이 편리하다.
83. [시험 포인트] Java 프로그램에서 필요한 클래스나 패키지는 환경 변수 CLASSPATH에 포함되어 있는 경로상에서 찾을 수 있어야 한다.
84. [시험 포인트] Java 프로그램의 실행 도중 심각하지는 않지만 정상적 흐름을 벗어난 비정상적 상황이 발생할 수 있으며 이것을 예외라고 한다.
85. [시험 포인트] checked Exception에 속하는 예외가 발생할 수 있는 경우, 반드시 예외 처리 구문을 작성해야 한다.
86. [시험 포인트] 예외의 직접 처리를 위해 try-catch 구문을 사용하고, 예외를 전파하려면 메서드 선언에 ‘throws 예외 유형’을 표시한다.
87. [핵심 정리] Java의 패키지는 관련이 있는 클래스와 인터페이스의 묶음이며 계층 구조로 구성된다.
88. [핵심 정리] 다른 패키지에 있는 클래스를 사용할 때는 import 구문을 사용하는 것이 편리하다.
89. [핵심 정리] Java 프로그램에서 필요한 클래스나 패키지는 환경 변수 CLASSPATH에 포함되어 있는 경로상에서 찾을 수 있어야 한다.
90. [핵심 정리] Java 프로그램의 실행 도중 심각하지는 않지만 정상적 흐름을 벗어난 비정상적 상황이 발생할 수 있으며 이것을 예외라고 한다.
91. [핵심 정리] checked Exception에 속하는 예외가 발생할 수 있는 경우, 반드시 예외 처리 구문을 작성해야 한다.
92. [핵심 정리] 예외의 직접 처리를 위해 try-catch 구문을 사용하고, 예외를 전파하려면 메서드 선언에 ‘throws 예외 유형’을 표시한다.
93. [시험 포인트] toString() 메서드와 equals() 메서드는 Object 클래스에서 정의되어 있으며, 몇몇 하위 클래스들에서 적절히 재정의되어 있다.
94. [시험 포인트] java.lang 패키지의 클래스를 사용할 때는 import 구문을 쓰지 않아도 된다.
95. [시험 포인트] String은 문자열 처리를 위한 다양한 메서드를 제공하지만 immutable 클래스로, String 객체는 생성 이후에 내용 변경이 불가능하다.
96. [시험 포인트] 자주 변경되는 문자열을 다루어야 할 경우에는 String 클래스보다 StringBuffer나 StringBuilder 클래스를 사용하는 것이 보다 효율적이다.
97. [시험 포인트] 기본형 값을 객체로 다루기 위해, 모든 기본형마다 상응하는 포장 클래스가 존재한다.
98. [시험 포인트] System 클래스는 표준 입력과 출력, 표준 오류 출력을 위해 in, out, err 객체를 제공한다.
99. [핵심 정리] java.lang 패키지의 클래스를 사용할 때는 import 구문을 쓰지 않아도 된다.
100. [핵심 정리] String은 문자열 처리를 위한 다양한 메서드를 제공하지만 immutable 클래스로, String 객체는 생성 이후에 내용 변경이 불가능하다.
101. [핵심 정리] toString() 메서드와 equals() 메서드는 Object 클래스에서 정의되어 있으며, 몇몇 하위 클래스들에서 적절히 재정의되어 있다.
102. [핵심 정리] 자주 변경되는 문자열을 다루어야 할 경우에는 String 클래스보다 StringBuffer나 StringBuilder 클래스를 사용하는 것이 보다 효율적이다.
103. [핵심 정리] 기본형 값을 객체로 다루기 위해, 모든 기본형마다 상응하는 포장 클래스가 존재한다.
104. [핵심 정리] System 클래스는 표준 입력과 출력, 표준 오류 출력을 위해 in, out, err 객체를 제공한다.
105. [시험 포인트] Java 프로그램에서는 스트림을 통해 데이터 생산자(또는 소비자)의 종류와 무관하게 동일한 방법으로 입력(또는 출력)을 수행할 수 있다.
106. [시험 포인트] BufferedInputStream(BufferdOutputStream)과 BufferedReader(BufferdWriter)는 보조 스트림으로 버퍼링 기능을 제공하여 입출력 성능을 향상시킨다.
107. [시험 포인트] InputStreamReader는 바이트 단위 입력 스트림을 캐릭터 단위 입력 스트림으로 변환하는 기능을 한다.
108. [시험 포인트] File 클래스는 파일 또는 디렉터리를 표현하는 클래스로 파일이나 디렉터리를 조작할 수 있는 메서드를 제공하나 입출력 메서드는 제공하지 않는다.
109. [시험 포인트] RandomAccessFile 클래스는 파일의 읽기/쓰기를 지원하며 파일 포인터가 가리키는 임의의 위치에 읽기/쓰기가 가능하다.
110. [시험 포인트] 기본 스트림의 객체를 먼저 생성하고, 그 객체를 인자로 하여 보조 스트림 객체를 생성한다.
111. [핵심 정리] Java 프로그램에서는 스트림을 통해 데이터 생산자(또는 소비자)의 종류와 무관하게 동일한 방법으로 입력(또는 출력)을 수행할 수 있다.
112. [핵심 정리] File 클래스는 파일 또는 디렉터리를 표현하는 클래스로 파일이나 디렉터리를 조작할 수 있는 메서드를 제공하나 입출력 메서드는 제공하지 않는다.
113. [핵심 정리] RandomAccessFile 클래스는 파일의 읽기/쓰기를 지원하며 파일 포인터가 가리키는 임의의 위치에 읽기/쓰기가 가능하다.
114. [핵심 정리] BufferedInputStream(BufferdOutputStream)과 BufferedReader(BufferdWriter)는 보조 스트림으로 버퍼링 기능을 제공하여 입출력 성능을 향상시킨다.
115. [핵심 정리] 기본 스트림의 객체를 먼저 생성하고, 그 객체를 인자로 하여 보조 스트림 객체를 생성한다.
116. [핵심 정리] InputStreamReader는 바이트 단위 입력 스트림을 캐릭터 단위 입력 스트림으로 변환하는 기능을 한다.
117. [시험 포인트] java.nio.file 패키지의 Path 인터페이스는 java.io 패키지의 File 클래스보다 풍부한 기능을 제공한다.
118. [시험 포인트] Files 클래스는 실제 파일(또는 디렉터리) 조작과 읽기·쓰기 기능을 수행하는 static 메서드를 제공한다.
119. [시험 포인트] java.nio와 그것의 서브 패키지는 java.io 패키지를 개선한 새로운 입출력 패키지이다.
120. [시험 포인트] 채널 입출력에서 버퍼는 기본형 데이터의 보관소이며, 버퍼 단위로 입출력할 수 있다.
121. [시험 포인트] java.nio.file의 FileChannel은 입출력을 모두 제공하고 기본적으로 버퍼링을 사용하며 멀티 스레드 환경에서도 안전하게 동작하도록 설계되어 있다.
122. [시험 포인트] java.nio.file 패키지의 WatchService는 디렉터리에 발생하는 이벤트를 감시하는 서비스이다.
123. [핵심 정리] java.nio와 그것의 서브 패키지는 java.io 패키지를 개선한 새로운 입출력 패키지이다.
124. [핵심 정리] java.nio.file 패키지의 Path 인터페이스는 java.io 패키지의 File 클래스보다 풍부한 기능을 제공한다.
125. [핵심 정리] Files 클래스는 실제 파일(또는 디렉터리) 조작과 읽기·쓰기 기능을 수행하는 static 메서드를 제공한다.
126. [핵심 정리] 채널 입출력에서 버퍼는 기본형 데이터의 보관소이며, 버퍼 단위로 입출력할 수 있다.
127. [핵심 정리] java.nio.file의 FileChannel은 입출력을 모두 제공하고 기본적으로 버퍼링을 사용하며 멀티 스레드 환경에서도 안전하게 동작하도록 설계되어 있다.
128. [핵심 정리] java.nio.file 패키지의 WatchService는 디렉터리에 발생하는 이벤트를 감시하는 서비스이다.
129. [시험 포인트] Map 인터페이스는 의 쌍으로 구성되는 자료의 묶음을 관리하는 메서드를 제공한다.
130. [시험 포인트] 컬렉션은 여러 원소를 저장하고 관리하는 객체를 통칭하는 용어이며, JCF는 컬렉션을 효율적으로 다루기 위한 API를 제공하는 프레임워크이다.
131. [시험 포인트] Set은 원소의 순서에 의미가 없고 중복을 허용하지 않는 컬렉션을 다루기 위한 인터페이스이다.
132. [시험 포인트] List와 Queue는 자료의 중복을 허용하고 순서에 의미가 있는 컬렉션을 관리하기 위한 인터페이스이다. 특히 Queue는 FIFO 방식으로 자료를 관리하는 인터페이스이다.
133. [시험 포인트] LinkedList 클래스는 List 인터페이스와 Queue 인터페이스를 모두 구현한 클래스이며, 스택 자료구조에서 필요한 메서드도 제공한다.
134. [핵심 정리] 컬렉션은 여러 원소를 저장하고 관리하는 객체를 통칭하는 용어이며, JCF는 컬렉션을 효율적으로 다루기 위한 API를 제공하는 프레임워크이다.
135. [핵심 정리] Set은 원소의 순서에 의미가 없고 중복을 허용하지 않는 컬렉션을 다루기 위한 인터페이스이다.
136. [핵심 정리] List와 Queue는 자료의 중복을 허용하고 순서에 의미가 있는 컬렉션을 관리하기 위한 인터페이스이다. 특히 Queue는 FIFO 방식으로 자료를 관리하는 인터페이스이다.
137. [핵심 정리] Map 인터페이스는 의 쌍으로 구성되는 자료의 묶음을 관리하는 메서드를 제공한다.
138. [핵심 정리] LinkedList 클래스는 List 인터페이스와 Queue 인터페이스를 모두 구현한 클래스이며, 스택 자료구조에서 필요한 메서드도 제공한다.
139. [시험 포인트] 내부 반복은 컬렉션이나 배열의 내부에서 각 원소의 반복 처리를 수행하는 것으로, 처리용 코드만 람다식으로 전달하며 반복 작업을 위한 코드를 작성하지 않는다.
140. [시험 포인트] forEach( ) 메서드는 람다식과 함께 사용하여 컬렉션과 배열의 원소들을 외부로 꺼내오지 않고 내부에서 탐색하게 한다.
141. [시험 포인트] 필터링은 스트림의 원소 중에서 중복을 제거하거나 특정 조건을 만족하는 원소만 걸러 내고 새로운 스트림을 리턴하는 중간 연산이다.
142. [시험 포인트] 매핑은 스트림의 원소를 다른 원소로 변환한 후 새로운 스트림으로 리턴하는 중간 연산이다.
143. [시험 포인트] 매칭은 스트림의 원소 중 특정 조건을 만족하는지 확인하는 최종 연산이다.
144. [시험 포인트] 수집은 스트림의 원소 중 필터링 또는 매핑 작업 후 결과 원소들을 취합하여 새로운 List, Set, 또는 Map 객체를 생성하는 최종 연산이다.
145. [핵심 정리] 내부 반복은 컬렉션이나 배열의 내부에서 각 원소의 반복 처리를 수행하는 것으로, 처리용 코드만 람다식으로 전달하며 반복 작업을 위한 코드를 작성하지 않는다.
146. [핵심 정리] forEach( ) 메서드는 람다식과 함께 사용하여 컬렉션과 배열의 원소들을 외부로 꺼내오지 않고 내부에서 탐색하게 한다.
147. [핵심 정리] 필터링은 스트림의 원소 중에서 중복을 제거하거나 특정 조건을 만족하는 원소만 걸러 내고 새로운 스트림을 리턴하는 중간 연산이다.
148. [핵심 정리] 매핑은 스트림의 원소를 다른 원소로 변환한 후 새로운 스트림으로 리턴하는 중간 연산이다.
149. [핵심 정리] 매칭은 스트림의 원소 중 특정 조건을 만족하는지 확인하는 최종 연산이다.
150. [핵심 정리] 수집은 스트림의 원소 중 필터링 또는 매핑 작업 후 결과 원소들을 취합하여 새로운 List, Set, 또는 Map 객체를 생성하는 최종 연산이다.
151. [시험 포인트] 스레드의 실행 코드는, 스레드 생성에 사용되는 Thread를 상속받는 클래스나 Runnable 인터페이스를 구현한 클래스에서 정의되어 있는 public void run( ) 메서드이다.
152. [시험 포인트] Thread 클래스에서 스레드의 상태를 제어하는 메서드로 start(), join(), interrupt()가 있으며, static 메서드인 yield()와 sleep()도 있다. 한편 Object 클래스에 정의된 메서드로는 notify()와 wait()가 있다.
153. [시험 포인트] 멀티 스레드 프로그래밍이란 하나의 프로세스에서 여러 스레드가 생성되어 동시 실행되게 하는 것을 말한다.
154. [시험 포인트] 스레드는 생성되어 소멸할 때까지 Startable, Runnable, Running, Not Running 등의 상태를 거친다.
155. [시험 포인트] 스레드 동기화는 데이터의 무결성을 유지하도록 한 번에 한 스레드만이 스레드 간 공유 객체에 접근하도록 하는 것이며, synchronized 메서드는 동기화를 보장한다.
156. [핵심 정리] 멀티 스레드 프로그래밍이란 하나의 프로세스에서 여러 스레드가 생성되어 동시 실행되게 하는 것을 말한다.
157. [핵심 정리] 스레드의 실행 코드는, 스레드 생성에 사용되는 Thread를 상속받는 클래스나 Runnable 인터페이스를 구현한 클래스에서 정의되어 있는 public void run( ) 메서드이다.
158. [핵심 정리] 스레드는 생성되어 소멸할 때까지 Startable, Runnable, Running, Not Running 등의 상태를 거친다.
159. [핵심 정리] Thread 클래스에서 스레드의 상태를 제어하는 메서드로 start(), join(), interrupt()가 있으며, static 메서드인 yield()와 sleep()도 있다. 한편 Object 클래스에 정의된 메서드로는 notify()와 wait()가 있다.
160. [핵심 정리] 스레드 동기화는 데이터의 무결성을 유지하도록 한 번에 한 스레드만이 스레드 간 공유 객체에 접근하도록 하는 것이며, synchronized 메서드는 동기화를 보장한다.
161. [시험 포인트] MariaDB는 MySQL과 호환되는 오픈 소스 관계형 데이터베이스 관리 시스템이다.
162. [시험 포인트] JDBC는 Java 프로그램에서 관계형 데이터베이스와 연동할 수 있게 하는 표준 API이다.
163. [시험 포인트] JDBC 프로그래밍을 위해서는 MariaDB용 JDBC 드라이버로 .jar 파일을 다운로드받아 Classpath에 추가해야 한다.
164. [시험 포인트] DBMS와 연결을 설정할 때는 java.sql.Connection 객체를 사용한다.
165. [시험 포인트] SQL 구문을 실행할 때 Statement 인터페이스의 execute( ), executeQuery( ), executeUpdate( )를 사용할 수 있다.
166. [시험 포인트] select 구문을 실행할 때 executeQuery( )를 호출하며, 검색 결과가 ResulteSet 객체로 리턴된다.
167. [시험 포인트] 같은 SQL 문을 여러 번 실행할 때는 PreparedStatement 객체를 사용하는 것이 효율적이며, SQL 문에 매개변수를 사용하고 실행 전에 값을 지정할 수 있다.
168. [핵심 정리] MariaDB는 MySQL과 호환되는 오픈 소스 관계형 데이터베이스 관리 시스템이다.
169. [핵심 정리] JDBC는 Java 프로그램에서 관계형 데이터베이스와 연동할 수 있게 하는 표준 API이다.
170. [핵심 정리] JDBC 프로그래밍을 위해서는 MariaDB용 JDBC 드라이버로 .jar 파일을 다운로드받아 Classpath에 추가해야 한다.
171. [핵심 정리] DBMS와 연결을 설정할 때는 java.sql.Connection 객체를 사용한다.
172. [핵심 정리] SQL 구문을 실행할 때 Statement 인터페이스의 execute( ), executeQuery( ), executeUpdate( )를 사용할 수 있다.
173. [핵심 정리] select 구문을 실행할 때 executeQuery( )를 호출하며, 검색 결과가 ResulteSet 객체로 리턴된다.
174. [핵심 정리] 같은 SQL 문을 여러 번 실행할 때는 PreparedStatement 객체를 사용하는 것이 효율적이며, SQL 문에 매개변수를 사용하고 실행 전에 값을 지정할 수 있다.
175. [시험 포인트] 자주 사용되는 클래스와 인터페이스를 컴파일된 형태로 제공한 것을 라이브러리라고 한다.
176. [시험 포인트] Java의 라이브러리는 .class 파일을 묶어서 만든 .jar 파일의 형태이다.
177. [시험 포인트] 모듈은 라이브러리의 업그레이드 버전으로 패키지 단위에서 접근성을 설정할 수 있다.
178. [시험 포인트] 라이브러리를 사용하려면 해당 .jar 파일을 Classpath에 추가하고, 모듈을 사용하려면 Modulepath에 추가한다.
179. [시험 포인트] 모듈 프로젝트에서 ‘module-info.java’ 파일에 ‘exports 패키지;’ 또는 ‘requires 모듈;’ 형식으로 모듈 간 의존관계를 설정할 수 있다.
180. [시험 포인트] JDK의 표준 라이브러리는 모두 모듈화되었으며, java.base는 자바 표준 모듈 중 가장 기본 모듈이다.
181. [핵심 정리] 자주 사용되는 클래스와 인터페이스를 컴파일된 형태로 제공한 것을 라이브러리라고 한다.
182. [핵심 정리] Java의 라이브러리는 .class 파일을 묶어서 만든 .jar 파일의 형태이다.
183. [핵심 정리] 모듈은 라이브러리의 업그레이드 버전으로 패키지 단위에서 접근성을 설정할 수 있다.
184. [핵심 정리] 라이브러리를 사용하려면 해당 .jar 파일을 Classpath에 추가하고, 모듈을 사용하려면 Modulepath에 추가한다.
185. [핵심 정리] 모듈 프로젝트에서 ‘module-info.java’ 파일에 ‘exports 패키지;’ 또는 ‘requires 모듈;’ 형식으로 모듈 간 의존관계를 설정할 수 있다.
186. [핵심 정리] JDK의 표준 라이브러리는 모두 모듈화되었으며, java.base는 자바 표준 모듈 중 가장 기본 모듈이다.

##### 정답 감각 리스트

1. Java 언어의 특징 중에 다음 내용이 의미하는 것은? → 2
2. 다음 파일의 이름은 A.java 이다. 이 파일을 컴파일할 때 결과로 만들어지는 것을 정확히 설명한 것은? → 3
3. 서술형 주관식 입력칸 → 서술형/공식 정답 미제공
4. 다음 중 클래스 이름에 해당하지 않는 것은? → 4
5. 다음 정수 리터럴 중 값이 다른 하나는 무엇인가? → 2
6. 서술형 주관식 입력칸 → (답)
7. 다음 중 문법적으로 올바른 문장은 무엇인가? → 3
8. int형 변수 i와 j의 값을 각각 출력하기 위해 적당한 출력문은 무엇인가? → 2
9. 퍼블릭 Alpha 클래스에서 protected 데이터 필드가 정의되었다고 가정할 때, 아래 4개의 클래스 중에서 이 데이터 필드를 사용할 수 없는 클래스는 무엇인가? → 4
10. 키워드 final에 관한 설명으로 틀린 것은? → 4
11. Circle 클래스에서 원주율 PI를 상수로 선언하기 위해 ㉠에 들어갈 적당한 내용은 무엇인가? → 4
12. 서술형 주관식 입력칸 → this.radius = radius;
13. 다음 프로그램을 실행했을 때 예상되는 출력은? → 2
14. 밑줄 친 ㉠, ㉡에 들어갈 키워드는 순서대로 무엇인가? → 2
15. 서술형 주관식 입력칸 → CSuper 클래스를 상속받는 익명 클래스를 정의하고, 동시에 익명 클래스의 객체를 생성한다.
16. 다음과 같은 제네릭 클래스가 있다고 가정하자. 보기에서 문법적으로 오류가 있는 것은? → 1
17. 다음과 같은 인터페이스가 있다고 가정할 때, 보기에서 람다식 사용이 잘못된 것은? → 3
18. 서술형 주관식 입력칸 → T get( )
19. Client 클래스를 myprogram.game 패키지에 위치시키려고 한다. Client 클래스를 정의하는 소스 파일의 맨 위에 포함시켜야 코드는 무엇인가? → 1
20. 밑줄 부분에 들어가야 할 내용은 무엇인가? → 2
21. 서술형 주관식 입력칸 → import java.util.Scanner; 또는 import java.util.*;
22. 다음 프로그램의 출력 결과는 무엇인가? → 1
23. Object 클래스와 String 클래스에 대한 설명이다. 잘못된 것은 무엇인가? → 4
24. 서술형 주관식 입력칸 → String 클래스는 immutable 클래스여서 반복이 진행될수록 계속해서 기존 문자열은 버려지고 새로운 문자열이 새롭게 만들어지기 때문이다(버려지거나 만들어지는 문자열의 길이도 점점 커진다). StringBuffer 클래스를 사용한 경우가 StringBuilder 클래스를 사용한 경우보다 시간이 더 걸리는 이유는 동시성 제어를 고려하여 메소드가 구현되었기 때문이다.
25. 입력 스트림은 기본 스트림과 보조 스트림으로 나뉜다. 다음 중 기본 스트림에 해당하지 않는 것은? → 4
26. BufferedReader 클래스에 관한 설명으로 틀린 것은 무엇인가? → 3
27. 서술형 주관식 입력칸 → InputStreamReader
28. Path 인터페이스에 관한 설명이다. 잘못된 것은? → 4
29. FileChannel 클래스에 관한 설명으로 틀린 것은 무엇인가? → 3
30. 서술형 주관식 입력칸 → WatchKey는 등록된 디렉터리에 어떤 관심 이벤트가 등록되어 있으며, 실제 어떤 이벤트가 일어났는지에 관한 상태 정보를 가지고 있다.
31. ArrayList 클래스에 관한 설명으로 적당하지 않은 것은? → 4
32. (key, value) 형태의 원소로 구성되는 자료 묶음을 다루기 위한 인터페이스나 클래스가 아닌 것은? → 2
33. 서술형 주관식 입력칸 → 추가 시 boolean offer(E), 삭제 시 E poll( )을 사용함
34. 주어진 배열을 스트림으로 만들려고 한다. 밑줄 부분에 들어갈 적당한 내용은? → 2
35. 주어진 문자열 배열에서 길이가 6 이상인 단어를 대문자로 변환한 후 정렬하여 출력하려고 한다. 밑줄 부분에 들어갈 메소드는 무엇인가? → 4
36. 서술형 주관식 입력칸 → 중간연산은 스트림을 변환하거나 필터링 하는 등의 작업을 수행하며, 다음 단계 처리를 위해 새로운 스트림을 리턴하는데, 체인 형태로 연속해 여러 번 호출될 수 있다. 종료연산은 스트림의 원소를 이용해 최종 결과를 만들어 리턴한다.
37. 스레드 동기화와 관련된 다음 설명 중 올바른 것은? → 4
38. 스레드 동기화의 의미를 가장 잘 설명한 것은? → 3
39. 주관식 정답 입력칸 → synchronized
40. JDBC 프로그래밍에 사용되는 클래스나 인터페이스가 아닌 것은? → 4
41. PreparedStatement 객체에 대한 설명으로 잘못된 것은? → 1
42. 서술형 주관식 입력칸 → ResultSet
43. .jar 파일 형식으로 배포된 라이브러리를 사용하고자 할 때, 이러한 .jar 파일을 어떤 환경 변수에 추가해야 하는가? → 1
44. Java의 표준 모듈 중 requires 키워드로 사용 선언을 하지 않아도 사용이 가능한 모듈은 무엇인가? → 4
45. 서술형 주관식 입력칸 → requires java.sql;

#### 파이썬프로그래밍기초

##### 암기 리스트

1. [시험 포인트] 컴퓨터는 데이터를 외부로부터 받아들이는 입력기능, 내부 장치를 조정하는 제어기능, 입력된 데이터를 저장하는 저장⦁기억기능, 계산을 수행하는 연산기능, 내부의 데이터를 사용자에게 전달하는 출력기능을 갖추고 있다.
2. [시험 포인트] 프로그램이란 컴퓨터가 어떠한 작업을 자동으로 처리할 수 있도록, 처리 방법 및 순서를 컴퓨터가 이해할 수 있는 언어로 기술해 놓은 것을 뜻한다. 프로그램을 제작하는 사람을 프로그래머라고 하며, 이 제작 과정을 프로그래밍이라 부른다.
3. [시험 포인트] 알고리즘은 문제를 풀기 위한 단계별 절차를 수학적으로 기술한 것을 뜻한다. 알고리즘에는 입력과 출력이 있어야 하며, 명확성과 유한성, 효율성이 보장되어야 한다.
4. [시험 포인트] 데이터란 관찰이나 측정을 통해 얻은 수치, 문자와 같은 변할 수 있는 질적 또는 양적 값을 뜻한다.
5. [시험 포인트] 정보란 데이터를 처리하여 실제 문제해결에 도움이 될 수 데이터를 정리한 결과물을 뜻한다. 데이터 처리에는 정렬, 합산, 군집화 등의 작업이 동반된다.
6. [시험 포인트] 데이터는 현상으로부터 관찰 및 측정을 통해 얻어지며 정보는 데이터를 처리 및 가공하여 추출된다.
7. [시험 포인트] 컴퓨터란 입력된 데이터를 자동으로 처리하는 기계로, 데이터 처리 과정을 순서화하여 전체 과정을 자동화할 수 있는 장치를 말한다.
8. [시험 포인트] 디지털 컴퓨터는 0과 1이라는 신호만으로 동작하기 때문에, 데이터 및 명령문 또한 0과 1로 표현되어야 한다. 즉, 수나 문자, 명령어 모두 0과 1로 표현된다.
9. [핵심 정리] 데이터란 관찰이나 측정을 통해 얻은 수치, 문자와 같은 변할 수 있는 질적 또는 양적 값을 뜻한다.
10. [핵심 정리] 정보란 데이터를 처리하여 실제 문제해결에 도움이 될 수 데이터를 정리한 결과물을 뜻한다. 데이터 처리에는 정렬, 합산, 군집화 등의 작업이 동반된다.
11. [핵심 정리] 데이터는 현상으로부터 관찰 및 측정을 통해 얻어지며 정보는 데이터를 처리 및 가공하여 추출된다.
12. [핵심 정리] 컴퓨터란 입력된 데이터를 자동으로 처리하는 기계로, 데이터 처리 과정을 순서화하여 전체 과정을 자동화할 수 있는 장치를 말한다.
13. [핵심 정리] 컴퓨터는 데이터를 외부로부터 받아들이는 입력기능, 내부 장치를 조정하는 제어기능, 입력된 데이터를 저장하는 저장⦁기억기능, 계산을 수행하는 연산기능, 내부의 데이터를 사용자에게 전달하는 출력기능을 갖추고 있다.
14. [핵심 정리] 디지털 컴퓨터는 0과 1이라는 신호만으로 동작하기 때문에, 데이터 및 명령문 또한 0과 1로 표현되어야 한다. 즉, 수나 문자, 명령어 모두 0과 1로 표현된다.
15. [핵심 정리] 프로그램이란 컴퓨터가 어떠한 작업을 자동으로 처리할 수 있도록, 처리 방법 및 순서를 컴퓨터가 이해할 수 있는 언어로 기술해 놓은 것을 뜻한다. 프로그램을 제작하는 사람을 프로그래머라고 하며, 이 제작 과정을 프로그래밍이라 부른다.
16. [핵심 정리] 알고리즘은 문제를 풀기 위한 단계별 절차를 수학적으로 기술한 것을 뜻한다. 알고리즘에는 입력과 출력이 있어야 하며, 명확성과 유한성, 효율성이 보장되어야 한다.
17. [핵심 정리] 프로그램은 프로그래밍 언어를 이용하여 알고리즘을 구현한 결과이며 프로그래밍 언어로 작성된 소스코드를 컴파일러나 인터프리터를 통해 컴퓨터에서 실행될 수 있는 프로그램으로 생성된다.
18. [시험 포인트] 파이썬은 명령형 프로그래밍, 절차적 프로그래밍, 객체지향 프로그래밍, 함수형 프로그래밍 패러다임을 지원한다.
19. [시험 포인트] 파이썬은 파이썬 확장 제안(PEP)를 통해 새로운 파이썬의 기능, 파이썬 프로세스, 환경에 대해 커뮤니티에 설계 문서나 정보를 제공하는 등 많은 개발자의 의견을 수용하고 토론하며 발전한 언어이다.
20. [시험 포인트] 파이썬 개발환경에서는 구문 강조, 자동 완성, 스마트 들여쓰기 등의 기능을 지원하는 IDLE, 주피터 노트북, 구글 코랩 등이 있다.
21. [시험 포인트] 파이썬은 히도 판로쉼에 의해 1991년 분산 운영 체제(아메바)의 시스템 관리를 위한 쉘 스크립팅 언어로 개발되었다.
22. [시험 포인트] 파이썬은 교육적인 목적뿐만 아니라 응용 프로그램과 웹, 백 엔드 개발, 사물 인터넷 등 다양한 분야에 활용된다.
23. [시험 포인트] 파이썬은 플랫폼에 독립적이며 인터프리터식 객체지향적, 동적 타이핑(dynamically typed) 대화형 언어이다.
24. [시험 포인트] 파이썬 소스 코드는 파이썬 인터프리팅 과정에서 CPython에 의해 바이트 코드로 변환된고 파이썬 가상머신(PVM)이 한 라인씩 기계어 코드로 변환하여 실행된다.
25. [시험 포인트] 구글 코랩(Colab)은 주피터 노트북과 구글 드라이브를 결합한 서비스로 데이터 분석 및 딥러닝 연산 등 고성능 컴퓨팅 리소스 활용 가능하다.
26. [핵심 정리] 파이썬은 히도 판로쉼에 의해 1991년 분산 운영 체제(아메바)의 시스템 관리를 위한 쉘 스크립팅 언어로 개발되었다.
27. [핵심 정리] 파이썬은 명령형 프로그래밍, 절차적 프로그래밍, 객체지향 프로그래밍, 함수형 프로그래밍 패러다임을 지원한다.
28. [핵심 정리] 파이썬은 교육적인 목적뿐만 아니라 응용 프로그램과 웹, 백 엔드 개발, 사물 인터넷 등 다양한 분야에 활용된다.
29. [핵심 정리] 파이썬은 파이썬 확장 제안(PEP)를 통해 새로운 파이썬의 기능, 파이썬 프로세스, 환경에 대해 커뮤니티에 설계 문서나 정보를 제공하는 등 많은 개발자의 의견을 수용하고 토론하며 발전한 언어이다.
30. [핵심 정리] 파이썬은 플랫폼에 독립적이며 인터프리터식 객체지향적, 동적 타이핑(dynamically typed) 대화형 언어이다.
31. [핵심 정리] 파이썬 소스 코드는 파이썬 인터프리팅 과정에서 CPython에 의해 바이트 코드로 변환된고 파이썬 가상머신(PVM)이 한 라인씩 기계어 코드로 변환하여 실행된다.
32. [핵심 정리] 파이썬 개발환경에서는 구문 강조, 자동 완성, 스마트 들여쓰기 등의 기능을 지원하는 IDLE, 주피터 노트북, 구글 코랩 등이 있다.
33. [핵심 정리] 구글 코랩(Colab)은 주피터 노트북과 구글 드라이브를 결합한 서비스로 데이터 분석 및 딥러닝 연산 등 고성능 컴퓨팅 리소스 활용 가능하다.
34. [시험 포인트] 데이터는 값의 종류에 따라 숫자와 문자 타입 등으로 구분하며, 이들이 연산자(+, -, *, /)와 결합한 표현식은 인터프리터에 의해 자동 계산된다.
35. [시험 포인트] 기본 연산 외에 거듭제곱(**), 몫(//), 나머지(%) 연산자가 있으며, 괄호와 연산자 종류에 따른 우선순위가 존재한다.
36. [시험 포인트] 파이썬은 히도 판로쉼(Guido van Rossum)에 의해 1991년 분산 운영 체제(아메바)의 시스템 관리를 위한 쉘 스크립팅 언어로 개발되었다.
37. [시험 포인트] 특정 작업을 수행하는 코드의 집합으로 함수의 이름만으로 실행할 수 있는 단위를 함수라고 한다.
38. [시험 포인트] 변수는 데이터를 임시 저장하는 컴퓨터의 메모리 공간이며, 할당연산자(=)를 통해 값을 저장한다.
39. [시험 포인트] 파이썬은 들여쓰기에 의존적인 언어로, 코드의 논리적 집합인 블록을 표현하기 위해 필수적으로 사용한다.
40. [시험 포인트] 프로그램의 가독성을 높이기 위해 #를 사용하여 코드의 의미를 설명하는 주석을 작성한다.
41. [핵심 정리] 파이썬은 히도 판로쉼(Guido van Rossum)에 의해 1991년 분산 운영 체제(아메바)의 시스템 관리를 위한 쉘 스크립팅 언어로 개발되었다.
42. [핵심 정리] 데이터는 값의 종류에 따라 숫자와 문자 타입 등으로 구분하며, 이들이 연산자(+, -, *, /)와 결합한 표현식은 인터프리터에 의해 자동 계산된다.
43. [핵심 정리] 특정 작업을 수행하는 코드의 집합으로 함수의 이름만으로 실행할 수 있는 단위를 함수라고 한다.
44. [핵심 정리] 변수는 데이터를 임시 저장하는 컴퓨터의 메모리 공간이며, 할당연산자(=)를 통해 값을 저장한다.
45. [핵심 정리] 기본 연산 외에 거듭제곱(**), 몫(//), 나머지(%) 연산자가 있으며, 괄호와 연산자 종류에 따른 우선순위가 존재한다.
46. [핵심 정리] 파이썬은 들여쓰기에 의존적인 언어로, 코드의 논리적 집합인 블록을 표현하기 위해 필수적으로 사용한다.
47. [핵심 정리] 프로그램의 가독성을 높이기 위해 #를 사용하여 코드의 의미를 설명하는 주석을 작성한다.
48. [시험 포인트] 파이썬은 구조적 프로그래밍 패러다임을 지원하며 goto 문을 사용하지 않고 프로그램을 구성할 수 있는 순차, 선택, 반복 구조를 제공한다.
49. [시험 포인트] 프로그램의 설계 결함이나 문법 오류 등으로 의도한 결과를 생성하지 못하는 프로그램 오류에는 구문 오류, 실행 오류, 의미 오류가 있다.
50. [시험 포인트] 구조적 프로그래밍 패러다임은 제어 흐름을 블록 단위로 계층화하고 지역화하여 프로그램의 예측 가능성을 보장한다.
51. [시험 포인트] 순차 구조는 명령 라인이 위에서 아래로 흐르는 직관적인 구조로 사람이 이해하고 오류를 수정하기 용이하다.
52. [시험 포인트] 사용자의 키보드 입력을 한 줄의 문자열로 읽어 반환하는 내장 함수 input()을 통해 데이터를 입력받을 수 있다.
53. [시험 포인트] 한 데이터를 다른 데이터 타입으로 변환하는 내장 함수를 통해 문자열을 정수나 소수 타입으로 변경할 수 있다.
54. [시험 포인트] 출력 함수의 파라미터로 여러 데이터를 입력할 수 있으며 옵션을 통해 데이터 사이의 공백을 변경할 수 있다.
55. [핵심 정리] 구조적 프로그래밍 패러다임은 제어 흐름을 블록 단위로 계층화하고 지역화하여 프로그램의 예측 가능성을 보장한다.
56. [핵심 정리] 파이썬은 구조적 프로그래밍 패러다임을 지원하며 goto 문을 사용하지 않고 프로그램을 구성할 수 있는 순차, 선택, 반복 구조를 제공한다.
57. [핵심 정리] 순차 구조는 명령 라인이 위에서 아래로 흐르는 직관적인 구조로 사람이 이해하고 오류를 수정하기 용이하다.
58. [핵심 정리] 사용자의 키보드 입력을 한 줄의 문자열로 읽어 반환하는 내장 함수 input()을 통해 데이터를 입력받을 수 있다.
59. [핵심 정리] 한 데이터를 다른 데이터 타입으로 변환하는 내장 함수를 통해 문자열을 정수나 소수 타입으로 변경할 수 있다.
60. [핵심 정리] 프로그램의 설계 결함이나 문법 오류 등으로 의도한 결과를 생성하지 못하는 프로그램 오류에는 구문 오류, 실행 오류, 의미 오류가 있다.
61. [핵심 정리] 출력 함수의 파라미터로 여러 데이터를 입력할 수 있으며 옵션을 통해 데이터 사이의 공백을 변경할 수 있다.
62. [시험 포인트] 코드 블록과 들여쓰기: 파이썬은 들여쓰기를 통해 코드 블록을 구분하며, 스페이스 4칸 사용을 권고한다.
63. [시험 포인트] 비교 연산자: , >=, ==, !=를 사용하여 불리언 결과값을 생성한다.
64. [시험 포인트] 연산자 우선순위: 괄호 내 수식 → 거듭제곱 → 곱셈/나눗셈 → 덧셈/뺄셈 → 비교 연산자 → 논리 연산자(not > and > or) 순으로 적용된다.
65. [시험 포인트] 이분 선택 구조 : 조건이 참일 때와 거짓일 때 실행할 명령문을 각각 나누어 정의한다.
66. [시험 포인트] 선택구조: 특정 영역 내의 명령문에 대한 실행 여부를 조건에 따라 판단하여 결정하는 구조이며 프로그램의 안정성과 예외 처리를 책임진다.
67. [시험 포인트] 논리 연산자: and, or, not 연산자를 사용한다.
68. [시험 포인트] 단락 평가: A and B에서 A가 거짓이면 B를 평가하지 않고, A or B에서 A가 참이면 B를 평가하지 않는 기법이다.
69. [시험 포인트] 다분 선택 구조 : 여러 개의 불리언식을 순차적으로 평가하여 해당하는 명령문 집합 하나를 실행한다.
70. [핵심 정리] 선택구조: 특정 영역 내의 명령문에 대한 실행 여부를 조건에 따라 판단하여 결정하는 구조이며 프로그램의 안정성과 예외 처리를 책임진다.
71. [핵심 정리] 코드 블록과 들여쓰기: 파이썬은 들여쓰기를 통해 코드 블록을 구분하며, 스페이스 4칸 사용을 권고한다.
72. [핵심 정리] 비교 연산자: , >=, ==, !=를 사용하여 불리언 결과값을 생성한다.
73. [핵심 정리] 논리 연산자: and, or, not 연산자를 사용한다.
74. [핵심 정리] 연산자 우선순위: 괄호 내 수식 → 거듭제곱 → 곱셈/나눗셈 → 덧셈/뺄셈 → 비교 연산자 → 논리 연산자(not > and > or) 순으로 적용된다.
75. [핵심 정리] 단락 평가: A and B에서 A가 거짓이면 B를 평가하지 않고, A or B에서 A가 참이면 B를 평가하지 않는 기법이다.
76. [핵심 정리] 이분 선택 구조 : 조건이 참일 때와 거짓일 때 실행할 명령문을 각각 나누어 정의한다.
77. [핵심 정리] 다분 선택 구조 : 여러 개의 불리언식을 순차적으로 평가하여 해당하는 명령문 집합 하나를 실행한다.
78. [핵심 정리] 중첩 선택 구조 : 선택 구조 내부에 또 다른 선택 구조를 포함시켜 더욱 세밀한 조건 분기를 수행할 수 있다.
79. [시험 포인트] for 문은 리스트 등과 같은 시퀀스의 첫 번째 요소부터 마지막 요소까지 하나씩 변수에 대입하여 블럭의 명령문을 수행하며 마지막 요소까지 수행한 후 종료하는 계수 제어 반복이다.
80. [시험 포인트] 반복 구조란 동일한 명령문에 대해서 사용자가 지정한 횟수만큼 또는 조건을 만족하는 동안 동일한 코드를 반복하여 수행할 때 사용하는 구조이다.
81. [시험 포인트] 조건 제어 반복은 조건을 평가하여 참일 경우 명령문을 실행하고 거짓일 경우 반복을 종료하며, 계수 제어 반복은 계수 제어 변수에 시퀀스의 원소를 순차적으로 할당하며 명령문을 반복 실행한다.
82. [시험 포인트] 증감 할당은 변수의 현재 값에 특정 값을 증가 또는 차감한 후 그 결과를 동일한 변수에 재할당하는 연산이다.
83. [시험 포인트] whlie문은 조건이 참(True)인 동안 while문 블럭 내부의 명령문을 수행하며 조건이 거짓(False)이 될 때 while문을 빠져나오는 조건 제어 반복 구조이다.
84. [시험 포인트] 리스트는 여러 데이터를 순서대로 묶어 저장하는 시퀀스 타입으로 데이터가 저장된 순서가 유지된다.
85. [시험 포인트] 리스트의 각 데이터는 0부터 시작하는 고유한 인덱스가 지정되어 개별 원소에 접근하거나 값을 수정할 수 있다.
86. [시험 포인트] 중첩 반복 구조는 하나의 외부 반복 구조에 여러 개의 내부 반복 구조가 중첩되어 사용되는 구조를 말하며 외부 반복 구조가 매 반복될 때마다 내부 반복 구조에 재진입하여 새롭게 실행된다.
87. [핵심 정리] 반복 구조란 동일한 명령문에 대해서 사용자가 지정한 횟수만큼 또는 조건을 만족하는 동안 동일한 코드를 반복하여 수행할 때 사용하는 구조이다.
88. [핵심 정리] 조건 제어 반복은 조건을 평가하여 참일 경우 명령문을 실행하고 거짓일 경우 반복을 종료하며, 계수 제어 반복은 계수 제어 변수에 시퀀스의 원소를 순차적으로 할당하며 명령문을 반복 실행한다.
89. [핵심 정리] 증감 할당은 변수의 현재 값에 특정 값을 증가 또는 차감한 후 그 결과를 동일한 변수에 재할당하는 연산이다.
90. [핵심 정리] whlie문은 조건이 참(True)인 동안 while문 블럭 내부의 명령문을 수행하며 조건이 거짓(False)이 될 때 while문을 빠져나오는 조건 제어 반복 구조이다.
91. [핵심 정리] 리스트는 여러 데이터를 순서대로 묶어 저장하는 시퀀스 타입으로 데이터가 저장된 순서가 유지된다.
92. [핵심 정리] 리스트의 각 데이터는 0부터 시작하는 고유한 인덱스가 지정되어 개별 원소에 접근하거나 값을 수정할 수 있다.
93. [핵심 정리] for 문은 리스트 등과 같은 시퀀스의 첫 번째 요소부터 마지막 요소까지 하나씩 변수에 대입하여 블럭의 명령문을 수행하며 마지막 요소까지 수행한 후 종료하는 계수 제어 반복이다.
94. [핵심 정리] 중첩 반복 구조는 하나의 외부 반복 구조에 여러 개의 내부 반복 구조가 중첩되어 사용되는 구조를 말하며 외부 반복 구조가 매 반복될 때마다 내부 반복 구조에 재진입하여 새롭게 실행된다.
95. [시험 포인트] 반환값의 유무에 따라 반환값이 없는 함수와 반환값이 있는 함수로 구분된다.
96. [시험 포인트] 함수는 def 함수이름(매개변수 리스트): 형식으로 정의하며 호출할 때 전달하는 값은 인자이고, 매개변수는 함수 내부에서 변수처럼 사용된다.
97. [시험 포인트] 변수의 스코프는 프로그램에서 변수가 참조될 수 있는 영역이다. 변수의 스코프에 따라 변수는 전역변수와 지역변수로 구분된다.
98. [시험 포인트] 함수 호출 시 매개변수를 사용자가 원하는 개수만큼 지정할 수 있는 매개변수를 가변 매개변수라고 하고 함수 정의 시 매개변수 이름 앞에 *를 사용한다.
99. [시험 포인트] 함수는 특정 작업을 수행하는 명령문의 집합으로, 특정 작업을 함수의 이름으로 대체하여 유사한 문제를 추상화하고 코드 중복을 줄인다.
100. [시험 포인트] 반환값이 없는 함수는 함수 실행 후 어떤 값도 남기지 않는 함수이다. 반면 반환값이 있는 함수는 함수 내부에서 return 명령어를 통해 값을 남긴다.
101. [시험 포인트] format 함수는 결과값의 폭·정렬·소수 자릿수를 형식화할 수 있으며, 동시 할당은 복수개의 변수에 값을 동시에 할당하고 교환이나 복수 결과 전달에 활용된다.
102. [시험 포인트] 동시 할당이란 복수개의 변수에 값을 동시에 할당하는 연산이다. 이를 통해 단일 명령문으로 변수의 값을 맞바꿈이 가능하다.
103. [핵심 정리] 함수는 특정 작업을 수행하는 명령문의 집합으로, 특정 작업을 함수의 이름으로 대체하여 유사한 문제를 추상화하고 코드 중복을 줄인다.
104. [핵심 정리] 반환값의 유무에 따라 반환값이 없는 함수와 반환값이 있는 함수로 구분된다.
105. [핵심 정리] 반환값이 없는 함수는 함수 실행 후 어떤 값도 남기지 않는 함수이다. 반면 반환값이 있는 함수는 함수 내부에서 return 명령어를 통해 값을 남긴다.
106. [핵심 정리] 함수는 def 함수이름(매개변수 리스트): 형식으로 정의하며 호출할 때 전달하는 값은 인자이고, 매개변수는 함수 내부에서 변수처럼 사용된다.
107. [핵심 정리] format 함수는 결과값의 폭·정렬·소수 자릿수를 형식화할 수 있으며, 동시 할당은 복수개의 변수에 값을 동시에 할당하고 교환이나 복수 결과 전달에 활용된다.
108. [핵심 정리] 동시 할당이란 복수개의 변수에 값을 동시에 할당하는 연산이다. 이를 통해 단일 명령문으로 변수의 값을 맞바꿈이 가능하다.
109. [핵심 정리] 값의 전달에 의한 호출이란 함수 호출 시 함수 내부에는 매개변수의 값이 전달된다.
110. [핵심 정리] 변수의 스코프는 프로그램에서 변수가 참조될 수 있는 영역이다. 변수의 스코프에 따라 변수는 전역변수와 지역변수로 구분된다.
111. [핵심 정리] 기본 매개변수란 함수 호출 시 매개변수가 전달되지 않을 경우 기본값이 전달되는 매개변수를 말한다.
112. [핵심 정리] 함수 호출 시 매개변수를 사용자가 원하는 개수만큼 지정할 수 있는 매개변수를 가변 매개변수라고 하고 함수 정의 시 매개변수 이름 앞에 *를 사용한다.
113. [시험 포인트] 콜렉션은 여러 데이터를 하나의 구조로 묶어 저장·관리하기 위한 자료 구조이며, 저장 방식과 접근 방식에 따라 시퀀스, 비시퀀스, 매핑으로 구분된다.
114. [시험 포인트] 리스트 메서드는 리스트 객체에 대해 동작하는 기능으로, append(), count(), index(), pop(), insert(), remove(), reverse(), sort() 등을 이용해 리스트를 조작한다.
115. [시험 포인트] 리스트 슬라이싱은 [시작인덱스:끝인덱스:간격] 형식으로 부분 리스트를 생성하는 방법이며, 시작 위치·끝 위치·간격을 조절하여 다양한 방식의 부분 추출이 가능하다.
116. [시험 포인트] 리스트 컴프리헨션은 반복과 조건을 결합하여 리스트를 간결하게 생성·변환·필터링하는 방법이다.
117. [시험 포인트] 리스트는 순서가 있는 가변 시퀀스로서 서로 다른 데이터 타입을 함께 저장할 수 있으며, 원소의 추가·삭제·수정이 가능하다.
118. [시험 포인트] 튜플은 순서가 있는 불변 시퀀스로 생성 후 원소를 변경할 수 없고, 세트는 중복을 허용하지 않으며 순서를 유지하지 않는다. 딕셔너리는 key-value 쌍으로 데이터를 저장하는 매핑 구조이다.
119. [시험 포인트] 리스트의 각 원소는 인덱스를 통해 접근할 수 있으며, 양수 인덱스는 왼쪽부터, 음수 인덱스는 오른쪽부터 원소를 가리킨다.
120. [시험 포인트] 리스트에는 in, not in, +, * 등의 연산자를 사용할 수 있고, len(), min(), max(), sum()과 같은 내장 함수를 이용해 리스트의 길이와 값의 특성을 처리할 수 있다.
121. [핵심 정리] 콜렉션은 여러 데이터를 하나의 구조로 묶어 저장·관리하기 위한 자료 구조이며, 저장 방식과 접근 방식에 따라 시퀀스, 비시퀀스, 매핑으로 구분된다.
122. [핵심 정리] 리스트는 순서가 있는 가변 시퀀스로서 서로 다른 데이터 타입을 함께 저장할 수 있으며, 원소의 추가·삭제·수정이 가능하다.
123. [핵심 정리] 튜플은 순서가 있는 불변 시퀀스로 생성 후 원소를 변경할 수 없고, 세트는 중복을 허용하지 않으며 순서를 유지하지 않는다. 딕셔너리는 key-value 쌍으로 데이터를 저장하는 매핑 구조이다.
124. [핵심 정리] 리스트의 각 원소는 인덱스를 통해 접근할 수 있으며, 양수 인덱스는 왼쪽부터, 음수 인덱스는 오른쪽부터 원소를 가리킨다.
125. [핵심 정리] 리스트에는 in, not in, +, * 등의 연산자를 사용할 수 있고, len(), min(), max(), sum()과 같은 내장 함수를 이용해 리스트의 길이와 값의 특성을 처리할 수 있다.
126. [핵심 정리] 리스트 메서드는 리스트 객체에 대해 동작하는 기능으로, append(), count(), index(), pop(), insert(), remove(), reverse(), sort() 등을 이용해 리스트를 조작한다.
127. [핵심 정리] 리스트 슬라이싱은 [시작인덱스:끝인덱스:간격] 형식으로 부분 리스트를 생성하는 방법이며, 시작 위치·끝 위치·간격을 조절하여 다양한 방식의 부분 추출이 가능하다.
128. [핵심 정리] 리스트 컴프리헨션은 반복과 조건을 결합하여 리스트를 간결하게 생성·변환·필터링하는 방법이다.
129. [시험 포인트] 튜플은 괄호보다 콤마가 본질적인 생성 요소이며, 여러 값을 하나로 묶어 반환하거나 여러 변수에 나누어 대입하는 데 활용된다.
130. [시험 포인트] 문자열은 문자로만 이루어진 불변 시퀀스로, 인덱싱, 슬라이싱, 반복 처리, 시퀀스 관련 기능을 적용할 수 있다.
131. [시험 포인트] 튜플은 데이터를 순서 있게 저장하는 불변 시퀀스로, 리스트와 유사하지만 생성 후 원소를 수정할 수 없다.
132. [시험 포인트] 튜플은 인덱싱과 슬라이싱이 가능하지만 sort()나 append()처럼 원소를 바꾸는 메서드는 사용할 수 없으므로, 정렬할 때는 sorted()를 사용하고 원소를 덧붙일 때는 + 연산자를 사용한다.
133. [시험 포인트] 문자열 메서드인 split(), join(), find(), replace(), strip(), upper(), lower(), isdigit(), isalpha(), isalnum() 등을 사용하면 문자열을 다양한 방식으로 처리할 수 있다.
134. [시험 포인트] 세트는 중복을 허용하지 않고 순서를 유지하지 않는 변경 가능 컬렉션으로, 데이터 중복 제거와 집합 연산에 유용하다. 세트는 슬라이싱할 수 없으며, 합집합, 교집합, 차집합, 부분집합과 상위집합 관계를 다룰 수 있다.
135. [시험 포인트] 세트 컴프리헨션은 다른 콜렉션에서 조건에 맞는 값을 추출하거나 변환하면서 중복을 제거하는 데 활용된다.
136. [시험 포인트] 딕셔너리는 key-value 쌍으로 데이터를 저장하는 매핑 자료형이며, 키를 이용해 값에 접근하고 새로운 항목을 추가하거나 기존 값을 수정할 수 있다.
137. [핵심 정리] 튜플은 데이터를 순서 있게 저장하는 불변 시퀀스로, 리스트와 유사하지만 생성 후 원소를 수정할 수 없다.
138. [핵심 정리] 튜플은 괄호보다 콤마가 본질적인 생성 요소이며, 여러 값을 하나로 묶어 반환하거나 여러 변수에 나누어 대입하는 데 활용된다.
139. [핵심 정리] 튜플은 인덱싱과 슬라이싱이 가능하지만 sort()나 append()처럼 원소를 바꾸는 메서드는 사용할 수 없으므로, 정렬할 때는 sorted()를 사용하고 원소를 덧붙일 때는 + 연산자를 사용한다.
140. [핵심 정리] 문자열은 문자로만 이루어진 불변 시퀀스로, 인덱싱, 슬라이싱, 반복 처리, 시퀀스 관련 기능을 적용할 수 있다.
141. [핵심 정리] 문자열 메서드인 split(), join(), find(), replace(), strip(), upper(), lower(), isdigit(), isalpha(), isalnum() 등을 사용하면 문자열을 다양한 방식으로 처리할 수 있다.
142. [핵심 정리] 세트는 중복을 허용하지 않고 순서를 유지하지 않는 변경 가능 컬렉션으로, 데이터 중복 제거와 집합 연산에 유용하다. 세트는 슬라이싱할 수 없으며, 합집합, 교집합, 차집합, 부분집합과 상위집합 관계를 다룰 수 있다.
143. [핵심 정리] 세트 컴프리헨션은 다른 콜렉션에서 조건에 맞는 값을 추출하거나 변환하면서 중복을 제거하는 데 활용된다.
144. [핵심 정리] 딕셔너리는 key-value 쌍으로 데이터를 저장하는 매핑 자료형이며, 키를 이용해 값에 접근하고 새로운 항목을 추가하거나 기존 값을 수정할 수 있다.
145. [핵심 정리] 딕셔너리는 슬라이싱할 수 없지만 keys(), values(), items(), get(), pop(), popitem() 등의 메서드와 딕셔너리 컴프리헨션을 이용하여 효율적으로 데이터를 처리할 수 있다.
146. [시험 포인트] 객체지향 프로그래밍은 객체와 객체의 상호작용을 중심으로 프로그램을 구성하는 방식이며, 프로그램을 유연하고 변경하기 쉽게 만든다.
147. [시험 포인트] 객체지향 패러다임의 대표적 특징에는 추상화, 캡슐화, 상속, 다형성이 있다.
148. [시험 포인트] 객체는 데이터와 연산을 함께 가지는 단위이며, 이러한 객체의 구조와 기능은 클래스에 의해 정의된다.
149. [시험 포인트] 데이터 은닉은 데이터 필드의 직접 변경을 방지하기 위한 방법이며, private 데이터 필드는 앞에 두 개의 밑줄을 붙여 정의한다.
150. [시험 포인트] 클래스는 데이터 필드와 메서드를 포함하며, 객체의 상태를 초기화하기 위해 __init__ 초기자를 사용한다.
151. [시험 포인트] 메서드의 첫 번째 매개변수인 self는 객체 자신을 참조하며, 객체 내부의 데이터 필드와 다른 메서드에 접근할 때 사용된다.
152. [시험 포인트] 클래스의 이름과 초기자의 매개변수를 이용하여 인스턴스를 생성할 수 있으며, 객체와 인스턴스는 같은 의미로 사용된다.
153. [시험 포인트] 생성된 객체는 객체 참조변수를 통해 다루며, 멤버 접근 연산자 .를 사용하여 데이터 필드에 접근하거나 메서드를 호출한다.
154. [핵심 정리] 객체지향 프로그래밍은 객체와 객체의 상호작용을 중심으로 프로그램을 구성하는 방식이며, 프로그램을 유연하고 변경하기 쉽게 만든다.
155. [핵심 정리] 객체지향 패러다임의 대표적 특징에는 추상화, 캡슐화, 상속, 다형성이 있다.
156. [핵심 정리] 객체는 데이터와 연산을 함께 가지는 단위이며, 이러한 객체의 구조와 기능은 클래스에 의해 정의된다.
157. [핵심 정리] 클래스는 데이터 필드와 메서드를 포함하며, 객체의 상태를 초기화하기 위해 __init__ 초기자를 사용한다.
158. [핵심 정리] 메서드의 첫 번째 매개변수인 self는 객체 자신을 참조하며, 객체 내부의 데이터 필드와 다른 메서드에 접근할 때 사용된다.
159. [핵심 정리] 클래스의 이름과 초기자의 매개변수를 이용하여 인스턴스를 생성할 수 있으며, 객체와 인스턴스는 같은 의미로 사용된다.
160. [핵심 정리] 생성된 객체는 객체 참조변수를 통해 다루며, 멤버 접근 연산자 .를 사용하여 데이터 필드에 접근하거나 메서드를 호출한다.
161. [핵심 정리] 클래스 설계는 UML 클래스 다이어그램을 통해 데이터 필드, 생성자, 메서드를 체계적으로 표현할 수 있다.
162. [핵심 정리] 파이썬의 기본 데이터 타입인 int, str 등도 객체이며, type()과 id()를 통해 객체의 타입과 식별 정보를 확인할 수 있다.
163. [핵심 정리] 데이터 은닉은 데이터 필드의 직접 변경을 방지하기 위한 방법이며, private 데이터 필드는 앞에 두 개의 밑줄을 붙여 정의한다.
164. [핵심 정리] private 데이터 필드는 객체 외부에서 직접 접근할 수 없으므로, 접근자와 변경자를 이용해 값을 읽거나 수정한다.
165. [시험 포인트] 네임스페이스는 변수와 함수 등의 이름이 구분되고 관리되는 공간이며, 이름 탐색은 지역, 전역, 빌트인 네임스페이스 순으로 이루어진다.
166. [시험 포인트] math 모듈은 원주율(pi), 올림(ceil), 내림(floor), 제곱근(sqrt) 등 다양한 수학적 계산 기능과 상수를 제공한다.
167. [시험 포인트] random 모듈은 임의의 숫자를 생성하는 난수 발생 기능을 제공하며, randint, choice, shuffle 등의 함수를 통해 게임이나 추첨 프로그램 구현에 활용된다.
168. [시험 포인트] time 모듈은 유닉스 에포크를 기준으로 경과 시간을 측정하거나, 시스템의 현재 시각을 확인하고 프로세스의 실행을 지연(sleep)시키는 기능을 지원한다.
169. [시험 포인트] 모듈은 함수, 상수, 클래스를 모아 놓은 파이썬 파일이며, 패키지는 이러한 모듈들의 폴더 모음, 라이브러리는 가장 큰 집합체이다.
170. [시험 포인트] 외부 모듈을 사용하기 위해서는 import 구문을 사용하여 네임스페이스에 추가해야 하며, from 구문을 통해 특정 멤버만 불러오거나 as 구문으로 별칭을 지정할 수 있다.
171. [핵심 정리] 모듈은 함수, 상수, 클래스를 모아 놓은 파이썬 파일이며, 패키지는 이러한 모듈들의 폴더 모음, 라이브러리는 가장 큰 집합체이다.
172. [핵심 정리] 네임스페이스는 변수와 함수 등의 이름이 구분되고 관리되는 공간이며, 이름 탐색은 지역, 전역, 빌트인 네임스페이스 순으로 이루어진다.
173. [핵심 정리] 외부 모듈을 사용하기 위해서는 import 구문을 사용하여 네임스페이스에 추가해야 하며, from 구문을 통해 특정 멤버만 불러오거나 as 구문으로 별칭을 지정할 수 있다.
174. [핵심 정리] math 모듈은 원주율(pi), 올림(ceil), 내림(floor), 제곱근(sqrt) 등 다양한 수학적 계산 기능과 상수를 제공한다.
175. [핵심 정리] random 모듈은 임의의 숫자를 생성하는 난수 발생 기능을 제공하며, randint, choice, shuffle 등의 함수를 통해 게임이나 추첨 프로그램 구현에 활용된다.
176. [핵심 정리] time 모듈은 유닉스 에포크를 기준으로 경과 시간을 측정하거나, 시스템의 현재 시각을 확인하고 프로세스의 실행을 지연(sleep)시키는 기능을 지원한다.
177. [시험 포인트] 데이터의 영속성을 위해 비휘발성 보조기억장치가 사용되며, 운영체제는 계층적 트리 구조의 파일 시스템을 통해 데이터를 논리적 단위인 '파일'로 관리한다.
178. [시험 포인트] 파일은 데이터의 성격에 따라 텍스트 파일(특정 인코딩 규칙에 따른 문자열)과 바이너리 파일(이미지, 실행 파일 등 이진 데이터)로 구분된다.
179. [시험 포인트] 기가바이트 단위의 대용량 데이터는 한 줄씩 읽고 처리하는 방식으로 메모리 오버플로우를 예방해야 하며, 정확한 비교 연산 및 처리를 위해 strip() 등을 활용하여 개행 문자와 공백을 제거하는 데이터 파싱 작업이 수행된다.
180. [시험 포인트] 파일의 위치를 지정할 때는 최상위 루트를 기준으로 하는 절대 경로와, 현재 작업 중인 디렉토리를 기준으로 하는 상대 경로 방식을 사용한다.
181. [시험 포인트] 파일 입출력은 '열기(open) → 접근(read/write) → 닫기(close)'의 3단계를 거치며, 데이터 무결성 보장 및 시스템 자원 누수 방지를 위해 작업 후 반드시 파일 객체를 닫아주어야(close) 한다.
182. [시험 포인트] 새로운 파일을 생성하거나 덮어쓸 때는 "w" 모드를, 기존 데이터 끝에 내용을 덧붙일 때는 "a" 모드를 사용한다.
183. [시험 포인트] 파일에서 데이터를 가져올 때는 파일 포인터를 이동하며 특정 개수만큼 읽는 read( ), 줄 단위로 읽는 readline( ), 전체 줄을 리스트로 반환하는 readlines( ) 등을 사용한다.
184. [핵심 정리] 데이터의 영속성을 위해 비휘발성 보조기억장치가 사용되며, 운영체제는 계층적 트리 구조의 파일 시스템을 통해 데이터를 논리적 단위인 '파일'로 관리한다.
185. [핵심 정리] 파일의 위치를 지정할 때는 최상위 루트를 기준으로 하는 절대 경로와, 현재 작업 중인 디렉토리를 기준으로 하는 상대 경로 방식을 사용한다.
186. [핵심 정리] 파일은 데이터의 성격에 따라 텍스트 파일(특정 인코딩 규칙에 따른 문자열)과 바이너리 파일(이미지, 실행 파일 등 이진 데이터)로 구분된다.
187. [핵심 정리] 파일 입출력은 '열기(open) → 접근(read/write) → 닫기(close)'의 3단계를 거치며, 데이터 무결성 보장 및 시스템 자원 누수 방지를 위해 작업 후 반드시 파일 객체를 닫아주어야(close) 한다.
188. [핵심 정리] 새로운 파일을 생성하거나 덮어쓸 때는 "w" 모드를, 기존 데이터 끝에 내용을 덧붙일 때는 "a" 모드를 사용한다.
189. [핵심 정리] 파일에서 데이터를 가져올 때는 파일 포인터를 이동하며 특정 개수만큼 읽는 read( ), 줄 단위로 읽는 readline( ), 전체 줄을 리스트로 반환하는 readlines( ) 등을 사용한다.
190. [핵심 정리] 기가바이트 단위의 대용량 데이터는 한 줄씩 읽고 처리하는 방식으로 메모리 오버플로우를 예방해야 하며, 정확한 비교 연산 및 처리를 위해 strip() 등을 활용하여 개행 문자와 공백을 제거하는 데이터 파싱 작업이 수행된다.
191. [시험 포인트] 데이터 분석의 표준 파이프라인은 데이터를 읽어오는 수집, 분석하기 쉽게 다듬는 전처리, 패턴을 추출하는 분석, 직관적으로 표현하는 시각화 단계로 구성된다.
192. [시험 포인트] 데이터 분석은 원시 데이터에서 유의미한 패턴을 찾고 정보로 변환하는 실질적인 문제 해결 작업과정이다.
193. [시험 포인트] 분석 결과의 품질은 데이터의 품질에 직접적으로 비례한다는 GIGO 원칙에 따라, 텍스트 분석 시 불용어를 필터링하는 전처리 과정이 매우 중요하다.
194. [시험 포인트] 문자열 정제를 위해 모든 문자를 소문자로 통일하는 lower(), 불필요한 문자를 치환하는 replace(), 공백 등을 기준으로 텍스트를 분리하는 split() 등의 내장 메서드를 적절히 조합하여 활용한다.
195. [시험 포인트] 프로그램이 실행 중인 현재 작업 위치를 확인하고 외부 텍스트 파일에 접근하기 위해 os 모듈을 활용할 수 있다.
196. [시험 포인트] 핵심 키워드의 출현 빈도는 Key와 Value 구조를 가진 딕셔너리를 통해 기록하며, get() 메서드로 단어의 등장 횟수를 안전하게 누적할 수 있다.
197. [시험 포인트] 분석이 완료된 딕셔너리 형태의 데이터는 가장 많이 등장한 순서대로 순위를 시각화하기 위해 값(빈도수)을 기준으로 정렬 가능한 리스트 형태로 자료구조를 변환해야 한다.
198. [핵심 정리] 데이터 분석은 원시 데이터에서 유의미한 패턴을 찾고 정보로 변환하는 실질적인 문제 해결 작업과정이다.
199. [핵심 정리] 데이터 분석의 표준 파이프라인은 데이터를 읽어오는 수집, 분석하기 쉽게 다듬는 전처리, 패턴을 추출하는 분석, 직관적으로 표현하는 시각화 단계로 구성된다.
200. [핵심 정리] 분석 결과의 품질은 데이터의 품질에 직접적으로 비례한다는 GIGO 원칙에 따라, 텍스트 분석 시 불용어를 필터링하는 전처리 과정이 매우 중요하다.
201. [핵심 정리] 문자열 정제를 위해 모든 문자를 소문자로 통일하는 lower(), 불필요한 문자를 치환하는 replace(), 공백 등을 기준으로 텍스트를 분리하는 split() 등의 내장 메서드를 적절히 조합하여 활용한다.
202. [핵심 정리] 프로그램이 실행 중인 현재 작업 위치를 확인하고 외부 텍스트 파일에 접근하기 위해 os 모듈을 활용할 수 있다.
203. [핵심 정리] 핵심 키워드의 출현 빈도는 Key와 Value 구조를 가진 딕셔너리를 통해 기록하며, get() 메서드로 단어의 등장 횟수를 안전하게 누적할 수 있다.
204. [핵심 정리] 분석이 완료된 딕셔너리 형태의 데이터는 가장 많이 등장한 순서대로 순위를 시각화하기 위해 값(빈도수)을 기준으로 정렬 가능한 리스트 형태로 자료구조를 변환해야 한다.

##### 정답 감각 리스트

1. 기계를 바꾸지 않고 처리 방법의 교체만으로 무엇이든 할 수 있도록 다음과 같이 입력, 기억 ·저장, 제어, 연산, 출력 장치로 역할을 구분한 구조는? → 1
2. ‘방송통신대학교 컴퓨터과학과의 파이썬 프로그래밍 기초는 전공 과목이다’라고 할 때, ‘전공’이라는 설명은 어떤 유형의 데이터에 속하는가? → 2
3. 해석의 모호성을 제거하고 자연어와 유사한 형태의 문법을 처음으로 도입한 프로그래밍 언어는? → 3
4. 다음 중 파이썬의 장점이라고 할 수 없는 것은? → 2
5. 파이썬 프로그램이 플랫폼 독립적으로 실행될 수 있도록 아래 그림의 (가)와 같이 파이썬 인터프리터가 동작하는 과정에서 중간 언어로 만드는 것은? → 1
6. 파이썬 프로그래밍 환경 중 오픈소스이며 문서화가 쉽고 전통적인 소스코드-컴파일-실행 방식에서 벗어나 웹 기반 대화형 개발 및 실행 환경을 제공하는 것은? → 3
7. 'Our goal is to predict the future' → 2
8. 다음 중 식별자로 사용할 수 없는 것은? → 3
9. print(detox) → 1
10. 특정 영역 내의 명령문에 대한 실행 여부를 프로그램 실행 과정 중 조건에 따라 결정하는 구조 → 1
11. 올바른 명령문은? (단, n은 number 변수에 저장된 정수를 의미한다.) → 4
12. ‘논리적으로 실행 불가능한 명령문 작성 시 발생’ 오류를 무엇이라고 하는가? → 2
13. 파이썬에서 명령어의 논리적 집합인 명령 블록을 표현하기 위한 방법은? → 2
14. 다음은 온도(temp)가 0 이하일 경우, “겨울입니다.”를 출력하는 프로그램일 일부이다. 밑줄 친 빈 칸에 들어갈 명령문은? → 4
15. 다음은 사용자가 입력한 수가 3과 5의 공배수일 경우 “3과 5의 공배수입니다”를, 아닐 경우 “3과 5의 공배수가 아닙니다.”를 출력하는 프로그램의 일부이다. 밑줄 친 빈 칸에 들어갈 연산자는? → 1
16. 계수 제어 반복 구조에서 특정 반복 횟수와 반복 시 계수의 값을 정하기 위해 사용하는 데이터 타입은? → 3
17. 다음과 같이 반복 구조 내부에 또 다른 반복 구조가 포함되어 실행 흐름을 만드는 구조를 무엇이라고 하는가? → 4
18. 다음 코드의 출력값으로 옳은 것은? → 4
19. 다음 중 반환값에 따른 함수의 종류를 구분할 때, 나머지와 다른 하나는? → 1
20. print(season) → 2
21. 다음 코드의 실행 결과는? → 1
22. 다음 중 key-value의 쌍으로 데이터를 저장하는 매핑 자료형은 무엇인가? → 4
23. print(hei_list[:3]) → 1
24. print(hei_even) → 2
25. 다음 중 튜플에 대한 설명으로 옳은 것은? → 4
26. print(msg.split(" ")) → 2
27. 다음 중 세트에 사용할 수 없는 연산자는 무엇인가? → 3
28. 객체의 상태를 초기화하기 위해 사용하는 특수 메소드는 무엇인가? → 2
29. print(unitcone.h) → 4
30. 다음 중 접근자와 변경자에 대한 설명으로 옳은 것은? → 2
31. 파이썬에서 관련된 함수, 변수, 클래스 등을 하나의 파일로 묶어 재사용성과 유지보수성을 높이는 기본 단위는 무엇인가? → 2
32. 특정 모듈(math)에서 특정 함수(sqrt)만을 가져오기 위한 올바른 구문 형식은? → 3
33. 코드의 실행을 잠시 3초 동안 멈추고자 할 때 사용하는 time 모듈의 함수는? → 4
34. 현재 작업 디렉토리의 위치가 /var/home/user/docs라고 가정하자. 부모 디렉토리에 있는 lib 폴더 내의 config.txt 파일을 상대 경로로 열고자 할 때, 가장 적절한 경로는? → 3
35. 프로그램이 데이터가 누적되도록 파일을 다루고자 할 때, open() 함수에 지정해야 할 가장 적절한 모드는? → 3
36. 대용량의 파일 분석 프로그램 작성 시, 다음 중 데이터를 안전하게 처리할 수 있는 방법은? → 3
37. 원시 데이터가 가치 있는 정보로 변환되기까지 거치는 표준화된 흐름을 뜻하는 것은? → 1
38. 텍스트 분석 시 문법적으로는 쓰이나 분석적 의미가 없어 제거하는 단어(예: a, the, of)를 무엇이라 하는가? → 1
39. 단어와 빈도수를 쌍으로 묶어 카운팅 데이터를 저장하고 관리하기에 가장 적합한 자료구조는? → 4

#### 데이터정보처리입문

##### 암기 리스트

1. [핵심 목표] 지식정보화사회에서 컴퓨터의 활용에 대하여 이해할 수 있다.
2. [핵심 목표] 데이터분석 절차를 설명할 수 있다.
3. [핵심 목표] 데이터분석 소프트웨어들을 설명할 수 있다.
4. [핵심 목표] 엑셀과 R을 이용한 데이터분석 예를 설명할 수 있다.
5. [핵심 목표] 데이터의 정의와 분석 목적을 설명할 수 있다.
6. [핵심 목표] 측정의 수준과 데이터의 종류에 대해서 설명할 수 있다.
7. [핵심 목표] 데이터의 입력과정을 설명할 수 있다.
8. [핵심 목표] 입력된 데이터에 대한 점검과정을 설명할 수 있다.
9. [핵심 목표] 데이터(data) : 어떤 관심 있는 주제에 대해서 수집된 구조화된 정보(organized information)를 말함. 넓은 의미의 데이터는 수치, 문자, 그림 등의 표현양식과 관계없이 어떤 주제에 대한 구조화된 정보를 뜻함
10. [핵심 목표] 케이스(case) : 특정 조사단위로부터 얻어진 정보의 집합체를 말함
11. [핵심 목표] 변수(variable) : 각 조사단위로부터 측정된 개별적인 속성들
12. [핵심 목표] 인터넷을 이용하여 국가통계정보를 검색할 수 있다.
13. [핵심 목표] 데이터제공 사이트를 찾아볼 수 있다.
14. [핵심 목표] 기술통계량을 이해할 수 있다.
15. [핵심 목표] 연속인 자료에 이용되는 통계그래프를 설명할 수 있다.
16. [핵심 목표] 웹을 활용하여 기술통계량을 구하고, 통계그래프를 그려볼 수 있다.
17. [핵심 목표] 한글 2018의 기초 사용법을 이해할 수 있다.
18. [핵심 목표] 한글 2018을 이용한 문서 작성 방법을 설명할 수 있다.
19. [핵심 목표] 한글 2018을 이용한 표 작성 방법을 이해할 수 있다.
20. [핵심 목표] 한글 수식편집기 사용 방법을 설명할 수 있다.
21. [핵심 목표] 서식 도구 모음 : 문서를 작성하면서 글꼴이나 글자크기, 글자 모양, 문단모양, 또는 스타일을 지정할 수 있는 기능을 모아놓은 것
22. [핵심 목표] 문단 정렬 방식 : 문단을 구성하는 단어들을 문단의 폭에 맞게 배치하는 방법으로 양쪽 정렬, 왼쪽 정렬, 오른쪽 정렬, 가운데 정렬, 배분 정렬, 나눔 정렬 중에 하나를 선택할 수 있음
23. [핵심 목표] 다단 편집 : 신문이나 잡지 등에서 단행본과는 달리 하나의 쪽을 세로 방향으로 공간을 나누어 각 공간마다 글을 배치한 것
24. [핵심 목표] 엑셀의 주요 기능을 제시할 수 있다.
25. [핵심 목표] 엑셀의 기본 용어를 설명할 수 있다.
26. [핵심 목표] 데이터의 입력과정을 이해할 수 있다.
27. [핵심 목표] 데이터의 이동과 복사, 저장과 출력 방법을 설명할 수 있다.
28. [핵심 목표] 케이스(case) : 특정 조사단위로부터 얻어진 정보의 집합체를 말함
29. [핵심 목표] 변수(variable) : 각 조사단위로부터 측정된 개별적인 속성들
30. [핵심 목표] 셀(cell) : 워크시트에서 데이터가 입력되는 열과 행이 만나는 지점을 말함. 각 셀은 A1(A와 1이 만나는 위치라는 뜻)처럼 셀이 속한 행과 열의 위치에 따라서 셀주소를 갖음
31. [핵심 목표] 엑셀의 연산자를 활용할 수 있다.
32. [핵심 목표] 엑셀 함수의 기능과 형식을 설명할 수 있다.
33. [핵심 목표] 엑셀 함수를 사용하여 계산할 수 있다.
34. [핵심 목표] 엑셀을 활용하여 데이터를 분석할 수 있다.
35. [핵심 목표] 케이스(case) : 특정 조사단위로부터 얻어진 정보의 집합체를 말함
36. [핵심 목표] 변수(variable) : 각 조사단위로부터 측정된 개별적인 속성들
37. [핵심 목표] 상대참조 : 행 이름이나 열 이름만을 사용하여 셀을 참조하는 방법(예: A1, B2, A1:F1 등)
38. [핵심 목표] 그래프의 역할을 설명할 수 있다.
39. [핵심 목표] 원 그래프와 사각형 그래프를 작성할 수 있다.
40. [핵심 목표] 막대 그래프를 작성할 수 있다.
41. [핵심 목표] 꺾은선 그래프를 이용하여 데이터 분석을 할 수 있다.
42. [핵심 목표] 정성적 데이터(qualitative data) : 명목형 척도나 순서형 척도로 측정된 데이터
43. [핵심 목표] 정량적 데이터(quantitative data) : 구간척도나 비율척도로 측정된 연속형 데이터
44. [핵심 목표] 원 그래프(pie chart) : 각 항목별 비율을 나타낼 때, 각 항목의 비율과 원 그래프에서 각 항목을 나타내는 면적의 비율이 같게 만든 그래프
45. [핵심 목표] 도수분포를 작성할 수 있다.
46. [핵심 목표] 엑셀을 이용하여 히스토그램을 작성할 수 있다.
47. [핵심 목표] 산점도를 작성하여 두 변수의 연관성을 설명할 수 있다.
48. [핵심 목표] 상관분석의 개념과 분석 결과를 해석할 수 있다.
49. [핵심 목표] 도수분포표(frequency table) : 겹치지 않는 몇 개의 범주 즉, 계급에 속한 관측치의 개수를 요약하여 작성한 표. 정성적 데이터뿐만 아니라 정량적 데이터에도 적용할 수 있음
50. [핵심 목표] 히스토그램(histogram) : 연속형 데이터에 대해서 데이터의 특징이나 분포 모양을 살펴볼 때 널리 이용되는 그래프. 수평축 위에 계급구간을 표시하고 그 위로 각 계급의 상대도수에 비례하는 넓이의 직사각형을 그린 것
51. [핵심 목표] 산점도(scatter plot) : 두 연속형 변수 사의 관계를 그래프를 통해서 살펴보고자 할 때 사용되는 그래프. 두 변수 사이의 연관성 정도를 살펴보기 위한 분석의 첫 단계임
52. [핵심 목표] 엑셀 함수의 기능을 이용할 수 있다.
53. [핵심 목표] 엑셀 함수의 기능을 활용하여 실제 문제를 해결할 수 있다.
54. [핵심 목표] 날짜/시간 함수를 이용할 수 있다.
55. [핵심 목표] 텍스트 함수 기능을 활용하여 실제 문제에 적용할 수 있다.
56. [핵심 목표] 채우기 기능 : 연속된 숫자나 일정한 규칙을 갖고 변화하는 숫자들을 쉽게 입력할 수 있게 하는 엑셀 기능을 말함
57. [핵심 목표] SUMPRODUCT 함수 : 두 배열의 대응되는 값끼리 곱해서 합을 구해 주는 기능
58. [핵심 목표] DATE(year, month, day) 함수 : 특정 날짜의 일련번호를 구해주는 엑셀 날짜/시간 함수 중 하나
59. [핵심 목표] 엑셀 함수 기능을 이용하여 원리합계를 계산할 수 있다.
60. [핵심 목표] 엑셀 차트 기능을 이용하여 주어진 함수의 그래프를 그릴 수 있다.
61. [핵심 목표] 목표값 찾기 기능을 활용할 수 있다.
62. [핵심 목표] 엑셀을 활용하여 적분값을 계산할 수 있다.
63. [핵심 목표] 상대참조 : 행 이름이나 열 이름만을 사용하여 셀을 참조하는 방법(예: A1, B2, A1:F1 등)
64. [핵심 목표] 절대참조 : 참조하는 셀이나 셀 범위를 표시할 때 “$”를 사용하여 참조하는 방법(예: $A$1, $B$2, $A$1:$F$1 등)
65. [핵심 목표] 혼합참조 : 행 이름이나 열 이름의 한 쪽에만 “$”를 사용하여 참조하는 방법(예: $A1, B$2, A$1:F$1 등)
66. [핵심 목표] R 사용법을 익힐 수 있다.
67. [핵심 목표] 데이터를 읽어 들이는 방법을 이해할 수 있다.
68. [핵심 목표] 행렬연산 방법을 설명할 수 있다.
69. [핵심 목표] 기술통계량을 구할 수 있다.
70. [핵심 목표] 통계그래프를 그릴 수 있다.
71. [핵심 목표] Function 문 사용법을 설명할 수 있다.
72. [핵심 목표] 정규분포를 이해할 수 있다.
73. [핵심 목표] 이산형 그래프를 그릴 수 있다.
74. [핵심 목표] 연속형 그래프를 그릴 수 있다.
75. [핵심 목표] 파이썬을 설치하는 방법을 이해할 수 있다.
76. [핵심 목표] Spyder 및 Jupyter Notebook을 사용할 줄 안다.
77. [핵심 목표] 파이썬 언어를 사용할 수 있다.
78. [핵심 목표] 파이썬을 이용하여 데이터를 읽을 수 있다.
79. [핵심 목표] 파이썬을 이용하여 기술통계량을 구할 수 있다.
80. [핵심 목표] 파이썬을 이용하여 통계그래프를 그릴 수 있다.

##### 정답 감각 리스트

1. 일반적인 데이터분석 절차이다. ( )안에 순서대로 가장 적합한 것은? → 4
2. 컴퓨터를 통하여 데이터 정보를 효율적으로 처리하기 위해 요구되는 사항이 아닌 것은? → 2
3. 다음 중 범용 통계패키지에 대한 설명으로 잘못된 것은? → 3
4. 오늘날과 같이 효율적인 정보의 수집, 수집된 정보의 가치판단, 정보활용 능력이 필수적으로 요구되는 사회를 가장 잘 나타낸 말은? → 3
5. R 통계패키지를 다운받고자 한다. R 공식 사이트는 ? → 2
6. 설문문항에서 명목척도에 해당하는 것은? → 3
7. 주어진 설문지를 이용하여 100명의 학생들에게 응답을 얻었다. 데이터의 구성에 대한 올바른 설명은? → 2
8. 다음의 데이터에 대한 설명 중 올바른 것끼리 짝지어진 것은? → 1
9. 다음 측정 수준에 대한 설명 중 옳지 않은 것은? → 3
10. 데이터를 입력한 후, 제대로 입력되었는지 점검하고자 한다. 데이터의 오류를 점검하기 위한 방법이 아닌 것은? → 4
11. 통계·데이터과학과에 재학 중인 학생들을 대상으로 통계조사를 실시하였다. 다음에 제시된 변수를 조사하였는데, 이 중 명목척도로 측정된 변수는 몇 개인가? → 3
12. 한국사회과학데이터센터 사이트는? → 1
13. 국가경제에 관한 통계들을 제공하는 한국은행 경제시스템 사이트는? → 2
14. 통계청 사이트는? → 3
15. 다음 중 카네기 멜론 통계학과에서 운영하는 사이트로서 다양한 데이터를 제공하는 사이트는? → 3
16. NIST(National Institute of Standards and Technology)에서 여러 통계 분석 등과 관련하여 유용한 데이터를 제공하는 사이트는? → 4
17. 다음 중 표본분산을 구하는 공식은? → 4
18. 자료의 개수나 측정단위가 다른 두 개 이상의 자료에 대한 표본집단 간의 상대적인 산포를 비교할 때 이용되는 측도로, 두 집단의 단위가 다르거나, 단위는 같지만 평균의 차이가 클 때 두 그룹의 산포를 비교하는 데 유용하게 이용되는 측도는? → 4
19. 다음 중 표본평균을 구하는 공식은? → 3
20. 조사된 자료가 다음과 같다. 중앙값은? → 3
21. 탐색적 자료분석의 관점에서 살펴볼 때 한 묶음의 자료를 정리하는 숫자로서 다섯숫자요약이란 다음 중 무엇인가? → 3
22. 다음 괄호 안에 알맞은 용어로 짝 지어진 것은? → 1
23. 수식편집창의 다음 ( ) 안의 ㉠, ㉡, ㉢에 적합한 것은? → 2
24. 수식편집창의 다음 ( ) 안의 a, b 에 적합한 것은? → 3
25. 다음은 한글 2018 사용법에 대한 설명이다. 옳은 설명을 모두 고른 것은? → 3
26. 다음은 한글 2018 사용법에서 한자 변환에 대한 설명이다. 옳은 설명을 모두 고른 것은? → 2
27. 다음은 엑셀에 대한 함수 사용방법에 대한 설명이다. 옳은 설명끼리 짝지어진 것은? → 1
28. 다음은 엑셀의 통합문서(workbook)와 워크시트(worksheet)에 대한 내용이다. 잘못된 것은? → 4
29. 선택된 셀의 정보를 보여주고, 데이터를 입력하거나 수정할 수 있으며, 계산을 위해서 수식을 입력할 수 있는 부분을 무엇이라 하는가? → 4
30. 데이터를 입력한 후 이웃하지 않은 셀들을 선택하고자 한다. 올바른 방법은? → 2
31. 다음 중 엑셀의 중요한 기능이라고 할 수 없는 것은? → 3
32. 엑셀에서 B2셀의 값이 60보다 작으면 “불합격”, 60이상이면 “합격”을 D2셀에 표시하고자 한다. 적합한 엑셀함수 사용은? → 4
33. B2와 C2 셀에 데이터를 입력한 후 두 값의 평균점수를 구하고자 한다. 적합한 수식입력은? → 2
34. 데이터를 입력한 후 그림과 같이 각 케이스에 대하여 평균점수를 구하고자 한다. (A) 부분에 적합한 수식입력은? → 2
35. D2 셀에 =IF(SUM(B2:C2)>=130, "합격“, ”불합격“)를 입력하였을 때 그 결과는? → 3
36. D2 셀에 =$B$2+$C$2를 입력한 후 드래그&드롭 기능을 이용하여 D11 셀까지 채워 넣었다. 이 때 D3 셀의 결과는? → 1
37. 다음과 같은 성별 도수분포를 그래프로 나타내고자 한다. 적합한 그래프끼리 묶인 것은? → 1
38. 통계청 홈페이지를 방문하여 최근 5년간의 매월 소비자 물가지수 데이터를 구하여 적합한 그래프를 엑셀로 그려서 전체적인 경향을 파악하고자 한다. 가장 알맞은 차트의 종류는 무엇인가? → 4
39. 시간의 흐름에 따라 관측된 시계열 데이터의 추세 변화나 경향을 파악하고자 한다. 다음 중 이와 같은 분석 목적에 가장 적합하지 않은 그래프는? → 2
40. 사람들이 즐겨보는 TV 프로그램에 대한 비율을 다음과 같이 조사하였다. 이러한 경우 전체에 대한 항목별 비율을 비교하거나, 특정 항목을 강조하기 하기 위해 가장 적합한 그래프는? → 3
41. 엑셀을 이용하여 간격이 일정한 시계열 데이터의 경향을 파악하고자 한다. 엑셀 차트 중에서 가장 적합한 차트 유형은? → 4
42. 엑셀 함수를 이용하여 구한 시간의 흐름에 따른 전압 값을 다음과 같은 그래프로 표시하였다. 이와 같은 그래프를 얻기 위해서 사용된 엑셀 차트의 유형은 무엇인가? → 4
43. 두 연속인 변수 X와 Y 사이의 상관계수를 구했더니 0.9가 나왔다. 올바른 해석은? → 1
44. 두 변수들 사이의 관계를 살펴보기 위해 산점도를 그리고자 한다. 차트마법사의 차트 종류 입력상자에서 선택할 차트 종류는? → 3
45. 도수분포표에 대한 다음 설명 중 옳지 않은 것은? → 4
46. 다음의 산점도에 대한 상관계수로 가장 적합할 것으로 생각되는 값은? → 3
47. A2 셀에 “대한민국”이라고 입력되어 있다. "=MID(A2, 2, 2)"의 결과로 알맞은 것은? → 3
48. G2 셀에 어떤 사람의 생년월일이 “1980-12-5”로 같이 입력되어 있다고 한다. 엑셀 함수 “=YEAR(NOW())-YEAR(G2))"의 결과를 바르게 설명한 것은? → 2
49. 학과코드의 첫 번째 자리 알파벳이 A이면 통계·데이터과학과, B이면 컴퓨터학과 학생이다. 학과 셀에 학과를 표기하고자 한다. C2 셀에 알맞은 함수식은? → 3
50. 최종 점수는 시험점수와 과제물, 태도점수의 합계에서 결석일수에 5를 곱한 수를 뺀 것으로 계산하고자 한다. H2 셀에 알맞은 함수식은? → 3
51. 다음과 같이 워크시트에 값이 입력되어 있을 때 ‘=SUMPRODUCT(A1:A3, B1:B3)’을 D1 셀에 입력하면 얻게 되는 계산값은 얼마인가? → 1
52. 문제 내용 확인 필요 → 4
53. 다음과 같이 셀 C7에 입력되어 있는 수식을 드래그 & 드롭으로 C11까지 채워서 1월부터 6월까지의 미달러($) 기준의 수출액을 원화(￦) 기준으로 바꾸고자 한다. 셀 C6에 입력할 수식으로 맞는 것은? → 3
54. 문제 내용 확인 필요 → 3
55. 지문 → 2
56. 데이터 객체 ex.data의 변수들을 직접 사용하고자 한다. 유용한 명령은 ? → 4
57. 그룹변수인 성별(sex)의 값에 따라 변수 salary의 평균(mean)을 구하는 명령은 ? → 3
58. > ( b )(ex.data) → 1
59. ex.data = ( a )("c:/data/example.txt", header=T) → 2
60. package xlsx를 인스톨하였다. 이를 가동시키기 위한 명령은 ? → 3
61. 다음 R 명령 수행결과는 ? → 4
62. 다음 R 명령 수행결과는 ? → 1
63. 다음 R 명령 수행결과는 ? → 3
64. 상자그림을 그리는 명령은 ? → 2
65. 파이썬에서 딕셔너리로 선언하고자 한다. 적합한 것은 ? → 2
66. 주관식 정답 입력칸 → pip
67. 주관식 정답 입력칸 → www.anaconda.com
68. 주관식 정답 입력칸 → 5
69. 주관식 정답 입력칸 → Hello Jeong !
70. 주관식 정답 입력칸 → plt.bar
71. 주관식 정답 입력칸 → pd.read_csv
72. 주관식 정답 입력칸 → nex8.describe()
73. 주관식 정답 입력칸 → nex8.iloc[:, 1:]
74. 주관식 정답 입력칸 → pd.read_excel

#### 유비쿼터스컴퓨팅개론

##### 암기 리스트

1. [시험 포인트] 유비쿼터스컴퓨팅 개념을 토대로 하는 접근 방법은
2. [시험 포인트] ii) 다양한 컴퓨터(칩, 센서, 태그, 구동체등)를 사물 속에 심고 이들을 네트워크로 연결하여 물리공간의 상황을 사물과 사물, 사물과 컴퓨터, 컴퓨터와 사물 간의 상호작용을 통해 유비쿼터스공간(전자공간-물리공간)으로 인식하는 방법이 있다.
3. [시험 포인트] 유비쿼터스 환경은 언제, 어디서나, 어떠한 통신 단말기를 가지고, 어느 정보통신망을 통해서든, 원하는 정보통신 서비스를 받을 수 있는 환경을 의미한다.
4. [시험 포인트] 유비쿼터스 혁명은 서로 이질적인 물리공간(제1공간)과 전자공간(제2공간)이 제3의 가치공간인 유비쿼터스공간으로 통합/진화되는 사회 전반에 걸친 변화이다.
5. [시험 포인트] i) 물리공간의 사물 이미지를 해석하여 사용자의 주변을 분석하고 주위의 특정 사물의 존재를 컴퓨터가 인식할 수 있도록 하거나,
6. [시험 포인트] 유비쿼터스 공간은 만지지 않아도 공간에 존재하거나 공간에 관한 정보를 사용자가 알 수 있는 ‘현실체가 지능적으로 증강된 공간’으로 인식된다.
7. [핵심 정리] 유비쿼터스 환경은 언제, 어디서나, 어떠한 통신 단말기를 가지고, 어느 정보통신망을 통해서든, 원하는 정보통신 서비스를 받을 수 있는 환경을 의미한다.
8. [핵심 정리] 유비쿼터스 혁명은 서로 이질적인 물리공간(제1공간)과 전자공간(제2공간)이 제3의 가치공간인 유비쿼터스공간으로 통합/진화되는 사회 전반에 걸친 변화이다.
9. [핵심 정리] 유비쿼터스컴퓨팅 개념을 토대로 하는 접근 방법은
10. [핵심 정리] i) 물리공간의 사물 이미지를 해석하여 사용자의 주변을 분석하고 주위의 특정 사물의 존재를 컴퓨터가 인식할 수 있도록 하거나,
11. [핵심 정리] ii) 다양한 컴퓨터(칩, 센서, 태그, 구동체등)를 사물 속에 심고 이들을 네트워크로 연결하여 물리공간의 상황을 사물과 사물, 사물과 컴퓨터, 컴퓨터와 사물 간의 상호작용을 통해 유비쿼터스공간(전자공간-물리공간)으로 인식하는 방법이 있다.
12. [핵심 정리] 유비쿼터스 공간은 만지지 않아도 공간에 존재하거나 공간에 관한 정보를 사용자가 알 수 있는 ‘현실체가 지능적으로 증강된 공간’으로 인식된다.
13. [시험 포인트] 유비쿼터스 컴퓨팅은 사람을 포함한 현실공간에 존재하는 모든 대상물들을 기능적·공간적으로 연결하여 사용자에게 필요한 정보나 서비스를 즉시에 제공할 수 있는 기반 기술이다. 즉, 사물의 일부가 된 컴퓨터들은 주변 상황을 인식할 수 있고, 지리적으로 떨어진 곳에서도 사용자가대상 사물과 그 주변 환경의 변화를 지각하거나 추적할 수 있도록 한다.
14. [시험 포인트] 유비쿼터스 컴퓨팅의 특징은 다음과 같다.
15. [시험 포인트] 유비쿼터스 컴퓨팅의 개념적 기술전개 방향은 다음과 같다.
16. [핵심 정리] 유비쿼터스 컴퓨팅은 사람을 포함한 현실공간에 존재하는 모든 대상물들을 기능적·공간적으로 연결하여 사용자에게 필요한 정보나 서비스를 즉시에 제공할 수 있는 기반 기술이다. 즉, 사물의 일부가 된 컴퓨터들은 주변 상황을 인식할 수 있고, 지리적으로 떨어진 곳에서도 사용자가대상 사물과 그 주변 환경의 변화를 지각하거나 추적할 수 있도록 한다.
17. [핵심 정리] 유비쿼터스 컴퓨팅의 특징은 다음과 같다.
18. [핵심 정리] 유비쿼터스 컴퓨팅의 개념적 기술전개 방향은 다음과 같다.
19. [핵심 정리] 유비쿼터스 컴퓨팅 기술 이슈는 다음과 같다.
20. [시험 포인트] 가상성의 연속성(Virtuality Continuum)은 현실과 가상세계 사이에서 가상성의 정도를 구분하기 위해 제시된 개념이다.
21. [시험 포인트] 가상현실은 실제로 존재하지 않는 완전히 허구적 환경이나 상황을 가능하게 하는 기술이다.
22. [시험 포인트] 증강현실은 현실에 가상정보를 추가하는 형태이다.
23. [시험 포인트] 혼합현실은 현실과 가상환경 사이에 존재하는 모든 것을 의미하며, 현실세계와 가상의 대상물이 함께 존재하는 것을 의미한다.
24. [시험 포인트] 가상현실 콘텐츠 관련 기술 :
25. [시험 포인트] ① 가상현실 콘텐츠를 개발∙실행하기 위한 개발 엔진
26. [시험 포인트] ② 가상현실 콘텐츠를 개발∙제작하기 위한 저작도구
27. [시험 포인트] ③ 가상현실 콘텐츠를 사용자에게 제공하는 서비스 기술
28. [핵심 정리] 가상현실은 실제로 존재하지 않는 완전히 허구적 환경이나 상황을 가능하게 하는 기술이다.
29. [핵심 정리] 증강현실은 현실에 가상정보를 추가하는 형태이다.
30. [핵심 정리] 가상성의 연속성(Virtuality Continuum)은 현실과 가상세계 사이에서 가상성의 정도를 구분하기 위해 제시된 개념이다.
31. [핵심 정리] 혼합현실은 현실과 가상환경 사이에 존재하는 모든 것을 의미하며, 현실세계와 가상의 대상물이 함께 존재하는 것을 의미한다.
32. [핵심 정리] 가상현실 콘텐츠 관련 기술 :
33. [핵심 정리] ① 가상현실 콘텐츠를 개발∙실행하기 위한 개발 엔진
34. [핵심 정리] ② 가상현실 콘텐츠를 개발∙제작하기 위한 저작도구
35. [핵심 정리] ③ 가상현실 콘텐츠를 사용자에게 제공하는 서비스 기술
36. [핵심 정리] 메타버스는 ‘가상, 추상’을 의미하는 메타(Meta)와 ‘현실세계’를 의미하는 유니버스(Universe)의 합성어이다.
37. [시험 포인트] 칩 자체가 하나의 시스템으로 기능할 수 있도록정보통신기기의핵심기능을담당하는메모리, 디지털 회로, 아날로그 회로, CPU, 센서, 안테나, 수동소자 등을 하나의 반도체 칩에 집적하는 기술
38. [시험 포인트] SoC의 장점 :
39. [시험 포인트] 유비쿼터스 네트워크나 초소형 휴먼 인터페이스 분야의 핵심요소인 3차원 미소 구조물, 센서 및 구동 장치 등을 소형화 및 고정밀화하고 복합화를 가능
40. [시험 포인트] SoC 기술 :
41. [핵심 정리] SoC 기술 :
42. [핵심 정리] 칩 자체가 하나의 시스템으로 기능할 수 있도록정보통신기기의핵심기능을담당하는메모리, 디지털 회로, 아날로그 회로, CPU, 센서, 안테나, 수동소자 등을 하나의 반도체 칩에 집적하는 기술
43. [핵심 정리] SoC의 장점 :
44. [핵심 정리] 미세 전자기계 시스템(MEMS) :
45. [핵심 정리] 유비쿼터스 네트워크나 초소형 휴먼 인터페이스 분야의 핵심요소인 3차원 미소 구조물, 센서 및 구동 장치 등을 소형화 및 고정밀화하고 복합화를 가능
46. [시험 포인트] : 기존의 2차 전지보다는 콘덴서 부품에서 기술적 근원을 가지고 있으며, 초고용량 커패시터로넘어오면서 기존 콘덴서 부품과 비교해 볼 때 사용처 등에서 유사한 흐름을 유지하였으므로 2차 전지와 구분됨
47. [시험 포인트] : 원자나 분자를 적절하게 결합시켜 새로운 미세한 구조를 만들어내어 기존 물질을 변형하거나 새로운 물질을 창출하는 것을 가능하게 하는 초미세기술
48. [시험 포인트] : 나노 수준의 새로운 원천 기술의 개발과 신물질합성을 통한 산업적 적용이 가능하며, 학문 간, 기술 간 급격한 융합화를 유도함
49. [시험 포인트] : 나노 소재, 나노 소자, 나노 바이오, 나노 에너지·환경, 나노 공정·측정·장비, 나노 안전성
50. [핵심 정리] : 원자나 분자를 적절하게 결합시켜 새로운 미세한 구조를 만들어내어 기존 물질을 변형하거나 새로운 물질을 창출하는 것을 가능하게 하는 초미세기술
51. [핵심 정리] : 나노 수준의 새로운 원천 기술의 개발과 신물질합성을 통한 산업적 적용이 가능하며, 학문 간, 기술 간 급격한 융합화를 유도함
52. [핵심 정리] : 나노 소재, 나노 소자, 나노 바이오, 나노 에너지·환경, 나노 공정·측정·장비, 나노 안전성
53. [핵심 정리] : 화학 반응을 통해 전기를 발생시키는 화학전지와 물리적 반응을 통해 전기를 발생시키는 물리전지
54. [핵심 정리] : 기존의 2차 전지보다는 콘덴서 부품에서 기술적 근원을 가지고 있으며, 초고용량 커패시터로넘어오면서 기존 콘덴서 부품과 비교해 볼 때 사용처 등에서 유사한 흐름을 유지하였으므로 2차 전지와 구분됨
55. [핵심 정리] : 수소와 산소의 반응을 통해 전기와 열, 물을 생산하는 고효율 무공해 전기화학장치로서 반응물을 공급하는 한 지속적인 전력 생산이 가능한 일종의 발전기
56. [시험 포인트] 유비쿼터스 네트워크 최종 사용자(End-User) 기능 : 최종 사용자 기능은 연결과 협력을 위한 최종 사용자 측면의 기능
57. [시험 포인트] 유비쿼터스 네트워크 기술
58. [시험 포인트] : PC 뿐만 아니라 모든 단말기가 네트워크에 항상 접속되어, 누구든지 시간과 장소의 제약 없이 네트워크를 활용한 다양한 서비스를 제공받을 수 있는 환경 또는 이를 가능하게 하는 기술
59. [시험 포인트] 유비쿼터스 네트워킹을 위한 구조적 모델
60. [핵심 정리] 유비쿼터스 네트워크 기술
61. [핵심 정리] : PC 뿐만 아니라 모든 단말기가 네트워크에 항상 접속되어, 누구든지 시간과 장소의 제약 없이 네트워크를 활용한 다양한 서비스를 제공받을 수 있는 환경 또는 이를 가능하게 하는 기술
62. [핵심 정리] 유비쿼터스 네트워킹을 위한 구조적 모델
63. [핵심 정리] 광대역 통합망 (BcN)
64. [핵심 정리] : 유선사업자들이 주도하는 차세대 기반 망과 무선 사업자들이 주축이 되는 통합 인터넷 프로토콜 망의 구축을 통하여 처음부터 융합된 망으로 시작한 후 단계적인 고도화를 통해 차세대 네트워크로 발전함
65. [핵심 정리] 사물인터넷에서의 사물 유형
66. [핵심 정리] 사물 인터넷 (IoT ; Internet of Things) :
67. [시험 포인트] 계속적으로 이루어지는 개인과 개인 사이의 거래 정보를 블록 단위로 저장하고, 구성원들에게 전송된 해당 블록의 유효성이 검증되면 기존 블록체인에 추가되는 방식으로 관리됨
68. [시험 포인트] 블록체인은 블록체인의 블록은 데이터를 저장하며, 헤더(header)와 몸체 부분(body)으로구성됨
69. [시험 포인트] -채굴자(miner) : 네트워크 내에서 보상을 얻는 참여자 노드
70. [핵심 정리] -채굴자(miner) : 네트워크 내에서 보상을 얻는 참여자 노드
71. [핵심 정리] -일반 참여자 : 블록체인 일부분의 데이터만 저장하는 라이트 노드
72. [핵심 정리] 블록체인은 블록체인의 블록은 데이터를 저장하며, 헤더(header)와 몸체 부분(body)으로구성됨
73. [핵심 정리] 블록체인은 참여자의 특성과 권한 범위, 참여자들의 네트워크 참여 방식과 활용에 따라 분류됨
74. [핵심 정리] 블록체인에서 사용하는 일반적인 합의 알고리즘
75. [시험 포인트] 컴퓨팅 리소스에 언제, 어디서나 필요에 따라 편리하게 네트워크를 통해 접근하는 기능을 제공하는 컴퓨팅 모델
76. [시험 포인트] -인터넷을 통해 액세스할 수 있는 IT 서비스로서 사용자가 신청하여 즉시 사용할 수 있고, 사용한 만큼 비용을 지불하는 컴퓨팅 서비스
77. [핵심 정리] -인터넷을 통해 액세스할 수 있는 IT 서비스로서 사용자가 신청하여 즉시 사용할 수 있고, 사용한 만큼 비용을 지불하는 컴퓨팅 서비스
78. [핵심 정리] 클라우드 컴퓨팅 서비스 모델
79. [시험 포인트] 센서 네트워크는 기본적으로 센서 노드(Sensor node)와 싱크 노드(Sink node)로 구성된다.
80. [시험 포인트] 싱크 노드는 센서 네트워크 내의 센서 노드들을 관리하고 제어하며, 센서 노드들이 수집한 데이터를 모아서 외부 네트워크로 전송하는 게이트웨이 역할을 수행한다.
81. [시험 포인트] USN 미들웨어는 물리적으로 USN 응용 서비스 시스템과 센서 네트워크의 중간에 위치하며, USN 응용 서비스 시스템과 센서 네트워크의 통합이 유연하게 이루어지도록 하는 역할을 수행한다.
82. [시험 포인트] USN 미들웨어는 다양한 질의 유형 지원, 센싱 정보 관리, 메타 정보 관리, 서로 다른 기종의 센서 네트워크 통합 지원, 상황정보 생성 및 관리, QoS 보장, 센싱 정보의 보안센서 노드의 위치 인식 등의 기능을 수행한다.
83. [시험 포인트] 유비쿼터스 네트워크는 여러 센서들이 수집한 데이터를 실시간으로 전송하기 위한 목적의 네트워크이다.
84. [시험 포인트] 센서는 일반적으로 측정 대상물을 감지 또는 측정하거나 전파의 강도를 감지/측정하여 유용한 신호로 변환하고, 그 측정량을 전기신호로 변환하는 장치이다. 즉, 물리량이나 화학량의 절대치와 변화, 혹은 소리, 빛 등을 감지한다.
85. [시험 포인트] 유비쿼터스 센서 네트워크는 여러 개의 센서 네트워크 영역이 게이트웨이를 통해 외부 네트워크에 연결되는 구조이며, 센서 노드는 집적된 데이터를 가까운 싱크 노드를 거쳐 게이트웨이로 전송한다.
86. [시험 포인트] 대부분의 USN 미들웨어는 응용 서비스 지원을 위하여 서버 시스템에 설치되거나 노드들의 원활한 동작과 성능 향상을 위하여 센서 노드와 싱크 노드에도 설치된다.
87. [핵심 정리] 유비쿼터스 네트워크는 여러 센서들이 수집한 데이터를 실시간으로 전송하기 위한 목적의 네트워크이다.
88. [핵심 정리] 센서는 일반적으로 측정 대상물을 감지 또는 측정하거나 전파의 강도를 감지/측정하여 유용한 신호로 변환하고, 그 측정량을 전기신호로 변환하는 장치이다. 즉, 물리량이나 화학량의 절대치와 변화, 혹은 소리, 빛 등을 감지한다.
89. [핵심 정리] 유비쿼터스 센서 네트워크는 여러 개의 센서 네트워크 영역이 게이트웨이를 통해 외부 네트워크에 연결되는 구조이며, 센서 노드는 집적된 데이터를 가까운 싱크 노드를 거쳐 게이트웨이로 전송한다.
90. [핵심 정리] 센서 네트워크는 기본적으로 센서 노드(Sensor node)와 싱크 노드(Sink node)로 구성된다.
91. [핵심 정리] 싱크 노드는 센서 네트워크 내의 센서 노드들을 관리하고 제어하며, 센서 노드들이 수집한 데이터를 모아서 외부 네트워크로 전송하는 게이트웨이 역할을 수행한다.
92. [핵심 정리] USN 미들웨어는 물리적으로 USN 응용 서비스 시스템과 센서 네트워크의 중간에 위치하며, USN 응용 서비스 시스템과 센서 네트워크의 통합이 유연하게 이루어지도록 하는 역할을 수행한다.
93. [핵심 정리] 대부분의 USN 미들웨어는 응용 서비스 지원을 위하여 서버 시스템에 설치되거나 노드들의 원활한 동작과 성능 향상을 위하여 센서 노드와 싱크 노드에도 설치된다.
94. [핵심 정리] USN 미들웨어는 다양한 질의 유형 지원, 센싱 정보 관리, 메타 정보 관리, 서로 다른 기종의 센서 네트워크 통합 지원, 상황정보 생성 및 관리, QoS 보장, 센싱 정보의 보안센서 노드의 위치 인식 등의 기능을 수행한다.
95. [시험 포인트] 상황에 대한 이해 : 상황정보의 선택/활용 방법/상황 인식 행위의 결정을 지원해줌
96. [시험 포인트] 상황(또는 상황정보, Context)의 정의 :
97. [시험 포인트] 상황인식 시스템의 구성 :
98. [시험 포인트] 상황인식 서비스 요소 기술 :
99. [시험 포인트] 상황인식 컴퓨팅(Context-Aware Computing)
100. [시험 포인트] : 실세계에서 획득한 상황정보를 추상화하고 연계하여 상황정보를 지식으로 만들고, 응용 분야에 적합한 지식 표현을 기반으로 사용자와 서비스를 연계하는 응용 기술
101. [핵심 정리] 상황인식 컴퓨팅(Context-Aware Computing)
102. [핵심 정리] : 실세계에서 획득한 상황정보를 추상화하고 연계하여 상황정보를 지식으로 만들고, 응용 분야에 적합한 지식 표현을 기반으로 사용자와 서비스를 연계하는 응용 기술
103. [핵심 정리] 상황인식 기술의 효율적인 이용을 위한 요구사항 :
104. [핵심 정리] 상황(또는 상황정보, Context)의 정의 :
105. [핵심 정리] 상황의 분류 :
106. [핵심 정리] 상황인식 시스템의 구성 :
107. [핵심 정리] 상황인식 응용 개발 도구 기술 :
108. [핵심 정리] 상황인식 서비스 요소 기술 :
109. [시험 포인트] 사람과 컴퓨터 시스템이 쉽고 편하게 상호작용하는 방법을 연구
110. [시험 포인트] 인간과 컴퓨터의 상호작용(HCI)
111. [핵심 정리] 인간과 컴퓨터의 상호작용(HCI)
112. [핵심 정리] 착용형(인간과 컴퓨터의)상호작용 기술
113. [핵심 정리] -(인간과 컴퓨터의)착용형상호작용 기술(Wearable HCI)은 인간 중심 기술의 중심에 있는 HCI 기술과 착용 컴퓨팅 기술의 융합 결과
114. [시험 포인트] 유비쿼터스 컴퓨팅 환경은 스마트폰, 태블릿 PC 등과 같은 다양한 단말기기들과 서비스 객체로 구성됨
115. [시험 포인트] ⇨ 서로 다른 운영체제나 통신 프로토콜을 갖는 단말기기가 독립적인 서비스 객체를 지원함
116. [시험 포인트] ⇨ 서비스 객체의 상호작용 과정에서 상호인증 작업이 필요함
117. [시험 포인트] ⇨ 유비쿼터스 환경의 다양한 단말기기들은 기존의 인증 시스템을 그대로 적용하기엔 부적절함
118. [시험 포인트] 부인방지(Non-repudiation)
119. [시험 포인트] 메시지의 송‧수신이나 교환 후, 또는 통신이나 처리가 실행된 후에 그 사실을 사후에 증명함으로써 사실 부인을 방지하는 보안 기술
120. [핵심 정리] 유비쿼터스 컴퓨팅 환경은 스마트폰, 태블릿 PC 등과 같은 다양한 단말기기들과 서비스 객체로 구성됨
121. [핵심 정리] ⇨ 서로 다른 운영체제나 통신 프로토콜을 갖는 단말기기가 독립적인 서비스 객체를 지원함
122. [핵심 정리] ⇨ 서비스 객체의 상호작용 과정에서 상호인증 작업이 필요함
123. [핵심 정리] ⇨ 유비쿼터스 환경의 다양한 단말기기들은 기존의 인증 시스템을 그대로 적용하기엔 부적절함
124. [핵심 정리] 부인방지(Non-repudiation)
125. [핵심 정리] 메시지의 송‧수신이나 교환 후, 또는 통신이나 처리가 실행된 후에 그 사실을 사후에 증명함으로써 사실 부인을 방지하는 보안 기술
126. [핵심 정리] 유비쿼터스 시대의 정보보호 요구사항
127. [시험 포인트] 스마트 자동차(Smart Car)는 ‘첨단 컴퓨터 · 통신 · 측정 기술 등을 이용하여 자동으로 운행할 수 있는 차량’이다. 즉, 자동차의 내외부 상황을 실시간 인식하여 도로 위의 위험에 대처할 수 있는 안전성과 탑승자의 만족을 극대화시키는 편의 기능을 갖춘 인간 친화적인 자동차이다.
128. [시험 포인트] 안전성 기술은 차량 결함, 사고 예방 및 회피, 충돌 등 위험 상황으로부터 운전자 및 탑승자를 보호하고 교통사고로 인한 피해를 경감하는 기술이다. 편의성 기술은 자동차에 흥미를 부여하고 운전자 편의를 극대화하여 자동차를 가정, 사무실에 이은 제3의 주거 공간으로 활용하는 기술이다.
129. [시험 포인트] 자율주행 기술은 운전자의 조작 없이 자동차 스스로 주행 환경을 인식하면서 목표 지점까지 운행하여, 운전자의 부주의로 인한 교통사고를 미연에 방지하고 편리한 운전 환경을 제공한다.
130. [핵심 정리] 스마트 자동차(Smart Car)는 ‘첨단 컴퓨터 · 통신 · 측정 기술 등을 이용하여 자동으로 운행할 수 있는 차량’이다. 즉, 자동차의 내외부 상황을 실시간 인식하여 도로 위의 위험에 대처할 수 있는 안전성과 탑승자의 만족을 극대화시키는 편의 기능을 갖춘 인간 친화적인 자동차이다.
131. [핵심 정리] 안전성 기술은 차량 결함, 사고 예방 및 회피, 충돌 등 위험 상황으로부터 운전자 및 탑승자를 보호하고 교통사고로 인한 피해를 경감하는 기술이다. 편의성 기술은 자동차에 흥미를 부여하고 운전자 편의를 극대화하여 자동차를 가정, 사무실에 이은 제3의 주거 공간으로 활용하는 기술이다.
132. [핵심 정리] 자율주행 기술은 운전자의 조작 없이 자동차 스스로 주행 환경을 인식하면서 목표 지점까지 운행하여, 운전자의 부주의로 인한 교통사고를 미연에 방지하고 편리한 운전 환경을 제공한다.
133. [시험 포인트] 빅데이터는 대개 3V(볼륨, 속도, 다양성) 혹은 5V(3V + 정확성, 가치)의 특징을 지닌다.
134. [시험 포인트] 향후 빅데이터는 여러 산업에서 비용 절감, 문제 해결, 트렌드 분석 등의 목적으로 활용될 것으로 예상되며, 그에 따라 해결해야 할 기술적인 요소도 많아지고 있다.
135. [시험 포인트] 대용량의 정형/비정형 데이터 집합을 빅데이터라고 하며, 이를 수집 · 저장 · 분석하여 의미 있는 결과를 도출하기 위해 사용한다.
136. [시험 포인트] 빅데이터를 활용하기 위해 빅데이터 수집 → 빅데이터 적재 → 빅데이터 처리 → 빅데이터 분석 과정을 수행하며, 각 단계별로 여러 세부 기술이 필요하다.
137. [핵심 정리] 대용량의 정형/비정형 데이터 집합을 빅데이터라고 하며, 이를 수집 · 저장 · 분석하여 의미 있는 결과를 도출하기 위해 사용한다.
138. [핵심 정리] 빅데이터는 대개 3V(볼륨, 속도, 다양성) 혹은 5V(3V + 정확성, 가치)의 특징을 지닌다.
139. [핵심 정리] 빅데이터를 활용하기 위해 빅데이터 수집 → 빅데이터 적재 → 빅데이터 처리 → 빅데이터 분석 과정을 수행하며, 각 단계별로 여러 세부 기술이 필요하다.
140. [핵심 정리] 향후 빅데이터는 여러 산업에서 비용 절감, 문제 해결, 트렌드 분석 등의 목적으로 활용될 것으로 예상되며, 그에 따라 해결해야 할 기술적인 요소도 많아지고 있다.
141. [시험 포인트] 딥러닝은 머신러닝의한 종류이며, 인공신경망(Artificial Neural Network)의 은닉층(hidden layer)을 여러 계층 쌓아서 만든 깊은 신경망(Deep Neural Network) 구조를 가진다.
142. [시험 포인트] 강화학습은 컴퓨터가 주어진 상황에 대해 적절한 행동을 수행한 후 적절한 보상을 수행하는 방식이며, 지도학습은 모든 학습 데이터(x)에 적당한 레이블(y)을 부여한 후에 기계가 입력집합 x를 모델에 넣어 결과에 해당하는 y값과 비교함으로써 모델의 성능을 높이는 학습을 진행한다. 그리고, 비지도학습은 주어진 데이터에 대한 어떤 지식이나 레이블도 없는 데이터를 다룬다.
143. [시험 포인트] 인공신경망은 인간의 뇌에 있는 뉴런 구조를 흉내 낸 모델로 인공지능에서 활용하는 기술 중 하나이다. 즉, 뇌에 있는 뉴런은 여러 계층으로 나열되어 있고, 인접한 계층의 뉴런들이 서로 연결되어 있다. 외부의 입력은 입력층으로 전달되고, 각 뉴런은 입력 신호들에 대하여 내부에 저장된 가중치(weight)를 이용하여 적절한 출력을 계산하여 다음 계층으로 보내게 된다.
144. [시험 포인트] 머신러닝(Machine Learning, 기계학습)은 인공지능 기술 중 하나이며, 관련 분야의 데이터를 바탕으로 지식을 자동으로 습득하여 스스로 성능을 향상시키는 기술이다.
145. [시험 포인트] 강화학습은 행동을 수행할 에이전트가 현재 상태를 기반으로 하여 행동을 선택하고, 가능한 행동 중에서 가장 큰 보상을 받을 수 있는 행동이 무엇인지 스스로 학습하도록 한다.
146. [핵심 정리] 인공신경망은 인간의 뇌에 있는 뉴런 구조를 흉내 낸 모델로 인공지능에서 활용하는 기술 중 하나이다. 즉, 뇌에 있는 뉴런은 여러 계층으로 나열되어 있고, 인접한 계층의 뉴런들이 서로 연결되어 있다. 외부의 입력은 입력층으로 전달되고, 각 뉴런은 입력 신호들에 대하여 내부에 저장된 가중치(weight)를 이용하여 적절한 출력을 계산하여 다음 계층으로 보내게 된다.
147. [핵심 정리] 머신러닝(Machine Learning, 기계학습)은 인공지능 기술 중 하나이며, 관련 분야의 데이터를 바탕으로 지식을 자동으로 습득하여 스스로 성능을 향상시키는 기술이다.
148. [핵심 정리] 딥러닝은 머신러닝의한 종류이며, 인공신경망(Artificial Neural Network)의 은닉층(hidden layer)을 여러 계층 쌓아서 만든 깊은 신경망(Deep Neural Network) 구조를 가진다.
149. [핵심 정리] 강화학습은 컴퓨터가 주어진 상황에 대해 적절한 행동을 수행한 후 적절한 보상을 수행하는 방식이며, 지도학습은 모든 학습 데이터(x)에 적당한 레이블(y)을 부여한 후에 기계가 입력집합 x를 모델에 넣어 결과에 해당하는 y값과 비교함으로써 모델의 성능을 높이는 학습을 진행한다. 그리고, 비지도학습은 주어진 데이터에 대한 어떤 지식이나 레이블도 없는 데이터를 다룬다.
150. [핵심 정리] 강화학습은 행동을 수행할 에이전트가 현재 상태를 기반으로 하여 행동을 선택하고, 가능한 행동 중에서 가장 큰 보상을 받을 수 있는 행동이 무엇인지 스스로 학습하도록 한다.

##### 정답 감각 리스트

1. 물리공간과 전자공간을 유기적으로 융합한 것을 무엇이라고 하는가 ? → 2
2. 물리공간과 전자공간의 연계를 위한 관점에 포함되지 않는 것은 무엇인가 ? → 4
3. 위 내용의 빈칸에 알맞은 것은 무엇인가 ? → 4
4. 유비쿼터스 컴퓨팅의 특징으로 틀리는 것은 무엇인가 ? → 2
5. 유비쿼터스 컴퓨팅 환경을 구현하기 위한 관심 분야로서 전자공간 사이의 좀 더 개선된 결합의 모습을 추구하는 개념은 무엇인가 ? → 4
6. 주관식 정답 입력칸 → 유비쿼터스 컴퓨팅
7. 실제로 존재하지 않은 환경이나 상황을 컴퓨터 등으로 구현한 후 인간의 감각을 이용하여 체험하게 하는 기술은 무엇인가 ? → 1
8. 미래 가속화 연구재단(ASF; Acceleration Studies Foundation)에서 정의한 메타버스 구성의 네 가지 시나리오가 아닌 것은 무엇인가 ? → 3
9. 소셜 미디어가 모임, 쇼핑, 게임 등이 가능한 메타버스로 발전한 형태인 소셜 기반 메타버스가 아닌 것은 무엇인가 ? → 4
10. SoC의 특징이 아닌 것은 무엇인가 ? → 3
11. SoC 제작과정중에서 펌웨어등의 개발이 관여되는 과정은 무엇인가 ? → 3
12. 다음 중 MEMS 기술의 특징이 아닌 것은 무엇인가 ? → 4
13. 나노 기술의 특징이 아닌 것은 무엇인가 ? → 3
14. 화학전지에 속하지 않는 것은 ? → 3
15. 나노미터 수준의 가공을 통해 나노미터 크기의 구조체를 인공적으로 형성하는 나노 기술의 접근 기술은 ? → 1
16. 유비쿼터스 네트워킹을 위한 구조적 모델에서 사물의 수행 환경과 상황정보는 통신하는 단말이나 사용자의 이동, 실행 환경의 변화 등 여러 통신 환경 변화에 따라 영향을 받기 때문에 각 사물은 예측할 수 없는 실행 상황에도 대처하는 계층은 무엇인가? → 1
17. 사물인터넷에서 서버상의 콘텐츠 같은 객체의 사물 유형은 무엇인가? → 4
18. 위 지문의 빈칸에 알맞은 것은 무엇인가? → 1
19. 블록체인의 특징이 아닌 것은 무엇인가? → 3
20. 승인된참여자만 참여할 수 있는 폐쇄형 블록체인 플랫폼은 무엇인가? → 4
21. 블록체인 플랫폼에서 특정 블록을 찾기 위해서는 식별자가 필요하며, 블록의 식별자는 ( )과 ( )으로 정의된다. → 1
22. 클라우드 서비스에 대한 설명으로 옳지 않은 것은 무엇인가? → 4
23. 클라우드에서 제공되는 가상 인프라를 제공하는 영역은 클라우드 아키텍처의 어느 부분인가? → 3
24. 클라우드 사용자에게 통신 네트워크를 통해 소프트웨어를 가상화하여 제공하는 클라우드 서비스 모델은 무엇인가? → 1
25. 일반적으로 측정 대상물을 감지 또는 측정하거나 전파의 강도를 감지/측정하여 유용한 신호로 변환하고, 그 측정량을 전기신호로 변환하는 장치는 무엇인가? → 1
26. 센서 네트워크의 센서 네트워크 프로토콜 아키텍처의 기능 측면에 해당되지 않는 것은 무엇인가? → 4
27. USN 미들웨어의 기능이 아닌 것은 무엇인가? → 1
28. 상황의 분류에 포함되지 않는 것은 무엇인가? → 1
29. 상황(또는 상황정보, Context)의 정의에 포함되지 않는 것은 무엇인가? → 4
30. 상황인식 서비스 요소 기술이 아닌 것은 무엇인가? → 1
31. HCI(Human-Computer Interactio)의 구성요소에 포함되지 않는 것은 무엇인가? → 1
32. 음성을 통한 HCI에 포함되지 않는 것은 무엇인가? → 4
33. 착용 컴퓨팅 기반의 HCI에서 운영체제, 미들웨어, 장치 관리/보안, 음성인식, 응용 프로그램 등의 기술 분류는 무엇에 해당하는가? → 3
34. 유비쿼터스 시대의 정보보호 요구사항에 포함되지 않는 것은 무엇인가? → 4
35. 메시지의 송‧수신이나 교환 후, 또는 통신이나 처리가 실행된 후에 그 사실을 사후에 증명함으로써 사실 부인을 방지하는 보안 기술은 무엇인가? → 4
36. 정상적인 기능을 하는 시스템으로 가장하여 프로그램 내에 숨어서 의도하지 않은 기능을 수행하는 백도어(backdoor) 프로그램 코드는 무엇인가? → 3
37. 일부 기능을 제외하면 정상적인 주행 혹은 사고나 충돌 임박 상황에서 자동차 제어권을 운전자가 소유하는 자동차의 자동화 단계는 무엇인가? → 4
38. 차량 상태의 위험 상황과 운전자 상태를 감지하여 사고 발생 가능성이 있는 경우, 사고를 회피시키는 차량 제어 기술은 무엇인가? → 4
39. 차량과 주변 인프라망이 유무선 통신에 의해 접속되는 단말과 서버 간에 이루어지는 무선통신. 차량에 인터넷 프로토콜(IP) 기반의 교통정보 및 안전운행 정보를 내려받을 수 있는 서비스는 무엇인가? → 3
40. 저장될 데이터에 대하여 미리 정해진 규칙(혹은 스키마)을 정의한 후, 그에 따라 저장된 데이터는 무엇인가? → 1
41. 위 지문은 무엇에 대한 설명인가? → 1
42. 여러 서버를 연결하여 대용량 빅데이터를 분산처리할 수 있도록 지원하는 자바(Java) 기반의 오픈소스 프레임워크는 무엇인가? → 3
43. 모든 학습 데이터(x)에 적당한 레이블(y)을 부여한 후, 기계가 입력집합 x를 모델에 넣어 결과에 해당하는 y값과 비교함으로써 모델의 성능을 높이는 학습 방식은 무엇인가? → 1
44. 위 지문은 무엇에 대한 설명인가? → 3
45. 출력되는 값이 분류 문제처럼 N개의 값 중 하나를 예측하는 것이 아니라, 연속된 값 중 하나를 출력하는 것은 무엇인가? → 1

#### 전체 30초 루프

- HTML5 = HTML 구조, CSS 표현, JS 동작, 폼, 시맨틱, 선택자, 박스모델, DOM/BOM, Canvas, API.
- Java = JVM, 기본형/참조형, 클래스/상속/인터페이스, 제네릭/람다, 예외, java.lang, IO/NIO, 컬렉션/스트림, 스레드, JDBC, 모듈.
- Python = 컴퓨터 기본, 인터프리터, 자료형, 조건/반복, 함수, 컬렉션, 객체, 모듈, 파일, 실전 코드 결과.
- 데이터정보처리 = 분석절차, 데이터 입력/점검, 통계정보 검색, 문서작성, 엑셀, 그래프, R, Python 분석.
- 유비쿼터스 = 물리+전자공간, 상황인식, VR/메타버스, SoC/MEMS/나노/전지, 네트워크, 블록체인/핀테크, 클라우드, USN, HCI/HRI, 보안, 스마트카, 빅데이터, AI.

## 지하철 압축본

### 지하철 암기용 요약본

> 목표: 지하철에서 반복해서 소리 없이 외우는 용도.  
> 원칙: **범위 먼저 → 과목별 핵심어 → 헷갈리는 짝 비교 → 정답 감각**.

#### 0. 시험 범위 먼저 외우기

| 과목 | 중간/대체 | 기말 |
|---|---|---|
| Java프로그래밍 | 교재 1~7장 + 해당 강의 | 교재 전체 + 멀티미디어 강의 |
| 데이터정보처리입문 | 대체: 온라인과제물, 학교 공지 | 교재 + 강의 전 범위 |
| HTML5웹프로그래밍 | 교재 1~3장, 강의 1~9강 | 교재 1~7장, 강의 1~15강 |
| 파이썬프로그래밍기초 | 대체: 강의 1~6강 | 멀티미디어 강의 전체 |
| 유비쿼터스컴퓨팅개론 | 중간과제물: 학교 공지 | 교재 전체 + 멀티미디어 강의 |
...
##### Java
- JVM+API, 클래스, 객체, 생성자, 오버로딩/오버라이딩, static/final, 제네릭, 컬렉션, 스트림, JDBC.

##### 데이터정보처리
- 문제정의-수집-정리분석-평가, 범주형/양적, 평균/중앙값, SPSS/SAS/R/Python, 엑셀 참조, 그래프 선택.

##### HTML5
- HTML/CSS/JS, 시맨틱, form/input, CSS 우선순위, 박스모델, DOM/BOM, Canvas, Storage/API.

##### Python
- 들여쓰기, 자료형, 슬라이싱, if/for/while, 함수, list/tuple/dict/set, class/self, import, with open.

##### 유비쿼터스
- 물리+전자공간, 상황인식, SoC/MEMS/RFID/USN, 클라우드, 블록체인, VR/AR/MR, 빅데이터 3V, 보안 CIA.

---

#### 7. 마지막 날 체크리스트

- [ ] HTML5 대체시험이면 **강의 1~9강** 먼저.
- [ ] Java 대체시험이면 **1~7장/강** 먼저.
- [ ] Python 대체과제면 **1~6강** 먼저.
- [ ] 데이터정보처리/유비쿼터스 과제는 학교 공지 최종 확인.
- [ ] 기말은 전 과목 전 범위 기준으로 복습.
- [ ] 각 과목 문서의 `연습문제`는 정답 가리고 한 번 더 풀기.
