...
updated : 2026-05-30 12:00:00 +0900
...
각 강 멀티미디어 강의 체크포인트(강당 2문항)의 정답과 풀이다. 문항 본문은 U-KNOU 강의의 체크포인트 화면에서 확인한다.

**1강**
- **1번 · 정답 2.** 컴퓨터는 숫자만 처리하는 것이 아니라 이미지·문자 등 다양한 데이터를 다룬다.
- **2번 · 정답 2.** 터치스크린은 화면 출력과 손동작 입력을 함께 수행하는 입출력 겸용 장치다. 모니터는 처리 결과를 보여 주는 출력장치에 해당한다.

**2강**
- **1번 · 정답 4.** 파이썬만으로 완전한 애플리케이션을 개발하기는 어려워 Rust·Go 등을 대안으로 고려한다. 인터프리터 언어라 자바나 C보다 실행 속도가 빠르지 않다.
- **2번 · 정답 3.** 인터프리터나 컴파일러가 없으면 기계어로 해석할 수 없어 실행되지 않는다. 인터프리터는 운영체제별로 존재해야 한다.

**3강**
- **1번 · 정답 3.** 상품 가격은 여러 값을 합산할 수 있어야 하므로 계산이 가능한 양적 자료여야 한다.
- **2번 · 정답 4.** 변수명은 문자나 언더바로 시작할 수 있어 해당 보기는 식별자 규칙에 어긋나지 않는다.

**4강**
- **1번 · 정답 1.** 한 변수에 두 값을 동시에 저장할 수 없고, 값이 자동으로 40으로 바뀌지도 않는다. 비교는 별도의 연산자가 담당하며 `=`는 할당 연산자다.
- **2번 · 정답 4.** `+` 연산자는 문자열과 문자열도 이어 붙일 수 있다.

**5강**
- **1번 · 정답 2.** 조건은 왼쪽부터 평가되어 true → false → true 순으로 판정된다.
- **2번 · 정답 3.** `10 / 0`은 그 자체로는 오류지만, `and` 앞의 `x != 0`이 false라 단락 평가로 뒤 조건을 보지 않고 곧장 else로 넘어간다.

**6강**
- **1번 · 정답 3.** while 안에서 count를 증가시켜야 한다(`count = count + 1` 또는 `count += 1`). 강의의 3단계 반복문 구성 전략을 참고한다.
- **2번 · 정답 3.** i가 4일 때도 실행되므로 1 + 2 + 3 + 4 = 10이다.

**7강**
- **1번 · 정답 2.**
- **2번 · 정답 4.** 스코프상 함수 밖에서는 r과 h를 사용할 수 없으며, 정의만으로는 출력이 일어나지 않는다.

**8강**
- **1번 · 정답 3.** 세트는 중복을 제거하므로 같은 점수가 있으면 사라진다. 문자열·불리언 변수는 여러 학생의 점수를 담기에 적절하지 않다. 리스트는 중복을 허용하면서 순서가 보장되고 원소 수정도 가능하다.
- **2번 · 정답 2.** 연산을 마치면 전체 원소는 6개다. append는 맨 뒤에 값을 붙이고, pop은 해당 인덱스의 값을 꺼내면서 리스트에서 삭제하며, insert는 지정한 인덱스에 값을 추가한다.

**9강**
- **1번 · 정답 2.** 순서가 보장되는 자료구조는 리스트와 튜플이며, 그중 튜플은 순서를 지키면서 원소 수정이 불가능하다.
- **2번 · 정답 2.** 딕셔너리는 숫자 값을 저장할 수 있고, 순서가 없어 인덱스로 접근할 수 없으며 키를 통해서만 접근한다.
...
**11강**
- **1번 · 정답 2.** `random.choice`는 값을 하나만 고르고, math 모듈에는 sample 함수가 없다. sample을 쓰려면 대상에 구현돼 있어야 하는데 students는 리스트다.
- **2번 · 정답 2.** `import math`로 가져오면 `math.factorial`로 호출하고, `from math import ...` 구문을 쓰면 `factorial`만으로 호출할 수 있다.

**12강**
- **1번 · 정답 1.** 현재 작업 폴더 안 data 폴더의 survey.txt는 `"./data/survey.txt"`, 한글 파일이라 `encoding="utf-8"`, 읽기 목적이라 모드는 `"r"`이다.
- **2번 · 정답 2.** 자동으로 줄바꿈되지 않으므로 `\n`으로 줄바꿈을 명시해야 한다.

**13강**
- **1번 · 정답 3.** 모든 단어가 중요한 것은 아니므로 불용어 처리를 적용해야 한다.
- **2번 · 정답 2.** 출력 형태는 `great, great, slow.`이다.

**14강**
- **1번 · 정답 2.** 계획 단계에서는 어떤 게임을 만들지와 장단점을, 분석 단계에서는 실제로 게임을 어떻게 진행할지를 다룬다.
- **2번 · 정답 3.** quit 입력 시 break로 반복을 끝낸다(방향키 입력은 좌표를 변경한다).
