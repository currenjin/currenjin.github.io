...
| 주제 | 외울 문장 |
|---|---|
| 분석 절차 | 데이터 분석은 문제 정의 → 수집 → 정리/분석 → 평가/해석 순서다. |
| 데이터와 정보 | 데이터는 관찰·측정된 값, 정보는 데이터를 목적에 맞게 처리한 결과다. |
| 자료 종류 | 범주형은 성별·지역처럼 분류값, 양적은 키·점수처럼 수치값이다. |
| 모집단·표본 | 모집단은 관심 대상 전체, 표본은 모집단에서 뽑은 일부다. |
| 중심 측도 | 평균은 모두 더해 개수로 나눈 값(이상치 약함), 중앙값은 정렬했을 때 가운데(이상치 강함), 최빈값은 가장 자주 나타나는 값이다. |
| 산포 측도 | 분산은 평균에서 얼마나 흩어졌는지(제곱 단위), 표준편차는 분산의 제곱근(원 단위)이다. |
| 통계 패키지 | SPSS는 GUI 중심, SAS는 대용량 강점, R은 오픈소스 통계 언어, Python은 범용+분석, Anaconda는 Python/R 배포판이다. |
| 엑셀 함수 | 엑셀 함수 입력은 `=`로 시작한다. |
| 셀 참조 | 상대참조는 복사 시 위치가 변함, 절대참조(`$A$1`)는 고정, 혼합참조(`$A1` / `A$1`)는 열 또는 행만 고정이다. |
| 그래프 종류 | 막대는 범주 비교, 꺾은선은 시간 변화, 원은 구성비, 산점도는 두 변수 관계에 쓴다. |
| pandas 기본 | `pd.read_csv()`는 CSV 읽기, `describe()`는 요약 통계 출력이다. |
...
| 주제 | 외울 문장 |
|---|---|
| 웹 구성 3요소 | HTML은 구조, CSS는 표현, JavaScript는 동작이다. |
| 문서 기본 | `<!DOCTYPE html>`은 HTML5 문서 선언, `<meta charset="UTF-8">`은 문자 깨짐 방지, viewport는 모바일 화면 대응 설정이다. |
| 텍스트 태그 | h1~h6은 제목 구조, `br`은 줄바꿈, `hr`은 주제 전환이다. |
| 컨테이너 | div는 블록·span은 인라인 컨테이너이며, 시맨틱 태그(header/nav/main 등)는 영역의 의미를 드러낸다. |
| 목록 | ul은 순서 없음, ol은 순서 있음, dl은 설명 목록이다. |
| 링크와 이미지 | `a href`는 하이퍼링크, `img src alt`는 이미지와 대체 텍스트다. |
| 표 | table은 표, tr은 행, th는 제목 셀, td는 데이터 셀이다. |
| 미디어 | audio와 video는 외부 플러그인 없이 미디어를 재생한다. |
| 폼 | form은 입력 묶음·input은 입력 요소이고, label은 입력 설명과 접근성을 높이며, required는 필수 입력 검사를 지정한다. |
| CSS 선택자 우선순위 | inline > id > class/속성/가상클래스 > 요소. |
| 박스/배치 | 박스 모델은 content → padding → border → margin 순서이고, display는 block(한 줄)·inline(내용만큼)을 결정하며, position은 요소 배치 방식을 정한다. |
| 변형·전환·애니메이션 | transform은 모양/위치 변형, transition은 값 변화의 부드러운 전환, animation은 keyframes 기반 반복 동작이다. |
| DOM/BOM | DOM은 HTML 문서를 객체로 다루는 모델, BOM은 브라우저 창·주소·기록 등을 다루는 모델이다. |
| 이벤트 | addEventListener는 이벤트 처리 함수를 등록한다. |
| 저장소 | localStorage는 영구 저장, sessionStorage는 세션 동안 저장이다. |
| Canvas·API | canvas 2D 컨텍스트는 `getContext("2d")`로 얻고, Geolocation은 위치 정보를 얻는 API다. |
...
| 주제 | 외울 문장 |
|---|---|
| Java 기본 | Java는 클래스 기반 객체지향 언어이고 JVM 위에서 바이트코드가 실행되며, Java 플랫폼은 JVM + Java API이고 개발에는 JDK가 필요하다. |
| 클래스와 객체 | Java 프로그램의 기본 구성 단위는 클래스이며, 클래스는 데이터와 메서드로 객체를 추상화한 틀이다. |
| 변수와 타입 | 변수는 인스턴스·클래스·지역·파라미터로 나뉘고, 기본형 8개는 byte/short/int/long/float/double/char/boolean이며, 클래스·배열은 참조형이다. 묵시적 형변환은 손실 없을 때 자동, 명시적은 캐스팅이고, 명령행 매개변수는 `main`의 `String[] args`로 전달된다. |
| 접근 제어 | 일반 최상위 클래스는 `public` 또는 생략만 가능하고, 생략은 같은 패키지에서 접근 가능(package-private)이다. |
| 생성자 | 생성자는 클래스 이름과 같고 반환형이 없으며, `this()`는 같은 클래스 생성자, `super()`는 부모 생성자를 호출한다. |
| 배열과 문자열 + | 배열은 생성 후 사용하고 크기는 `배열.length`로 얻으며, 문자열에 `+`를 적용하면 문자열 연결이다. |
| static과 초기화 | 모든 객체가 공유하는 데이터는 `static` 필드로 두고, 초깃값은 필드 선언문·초기화 블록·생성자에서 지정한다. |
| 오버라이딩과 오버로딩 | 오버라이딩은 부모 메서드 몸체를 자식 클래스에서 다시 정의, 오버로딩은 같은 이름의 메서드를 매개변수 목록으로 구별하는 것이다. |
| this와 super | `this`는 현재 객체, `super`는 부모 객체 쪽을 가리킨다. |
| 인터페이스와 추상 클래스 | 인터페이스는 추상 메서드 중심이며 default/static 메서드는 몸체가 있다. 의미적으로 유사한 클래스 묶음에는 추상 클래스를, 기능적으로 유사한 클래스 묶음에는 인터페이스를 사용한다. |
| enum | 열거형 상수값은 하나의 객체처럼 취급된다. |
| 다형성 | 다형성은 오버로딩·오버라이딩·상속·형변환·동적 바인딩으로 구현된다. |
| 익명 클래스 | 익명 클래스는 이름 없이 일회성 상속/구현 객체를 만든다. |
| 제네릭 | 제네릭 타입은 자료형을 매개변수로 가지는 클래스/인터페이스이고, 타입 파라미터는 필드형·반환형·인자형으로 쓰일 수 있으며, 제네릭 메서드는 자료형을 매개변수로 가지는 메서드다. |
| 람다 | 람다식은 함수형 인터페이스 객체 생성을 간결하게 표현하며, 람다식 결과 타입에 해당하는 인터페이스가 타깃 타입이다. |
| 패키지와 import | 패키지는 관련 클래스·인터페이스의 계층적 묶음, `import`로 다른 패키지 클래스를 편하게 쓰며, 필요한 클래스/패키지는 CLASSPATH에서 찾을 수 있어야 한다. |
| 예외 처리 | 예외는 실행 중 정상 흐름을 벗어난 비정상 상황이고, checked Exception은 반드시 처리하거나 `throws`로 선언해야 하며, 직접 처리는 `try-catch`·전파는 `throws`다. |
| Object와 java.lang | `toString()`/`equals()`는 Object에 정의되어 재정의 가능하고, java.lang 패키지는 import 없이 사용한다. |
| 문자열 클래스 | String은 immutable이라 생성 후 변경 불가, 자주 변경되는 문자열은 StringBuffer/StringBuilder가 효율적이며, 기본형 값을 객체로 다루기 위해 Wrapper 클래스가 있다. |
| 표준 입출력 | System은 in·out·err 표준 입출력 객체를 제공한다. |
| 스트림 I/O | 스트림은 생산자/소비자 종류와 무관하게 입출력을 수행하게 하고, Buffered 계열은 보조 스트림으로 버퍼링 성능을 제공하며, InputStreamReader는 바이트 입력을 문자 입력으로 변환한다. |
| 파일 I/O | File은 파일/디렉터리를 표현하지만 입출력 메서드는 없고, RandomAccessFile은 파일 포인터 위치에서 임의 읽기/쓰기가 가능하다. |
| NIO | java.nio는 java.io를 개선한 새 입출력 패키지이고 Path는 풍부한 경로 기능, Files는 조작·읽기/쓰기 static 메서드, Buffer는 채널 입출력의 기본형 데이터 보관소, FileChannel은 입출력 모두 제공·기본 버퍼링 사용, WatchService는 디렉터리 이벤트 감시를 담당한다. |
| 컬렉션(JCF) | JCF는 컬렉션을 효율적으로 다루기 위한 API 프레임워크다. Set은 순서 없음+중복 X, List는 순서 있음+중복 OK, Queue는 순서 있음+FIFO, Map은 key-value 관리이며, LinkedList는 List와 Queue를 모두 구현하고 스택 메서드도 제공한다. |
| 스트림 처리 | 내부 반복은 반복 코드를 직접 쓰지 않고 처리 코드만 람다로 전달하며, `forEach`는 외부로 꺼내지 않고 내부 탐색이다. 필터링은 중복 제거/조건 필터(중간 연산), 매핑은 원소 변환(중간 연산), 매칭은 조건 만족 확인(최종 연산), 수집은 List/Set/Map으로 취합(최종 연산)이다. |
| 스레드 | 스레드 실행 코드는 `public void run()`에 정의하고, 스레드는 Thread 상속 또는 Runnable 구현으로 만들며, `start()`는 새 스레드를 시작해 `run()`을 실행한다. start·join·interrupt·yield·sleep·wait·notify 등 제어 메서드를 구분하고, 상태는 Startable·Runnable·Running·Not Running 등을 거치며, `synchronized`는 공유 객체 접근을 한 번에 한 스레드로 제한한다. |
| JDBC | JDBC는 Java에서 관계형 DB와 연동하는 표준 API이고, MariaDB는 MySQL 호환 오픈소스 RDBMS이며, JDBC 드라이버 jar를 Classpath에 추가해야 한다. DBMS 연결은 java.sql.Connection, SQL 실행은 Statement의 execute()/executeQuery()/executeUpdate()이고 SELECT는 executeQuery()(결과는 ResultSet), INSERT/UPDATE/DELETE는 executeUpdate()를 사용한다. |
| 라이브러리·모듈 | 라이브러리는 자주 쓰는 클래스/인터페이스를 컴파일된 형태(`.jar`)로 제공한 것이고, 모듈은 라이브러리의 업그레이드로 패키지 단위 접근성 설정이다. 라이브러리는 Classpath, 모듈은 Modulepath에 추가하고, `module-info.java`에서 `exports`/`requires`로 의존관계를 설정하며, `java.base`는 가장 기본 모듈이다. |
...
| 주제 | 외울 문장 |
|---|---|
| Python 특징 | Python은 인터프리터, 동적 타입, 들여쓰기 블록 언어다. |
| 변수와 자료형 | 변수는 값을 가리키는 이름이고, 주요 자료형은 int/float/str/bool/list/tuple/dict/set이다. |
| 입출력 | input은 문자열을 입력받고(숫자는 변환 필요), print는 값을 화면에 출력한다. |
| 대입/비교 | `=`는 대입, `==`는 비교다. |
| 문자열 | 문자열 인덱스는 0부터 시작하고, 슬라이싱은 end를 포함하지 않는다. |
| 조건과 반복 | 조건문은 if/elif/else, for는 정해진 반복·while은 조건 반복에 주로 쓰고, `range(n)`은 0부터 n-1까지(끝 미포함), `break`는 반복문 종료, `continue`는 이번 반복만 건너뛴다. |
| 함수 | 함수는 `def name(params):`로 정의하고, `return`은 값 반환과 함수 종료이며, 지역변수는 함수 안에서만 유효, 전역변수 변경에는 `global` 선언이 필요하다. |
| 컬렉션 | list는 순서 O·변경 O, tuple은 순서 O·변경 X, dict는 key-value 구조, set은 중복을 허용하지 않는다. |
| 클래스 | 클래스는 객체의 설계도이고 `__init__`은 객체 생성 시 초기화 메서드이며 `self`는 객체 자기 자신을 가리킨다. |
| 모듈과 파일 | `import`는 모듈을 가져오고, 파일은 `with open(...) as f`로 열면 자동으로 닫힌다. |
...
| 주제 | 외울 문장 |
|---|---|
| 기본 개념 | 유비쿼터스는 언제 어디서나 자연스럽게 컴퓨팅을 이용하는 환경이고, 유비쿼터스 공간은 물리공간과 전자공간의 융합이며, 상황인식은 사용자·위치·시간·환경 정보를 바탕으로 서비스한다. |
| HCI / HRI | HCI는 인간과 컴퓨터의 상호작용, HRI는 인간과 로봇의 상호작용이다. |
| 하드웨어 요소 | SoC는 여러 기능을 하나의 칩에 통합, MEMS는 초소형 기계·전자 시스템, RFID는 무선 주파수로 대상 식별, USN은 유비쿼터스 센서 네트워크이며, 나노기술은 초미세 단위 물질 조작, 차세대 전지는 모바일·센서 기기의 지속성을 높인다. |
| 유비쿼터스 네트워크 | 언제 어디서나 연결되는 네트워크다. |
| 클라우드 | 클라우드는 컴퓨팅 자원을 네트워크로 제공하며, 장점은 확장성·비용 효율·접근성, 단점은 보안·장애 의존성·개인정보 문제다. |
| 블록체인 / 핀테크 | 블록체인은 분산 원장으로 위변조가 어렵고, 핀테크는 금융과 IT의 결합이다. |
| 가상/현실 기술 | VR은 완전한 가상현실, AR은 현실 위에 정보 덧붙임, MR은 현실과 가상의 상호작용 혼합, 메타버스는 지속적 가상 세계와 사회적 상호작용이다. |
| 응용 분야 | 스마트카는 센서·통신·AI·자율주행 결합, 빅데이터 3V는 Volume·Velocity·Variety, AI는 학습·추론·인식 등 지능형 처리를 수행한다. |
| 정보보호 | 정보보호 3요소는 기밀성·무결성·가용성, 인증은 사용자가 누구인지 확인, 접근통제는 허가된 자원만 사용하게 제한한다. |
