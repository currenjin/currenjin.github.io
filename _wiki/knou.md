...
- 블록 지정 주의: 연습문제/기출 기준으로 **`F3=칸 단위, F4=줄 단위` 설명은 오답 포인트**. F3/F4를 실제 조작으로 깊게 재현하기보다, **F9 한자 변환 / 그림 삽입 가능 / 다단 편집 가능**처럼 확실한 보기를 먼저 잡아 검산한다.
- 한자 변환: 한글로 입력 후 [입력]-[한자로 변환], **`F9`**(또는 한자키)로 대신 / 두 글자 이상 단어는 **`F3`로 블록 지정** 후 변환 (`F5`로 한자입력을 대신한다는 설명은 틀림)
- 기출 단축키 묶음:
  - **`Ctrl+F10`** = 특수문자/문자표 입력(☎, ※ 같은 특수문자)
  - **`F9`** = 한자 변환, **`F5`는 한자 변환 아님**
  - **`F3`/`F4`** = 블록 지정 계열 함정으로 반복 출제; **`F3=칸 단위, F4=줄 단위`라고 단순 암기하지 말고 오답 보기로 경계**
  - **`Alt+N`** = 새 글, **`Alt+O`** = 불러오기, **`Alt+S`** = 저장하기
  - **`Ctrl+X/C/V`** = 잘라내기/복사/붙이기, **`Delete`/`Backspace`** = 글자 지우기
  - **왼쪽 `Shift+Space` 또는 `한/영`** = 글자판 전환, **`Caps Lock`** = 영문 대소문자 전환
...
- 이웃하지 않은 셀 다중 선택 = **`Ctrl` 키**를 누른 채 클릭(엑셀 기초 단축키형 기출)
