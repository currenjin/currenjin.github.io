...
- 데이터 분석은 문제 정의 → 수집 → 정리/분석 → 평가/해석 순서다.
...
- 정보는 데이터를 목적에 맞게 처리한 결과다.

##### 범주형

- 범주형 자료는 성별, 지역처럼 분류값이다.
...
- Python은 범용 프로그래밍과 데이터 분석에 모두 쓴다.

##### Anaconda

- Anaconda는 Python/R 데이터 분석 배포판이다.
...
- 막대그래프는 범주별 크기 비교에 쓴다.

##### 꺾은선

- 꺾은선그래프는 시간에 따른 변화를 보여준다.
...
- `<!DOCTYPE html>`은 HTML5 문서 선언이다.

##### 문자 인코딩

- `<meta charset="UTF-8">`은 문자 깨짐을 막는다.

##### 반응형

- viewport는 모바일 화면 크기 대응 설정이다.
...
- `br`은 줄바꿈, `hr`은 주제 전환이다.

##### 컨테이너

- div는 블록, span은 인라인 컨테이너다.

##### 목록

- ul은 순서 없음, ol은 순서 있음, dl은 설명 목록이다.
...
- `img src alt`는 이미지와 대체 텍스트다.

##### 표

- table은 표, tr은 행, th는 제목 셀, td는 데이터 셀이다.

##### 미디어

- audio와 video는 외부 플러그인 없이 미디어를 재생한다.

##### 시맨틱

- 시맨틱 태그는 영역의 의미를 드러낸다.

##### 폼

- form은 입력 묶음, input은 입력 요소다.

##### 접근성

- label은 입력 요소 설명과 접근성을 높인다.

##### 필수입력

- required는 입력 필수 검사를 지정한다.
...
- 박스 모델은 content → padding → border → margin 순서다.

##### display

- block은 한 줄 차지, inline은 내용만큼 차지한다.

##### position

- position은 요소 배치 방식을 정한다.

##### 변형

- transform은 요소 모양/위치를 변형한다.

##### 전환

- transition은 값 변화 과정을 부드럽게 한다.

##### 애니메이션

- animation은 keyframes 기반 반복 동작이다.

##### DOM

- DOM은 HTML 문서를 객체로 다루는 모델이다.

##### BOM

- BOM은 브라우저 창/주소/기록 등을 다루는 모델이다.

##### 이벤트

- addEventListener는 이벤트 처리 함수를 등록한다.

##### 저장

- localStorage는 영구, sessionStorage는 세션 동안 저장이다.

##### Canvas

- canvas 2D는 `getContext('2d')`로 얻는다.

##### API

- Geolocation은 위치 정보를 얻는 API다.
...
- Java는 클래스 기반 객체지향 언어이며 JVM 위에서 바이트코드가 실행된다.

##### Java 플랫폼

- Java 플랫폼은 JVM + Java API다.

##### JDK

- Java 프로그래밍을 하려면 JDK가 필요하다.

##### 클래스

- Java 프로그램의 기본 구성 단위는 클래스다.

##### 객체지향

- 클래스는 데이터와 메서드로 객체를 추상화한 틀이다.

##### 변수 종류

- 변수는 인스턴스 변수, 클래스 변수, 지역 변수, 파라미터로 나뉜다.

##### 기본형 8개

- 기본형은 byte, short, int, long, float, double, char, boolean이다.

##### 참조형

- 클래스와 배열은 참조형이고 참조값을 저장한다.

##### 형변환

- 묵시적 형변환은 손실 없을 때 자동, 명시적 형변환은 캐스팅이다.

##### main 인자

- 명령행 매개변수는 main의 String[] args로 전달된다.

##### 접근 제어

- 일반 최상위 클래스 접근 제어자는 public 또는 생략만 가능하다.

##### default 접근

- 접근 제어자 생략은 같은 패키지에서 접근 가능하다.
...
- 배열은 생성 후 사용하며 `배열.length`로 크기를 얻는다.

##### 문자열 +

- 문자열에 `+`를 적용하면 문자열 연결이다.

##### static 필드

- 모든 객체가 공유하는 데이터는 static 필드로 둔다.

##### 초기화

- 필드 선언문, 초기화 블록, 생성자로 초깃값을 지정한다.

##### 오버라이딩

- 부모 메서드 몸체를 자식 클래스에서 다시 정의하는 것이다.

##### 오버로딩

- 같은 이름의 메서드를 매개변수 목록으로 구별한다.

##### this/super

- this는 현재 객체, super는 부모 객체 쪽을 가리킨다.

##### this()/super()

- this()는 같은 클래스 생성자, super()는 부모 생성자를 호출한다.

##### 인터페이스

- 인터페이스는 추상 메서드 중심이며 default/static 메서드는 몸체가 있다.

##### 추상클래스

- 의미적으로 유사한 클래스 묶음은 추상 클래스를 쓴다.

##### 인터페이스 용도

- 기능적으로 유사한 클래스 묶음은 인터페이스를 쓴다.

##### enum

- 열거형 상수값은 하나의 객체처럼 취급된다.

##### 다형성

- 다형성은 오버로딩/오버라이딩/상속/형변환/동적 바인딩으로 구현된다.

##### 익명 클래스

- 익명 클래스는 이름 없이 일회성 상속/구현 객체를 만든다.

##### 제네릭 타입

- 제네릭 타입은 자료형을 매개변수로 가지는 클래스/인터페이스다.

##### 타입 파라미터

- 타입 파라미터는 필드형, 반환형, 인자형으로 쓰일 수 있다.

##### 제네릭 메서드

- 제네릭 메서드는 자료형을 매개변수로 가지는 메서드다.

##### 람다식

- 람다식은 함수형 인터페이스 객체 생성을 간결하게 한다.

##### 타깃 타입

- 람다식의 결과 타입에 해당하는 인터페이스가 타깃 타입이다.

##### 패키지

- 패키지는 관련 클래스와 인터페이스의 계층적 묶음이다.

##### import

- 다른 패키지 클래스를 편하게 쓰려면 import를 사용한다.

##### CLASSPATH

- 필요한 클래스/패키지는 CLASSPATH 경로에서 찾을 수 있어야 한다.

##### 예외

- 예외는 실행 중 정상 흐름을 벗어난 비정상 상황이다.

##### checked 예외

- checked Exception은 반드시 처리하거나 throws로 선언해야 한다.

##### try-catch

- 직접 처리는 try-catch, 전파는 throws다.

##### Object

- toString()과 equals()는 Object에 정의되어 있고 재정의될 수 있다.
...
- String은 immutable이라 생성 후 내용 변경이 불가능하다.

##### StringBuffer/Builder

- 자주 변경되는 문자열은 StringBuffer나 StringBuilder가 효율적이다.

##### Wrapper

- 기본형 값을 객체로 다루기 위해 포장 클래스가 있다.

##### System

- System은 in, out, err 표준 입출력 객체를 제공한다.

##### 스트림

- 스트림은 생산자/소비자 종류와 무관하게 입출력을 수행하게 한다.

##### 보조 스트림

- Buffered 계열은 보조 스트림으로 버퍼링 성능을 제공한다.

##### InputStreamReader

- InputStreamReader는 바이트 입력을 문자 입력으로 변환한다.

##### File

- File은 파일/디렉터리를 표현하지만 입출력 메서드는 제공하지 않는다.

##### RandomAccessFile

- RandomAccessFile은 파일 포인터 위치에서 임의 읽기/쓰기가 가능하다.

##### NIO

- java.nio는 java.io를 개선한 새 입출력 패키지다.

##### Path

- Path는 File보다 풍부한 파일 경로 기능을 제공한다.

##### Files

- Files는 파일/디렉터리 조작과 읽기·쓰기 static 메서드를 제공한다.

##### Buffer

- 채널 입출력에서 버퍼는 기본형 데이터 보관소다.

##### FileChannel

- FileChannel은 입출력 모두 제공하고 기본 버퍼링을 사용한다.

##### WatchService

- WatchService는 디렉터리 이벤트를 감시한다.

##### JCF

- JCF는 컬렉션을 효율적으로 다루기 위한 API 프레임워크다.

##### Collection

- 컬렉션은 여러 원소를 저장·관리하는 객체다.

##### Set

- Set은 순서 의미가 없고 중복을 허용하지 않는다.

##### List

- List는 순서 의미가 있고 중복을 허용한다.

##### Queue

- Queue는 순서 의미가 있고 FIFO 방식으로 자료를 관리한다.

##### Map

- Map은 key-value 쌍을 관리한다.

##### LinkedList

- LinkedList는 List와 Queue를 모두 구현하고 스택 메서드도 제공한다.

##### 내부 반복

- 내부 반복은 반복 코드를 직접 쓰지 않고 처리 코드만 람다로 전달한다.

##### forEach

- forEach는 원소를 외부로 꺼내지 않고 내부에서 탐색한다.

##### 필터링

- 필터링은 중복 제거 또는 조건 만족 원소만 걸러내는 중간 연산이다.

##### 매핑

- 매핑은 원소를 다른 원소로 변환하는 중간 연산이다.

##### 매칭

- 매칭은 조건 만족 여부를 확인하는 최종 연산이다.

##### 수집

- 수집은 결과 원소를 List/Set/Map 등으로 취합하는 최종 연산이다.

##### 스레드 코드

- 스레드 실행 코드는 `public void run()`에 정의한다.

##### Thread/Runnable

- 스레드는 Thread 상속 또는 Runnable 구현으로 만든다.

##### start

- start()는 새 스레드를 시작하고 run()을 실행하게 한다.

##### 제어 메서드

- start, join, interrupt, yield, sleep, wait, notify를 구분한다.

##### 상태

- 스레드는 Startable, Runnable, Running, Not Running 등의 상태를 거친다.

##### 동기화

- synchronized는 공유 객체 접근을 한 번에 한 스레드로 제한한다.

##### JDBC

- JDBC는 Java에서 관계형 DB와 연동하는 표준 API다.

##### MariaDB

- MariaDB는 MySQL 호환 오픈소스 RDBMS다.

##### 드라이버

- JDBC 사용에는 DB용 JDBC 드라이버 jar를 Classpath에 추가해야 한다.

##### Connection

- DBMS 연결은 java.sql.Connection 객체로 표현한다.

##### Statement

- SQL 실행은 execute(), executeQuery(), executeUpdate()를 사용한다.

##### executeQuery

- SELECT 실행은 executeQuery()이고 결과는 ResultSet이다.
...
- 라이브러리는 자주 쓰는 클래스/인터페이스를 컴파일된 형태로 제공한 것이다.

##### jar

- Java 라이브러리는 .class 파일을 묶은 .jar 형태다.

##### 모듈

- 모듈은 라이브러리의 업그레이드로 패키지 단위 접근성을 설정한다.

##### Classpath/Modulepath

- 라이브러리는 Classpath, 모듈은 Modulepath에 추가한다.

##### module-info

- module-info.java에서 exports와 requires로 의존관계를 설정한다.

##### java.base

- java.base는 Java 표준 모듈 중 가장 기본 모듈이다.
...
- Python은 인터프리터, 동적 타입, 들여쓰기 블록 언어다.
...
- 주요 자료형은 int, float, str, bool, list, tuple, dict, set이다.
...
- 조건문은 if, elif, else로 분기한다.

##### 반복

- for는 정해진 반복, while은 조건 반복에 주로 쓴다.

##### range

- range(n)은 0부터 n-1까지다.
...
- 함수는 `def name(params):`로 정의한다.

##### 반환

- return은 값을 반환하고 함수를 끝낸다.
...
- 전역변수 변경은 global 선언이 필요하다.

##### 리스트

- list는 순서가 있고 변경 가능하다.

##### 튜플

- tuple은 순서가 있고 변경 불가능하다.
...
- 파일은 `with open(...) as f`로 열면 자동으로 닫힌다.
...
- 유비쿼터스는 언제 어디서나 자연스럽게 컴퓨팅을 이용하는 환경이다.

##### 공간

- 유비쿼터스 공간은 물리공간과 전자공간의 융합이다.

##### 상황인식

- 상황인식은 사용자, 위치, 시간, 환경 정보를 바탕으로 서비스한다.

##### HCI

- HCI는 인간과 컴퓨터의 상호작용이다.
...
- SoC는 여러 기능을 하나의 칩에 통합한 것이다.

##### MEMS

- MEMS는 초소형 기계·전자 시스템이다.

##### RFID

- RFID는 무선 주파수로 대상을 식별한다.
...
- 나노기술은 초미세 단위 물질 조작 기술이다.

##### 차세대 전지

- 차세대 전지는 모바일/센서 기기의 지속성을 높인다.

##### 네트워크

- 유비쿼터스 네트워크는 언제 어디서나 연결되는 네트워크다.

##### 클라우드

- 클라우드는 컴퓨팅 자원을 네트워크로 제공한다.
...
- MR은 현실과 가상을 상호작용하게 섞는다.

##### 메타버스

- 메타버스는 지속적 가상 세계와 사회적 상호작용이다.

##### 스마트카

- 스마트 자동차는 센서, 통신, AI, 자율주행을 결합한다.

##### 빅데이터

- 빅데이터 3V는 Volume, Velocity, Variety다.

##### AI

- AI는 학습, 추론, 인식 등 지능형 처리를 수행한다.

##### 정보보호

- 정보보호 3요소는 기밀성, 무결성, 가용성이다.
