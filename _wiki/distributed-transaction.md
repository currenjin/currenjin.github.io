...
summary : 여러 시스템에 걸친 하나의 업무를 어긋나지 않게 — 2PC·Saga·Outbox·멱등 소비
...
이 셋을 푸는 접근이 크게 둘이다. 전체를 하나의 원자 커밋으로 묶거나(2PC), 로컬 커밋의 연쇄로 풀고 실패는 되돌리는 동작으로 상쇄하거나(Saga).
...
정합성은 얻지만 가용성·자율성·성능을 내주는 거래다. 서비스 자율성이 핵심인 MSA와는 반대 방향이라, 경계 안(단일 DB)에서는 로컬 트랜잭션, 경계 밖에서는 Saga 계열로 가는 것이 일반적이다.
...
- **권위 일원화**: 한 데이터의 상태 전이는 소유자 하나가 판정한다. 사가는 "전이해 달라"고 시도만 하고, 유효성 판정(지금 이 전이가 가능한 상태인가)은 소유 서비스가 가드와 낙관적 잠금으로 직렬화한다. 동시에 상충하는 요청이 와도 하나만 승자가 된다.
...
사가는 이벤트·커맨드 메시지로 굴러가는데, 그 메시지 발행 자체가 이중 쓰기 문제를 갖고 있다. 커밋은 됐는데 발행이 안 되면 사가가 멈추고, 발행은 됐는데 커밋이 롤백되면 유령 이벤트가 돈다.
...
| 격리 부재 | 시맨틱 락 · 낙관적 잠금 · 권위 일원화 |
| 전부를 하나의 원자 커밋으로 | 2PC — 경계 안에서만, MSA에서는 회피 |

2PC가 트랜잭션을 인프라에 맡기는 접근이라면, Saga+Outbox+멱등 소비는 트랜잭션을 애플리케이션 설계로 옮기는 접근이다. 원자성은 로컬 트랜잭션들에서, 최종 일관성은 메시지와 보상에서 얻는다.
